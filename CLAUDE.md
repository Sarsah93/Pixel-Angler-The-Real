...
- **직전 작업**: 87차 — **두족류 손질 잠금 해제(무늬오징어 14스테이지 완주)** + 어종별 도마 스프라이트 10키 + 돌돔 성별 배선.
  상세는 `docs/wiki/03-WORKLOG/2026-08-09-087-cephalopod-squid-tree.md`.
- **⏸ 재개 지점**: **무늬오징어 손질 수동 실플레이 검증**. 87차는 **자동 검증만** 통과했다
  (core 실판정 12/12 + Playwright 실렌더 + 부산물 10종 지급). 사람이 조작해 본 적이 없어
  유도선 위치·드래그 체감·연출 타이밍·안내 문구가 미확인이다. **나머지 3종 확장은 이 검증 뒤에.**
  재현 절차는 워크로그 087 §5.
- **완료**: 원물 자유 손질(돔류+방어류) · 넙치류 다섯장뜨기 · 광어 포 뜨기 3단계+칼 팔로우(84~86차) ·
  회썰기 2뷰 미니게임 + 유도선 dev 편집기(86차) · 사시미 접시 플레이팅 · 재장착 체인 · 장비 시스템 개편.
- **다음 착수**:
  **0.** 무늬오징어 수동 검증 ← **현재 지점**
  **1.** 두족류 3종 확장 (한치 15 / 갑오징어 13 / 문어 11 — 스펙 §11.3 ④~⑧) +
     시트 도트 추출(`gen_ceph_stages.cjs`) + 신규 가이드 9경로 F9 실측
  **2.** 광어 F9 잔여 (`upSep1/2`·`dnSep1/2`·`dnScore`) + 등쪽 단면 실사
  **3.** 해루질 관련 작업 (사용자 지정)
- **사용자 대기 에셋**: ① 광어 **등쪽 단면(사진 2번) 투명본** ② 두족류 **껍질·아가미·갑오징어 속껍질** 3종.
