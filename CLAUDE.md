...
## 현재 진행 상황 (2026-08-11) — 이어받기 요약

> 상세·우선순위·로드맵은 **`IMPLEMENTATION_PLAN.md` 최상단 "🚧 다음 착수"** 참고.

- **전체 위치**: Phase 6(게임플레이 심화) 안의 **손질(회뜨기) 시스템 확장** — 낚시 루프의 서브시스템 하나.
- **직전 작업**: 90차 — **무늬오징어 가이드 F9 실측 반영(1~3단계)** + 트리 12스테이지 개편(펼치기·결과
  확인 제거) + 당김 연출 분리(`drag_out` 칼 글리프 제거) + `evalNerveCut` 재작성.
  상세는 `docs/wiki/03-WORKLOG/2026-08-11-090-squid-guide-coords-stage-trim.md`.
- **🚧 재개 지점**: **무늬오징어 수동 검증 계속** — 1~3단계는 실측 반영 완료.
  **4단계(껍질 분리) 재구성은 에셋 대기**(껍질 온전 몸통 필수 — 요구사항은 90차 참고),
  5~6단계(아가미·날개·3분할·부리) F9 실측은 사용자 진행 중. **나머지 3종 확장은 이 검증 뒤에.**
- **완료**: 원물 자유 손질(돔류+방어류) · 넙치류 다섯장뜨기 · 광어 포 뜨기 3단계+칼 팔로우(84~86차) ·
  회썰기 2뷰 미니게임 + 유도선 dev 편집기(86차) · 사시미 접시 플레이팅 · 재장착 체인 · 장비 시스템 개편 ·
  두족류 도마 실사 도트 + 피사체 좌표계(89차).
- **다음 착수**:
  **0.** 무늬오징어 수동 검증 계속 (4단계 재구성 = 에셋 입수 시 / 5~6단계 실측) ← **현재 지점**
  **1.** 두족류 3종 확장 (한치 15 / 갑오징어 13 / 문어 11 — 스펙 §11.3 ④~⑧, 종별 실사 사진 필요)
  **2.** 광어 F9 잔여 (`upSep1/2`·`dnSep1/2`·`dnScore`) + 등쪽 단면 실사
  **3.** 해루질 관련 작업 (사용자 지정)
- **사용자 대기 에셋**: ① 오징어 **껍질 온전 몸통·분리 완료·껍질 단독**(4단계 재구성 — 90차 요구)
  ② 광어 **등쪽 단면(사진 2번) 투명본** ③ 두족류 **아가미(붙은 상태)·3분할 결과·갑오징어 속껍질**.
