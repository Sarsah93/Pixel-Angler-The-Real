...
## 현재 진행 상황 (2026-08-05) — 이어받기 요약

> 상세·우선순위·로드맵은 **`IMPLEMENTATION_PLAN.md` 최상단 "🚧 다음 착수"** 참고.

- **전체 위치**: Phase 6(게임플레이 심화) 안의 **회뜨기(손질) 시스템 마무리 단계** — 낚시 루프의 서브시스템 하나.
- **완료**: 원물 자유 손질(돔류+방어류 — 가이드 좌표 어종군 분리까지) · 회썰기 2뷰 미니게임 · **사시미 접시 플레이팅 + 가격 체계**(필렛 = 원물가 분할 · 모듬 고정표 · 단품 = 괴리율 보정식) · 재장착 체인(rib→pin→peel) · 도마 조각 스테이징 → 접시 드래그 · 브랜딩(아이콘/타이틀 로고) · 도마 도움말 팝업 · **도감 정비(페이징·실사 카드)** · 메인메뉴 씬 전환 안전망 · **장비 시스템 개편(72차 — 착용품 인벤 그리드 이탈·인체 배치형 장비창·드래그 장착·캐스팅 착용 게이트)**.
- **다음 착수 (사용자 지정 순서 — 2026-08-05)**:
  **0.** (사용자) 회뜨기·플레이팅 + 72차 장비 개편 수동 검증
  **1.** **넙치류(광어·도다리) 다섯장뜨기 트리 분리**
  **2.** **두족류(오징어·문어·갑오징어) 전용 손질 트리 신규**
  **3.** **해루질 관련 작업** (손질 어종 확장 완료 직후 — 사용자 지정)
- ⚠ 방어류/돔류 손질은 마감(단계 스프라이트가 원래 방어 실사 기반 — 어종별로 다른 건 밑손질뿐이고 이미 분리됨). **퀘스트/스토리는 모든 컴포넌트 구현 후 도입.**
