...
- **전체 위치**: Phase 6(게임플레이 심화) — 맵 인프라·해루질·통발 인-맵 1차 완료 후
  **성장·생존 스펙 구현 중**(**P1~P7 완료** → 다음 = **스킬 트리 확장 차수**).
  이후 대과제: 통발 심화 → 불요리 → 농장.
- **로드맵(사용자 확정 2026-09-11)**: **스킬 트리 확장 (a)(c)(d)(e)** → P8 퀘스트 XP → P9 도움말
  → 통발 D2 → 불요리 → 농장(농사 스킬 배선·타입은 농장 단계 이연).
  맵 확정(육안)·OSM land 14개는 사용자 동반 병행 트랙.
- **직전 작업**: 129차 — **성장·생존 P7**: core `CraftingDatabase`(**도면 14종** — hand 8/workbench 6 ·
...
- **🚧 재개 지점**: ① **스킬 트리 확장 차수**(사용자 결정 2026-09-11 — (a) 생활 스킬 최대치 증가 ·
  (c) 저확률 행동력 미소비 · (d) 스킬 해제 조건 티어화 · (e) 시너지 히든 스킬. **Σ=200pt·티어당 ≤6노드
  제약 재조정 필요**) ② **맵 확정 — 사용자 육안**(109~115차 신설분: 래스터 도심 `'r'`·교차부 마킹·
...
- **완료**: 원물 자유 손질(돔류+방어류) · 넙치류 다섯장뜨기 · 회썰기 2뷰 + 사시미 플레이팅 ·
  **성장·생존 P1~P7**(레벨 200·스킬 83노드·생존 지표·상태이상·기절/사망·날씨 캐스팅·**제작·구급품·보건소**) ·
