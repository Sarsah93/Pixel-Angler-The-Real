...
- **직전 작업**: 97차 — **두족류 부산물 개편 + 회뜨기 3모드 + 한치 개방 + 문어 8스테이지**:
  껍질·아가미 미지급 · 뼈 명명 · **순수 필렛 최종 지급 제거**(몸통살 = 원물가 ×0.75 승계) ·
  회뜨기 = 몸통살 **22점**(가로 1+세로 10)·날개살 **4점**(좌우 2장)·다리부 → **촉완 ×2 + 다리부**(회 아님) ·
  한치 = 무늬오징어 트리 공유 · 문어 8스테이지(**굵은소금 1개 소모** — 시드+마트) + OCTO 3뷰 파라메트릭.
  상세는 `docs/wiki/03-WORKLOG/2026-08-13-097-ceph-byproducts-slicing-octopus.md`.
- **🚧 재개 지점**: **두족류 수동 검증** — ① 무늬오징어 잔여 F9(내장 2/2·뜯기 ②·날개 껍질째 2경로)
  ② 97차 신설분 체감(회뜨기 3모드·문어 파라메트릭 뷰·OCTO 가이드 좌표 — **전부 근사, F9 실측 대상**).
- **완료**: 원물 자유 손질(돔류+방어류) · 넙치류 다섯장뜨기 · 광어 포 뜨기 3단계+칼 팔로우(84~86차) ·
  ...
  **한치·문어 개방 + 두족류 회뜨기(97차)**.
- **다음 착수**:
  **0.** 두족류 수동 검증 (무늬오징어 잔여 F9 + 97차 신설분 체감·OCTO 좌표 실측) ← **현재 지점**
...
- **사용자 대기 에셋**: ① 오징어 **껍질 완전 분리본·아가미 붙은 안쪽면·3분할 결과**(합성/근사 교체용)
  ② 광어 **등쪽 단면(사진 2번) 투명본** ③ 갑오징어 속껍질 ④ **문어 실사 사진**(현재 파라메트릭) ·
  한치 전용 실사(현재 무늬오징어 공유).
