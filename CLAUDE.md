...
- **현재 진행(사용자 수동 검증)**: 회뜨기 **가이드 좌표 dev F9 실측 튜닝** + **2면 스프라이트 육안 확인**. (코드는 대기)
  - 55차부터 **밑손질(개복 전 측면 뷰)은 온마리 원형 틀 고정 + 부위 영역 삭제** — 작업 순서가 바뀌어도 생선 크기·위치가 불변이라 좌표가 안정적. dev 좌측 하단 **[섹션 건너뛰기]/[개별 작업(확장)]** 으로 임의 단계 점프 가능.
- **다음 착수(코드, 순서)**: ~~① UI 전수조사 고순위 4건~~ ✅ 완료(54차) → **② 필렛 재장착 배선 + 껍질 있는 필렛 스프라이트** ③ 회 썰기(12단계) 미니게임 + 측면 뷰 에셋.
