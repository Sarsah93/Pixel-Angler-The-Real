...
- **전체 위치**: Phase 6(게임플레이 심화) — 맵 인프라·해루질·통발 인-맵 1차 완료 후
  **성장·생존 스펙 구현 중**(P1~P6 완료 → **다음 = P7 제작**).
  이후 대과제: 통발 심화 → 불요리 → 농장.
- **로드맵(사용자 확정 2026-09-10)**: **성장·생존 스펙 P7~P9 선행** → 통발 D2 → 불요리 → 농장
  (농사 스킬 배선·타입은 농장 단계 이연). 맵 확정(육안)·OSM land 14개는 사용자 동반 병행 트랙.
- **직전 작업**: 127차 — **성장·생존 P5·P6**: core `CastWeather.ts`(**풍속 3m/s 초과부터** 맞바람 비거리
...
- **🚧 재개 지점**: ① **맵 확정 — 사용자 육안**(109~115차 신설분: 래스터 도심 `'r'`·교차부 마킹·
  **차량 밀도**·**방파제 테트라포드+등대**·**조도 갯바위 주간 톤**) ② **OSM land 14개 확장**
  ...
  subjectRect 변동) ④ ~~해루질·통발~~ → **121차 인-맵 1차 완료** — 잔여는 야간 실검증·F8 조율·실사 스프라이트·wade/dive 접근(백로그 A1-F).
