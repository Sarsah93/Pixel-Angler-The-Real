...
const BG_TOL = {
  ...
  halibut_gut_lift: 6,
  halibut_lift_b: 6,
  // 문어(octo_*)는 100차부터 전부 투명 픽셀 에셋 — 알파 경로 자동이라 임계 항목 불필요
  // (구 싱크대 실사 시절의 BG_TOL 4 + KEEP_POLY 전담 분리는 함께 폐기됨).
};
 ...
 */
const CEPH_ROOT_DIR = path.resolve(SRC_DIR, 'reference', 'cephalopod');
// 100차: 사용자 정리(2026-08-14 커밋 5efdd95)로 무늬오징어 레퍼런스가 전용 하위 폴더로 이동됨.
//  구 경로(cephalopod 루트)를 그대로 보면 전 키가 "입력 없음"으로 건너뛰어져 재생성 시 소실된다.
const CEPH_SRC_DIR = path.resolve(CEPH_ROOT_DIR, '무늬오징어 레퍼런스');
const CEPH_SRC = {
...
// `reference/cephalopod/octopus/` — 구 싱크대 공정 실사(JPG, 098차 소비)는 레퍼런스로만 보존.
// PNG 4종(삶은 문어·머리·다리·숙회)은 투명 배경이라 `gen_octo_assets.cjs`(직접 로드 아이콘)가 소비한다.
const OCTO_SRC_DIR = path.resolve(CEPH_ROOT_DIR, 'octopus');
/**
 * ── 문어 픽셀 투명 에셋 (100차 — 사용자 제작 2026-08-14, 커밋 5efdd95) ──────────────
 * `octopus/손질 가이드에 적용할 픽셀 투명 에셋 이미지/0~9.png`. 전부 RGBA 투명(알파 경로 자동)
 * 픽셀아트 완성본이라 **KEEP_POLY·BG_TOL이 불필요** — 098차 싱크대 실사(JPG) 추출본을 전량 대체한다.
 * 구 JPG 원본(2~9문어*.jpg)은 레퍼런스로 보존만 하고 더 이상 굽지 않는다.
 *
 * 번호 = 공정 순서(구 사진 넘버링 계승):
 *   0 = 손질 전 활어 통마리(구 octo_live 투명본 대체 — 세로 구도)
 *   1 = 외번 시작(주머니 들림 — 구 "뒤집기1" 미채택 자리가 픽셀본으로 복원)
 *   5→6 = 부리 보임 → 부리 돌출(뽑기 완료 프레임 — 구 "구멍" 사진 대신 뽑히는 그림)
 *   7 = 뽑힌 부리 단독 → 스테이지가 아니라 부산물 아이콘(gen_octo_assets.cjs가 소비)
 */
const OCTO_PIXEL_DIR = path.resolve(OCTO_SRC_DIR, '손질 가이드에 적용할 픽셀 투명 에셋 이미지');
const OCTO_SRC = {
  '0.png': 'octo_live',     // 손질 전 활어 통마리 — 도마 시작 뷰 + 외번 시작 프레임
  '1.png': 'octo_invert1',  // 외번 시작 — 외투막 주머니가 들리기 시작 (신규 드래그 중간 프레임)
  '2.png': 'octo_invert2',  // 외번 진행 — 속면(흰 주머니)이 나옴
  '3.png': 'octo_invert3',  // 외번 완료 — 흰 속면 노출
  '4.png': 'octo_viscera',  // 내장 떼어내기 — 덩어리를 당겨 분리 (먹물주머니 포함)
  '5.png': 'octo_beak1',    // 입면 — 악판(검은 부리) 보임
  '6.png': 'octo_beak3',    // 악판 돌출 — 뽑기 완료 프레임 (7.png = 뽑힌 부리는 아이콘 전용)
  '8.png': 'octo_salt',     // 굵은소금 뿌린 상태
  '9.png': 'octo_scrub',    // 거품 문지르기 (구 손·거품 실사 품질 하위 해소)
};

/**
 * 문어 완성본 (098-b — 사용자 직접 제작 투명 PNG, `trimmings/octopus/`).
 *  - octo_clean = 손질 최종 완료(내장·소금 치대기·세척 끝) — 픽셀 세트(0~9)에 완료본이 없어 유지.
 */
const OCTO_TRIM_DIR = path.resolve(__dirname, '..', 'food assets', 'trimmings', 'octopus');
const OCTO_TRIM_SRC = {
  // octo_live는 100차부터 픽셀 에셋 0.png(OCTO_SRC)이 정본 — trimmings 투명본 항목 제거.
 ...
 */
const KEEP_POLY = {
  // (비어 있음 — 구 문어 싱크대 실사용 폴리곤은 100차 투명 픽셀 에셋 전환으로 전량 폐기.
  //  향후 손·배경이 프레임을 채우는 공정 실사를 다시 받으면 그리드 오버레이 실측으로 재등록:
  //  키별 폴리곤(정규화 0~1, 복수 가능) 바깥을 전부 제거 — ERASE_POLY와 별개, 회전/미러보다 먼저.)
};
...
for (const [fname, key] of Object.entries(OCTO_SRC)) {
  const abs = path.join(OCTO_PIXEL_DIR, fname);
