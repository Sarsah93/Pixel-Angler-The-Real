...
| **어종 픽셀아트 6종 교체 + 참복어 신규** — 병어·덕대·참복어·농어·대구·대문어 | ✅ | **176** — 신규 4장은 실알파 보유(매트 제거 불요) · **참복어는 부재였다**(DB에만 있고 텍스처 0) · 대문어만 흰 매트(참문어 정상). `FISH_TEXTURE` 49 → **50** · 그림 **231**장 |
| **표시명·에셋 정정 3건** — 문치가자미 표준명 · 대구 흰 배경 · 갈치 픽셀아트 | ✅ | **175** — 갈치는 파일명 대소문자 불일치(`Cutlassfish.png`)로 교체가 빗나가 있었다 |
...
10. **텍스처 키와 파일명이 다를 수 있다**(175차) — `fish_hairtail`이 읽던 파일은
    **`Cutlassfish.png`(대문자 C)** 였다. 어종 id나 소문자 통칭으로 파일을 쓰면
    **게임이 한 번도 읽지 않는 새 파일이 생기고 구 이미지가 그대로 남는다**(교체한 줄 알았는데 안 바뀜).
    에셋 교체 전에 **`BootScene`의 해당 `load.image` 줄로 실제 파일명을 확인**한다.
    gh-pages는 대소문자를 구분하므로 `public/fish/`는 **소문자로 통일**한다.
