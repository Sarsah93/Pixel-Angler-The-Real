...
| **두족류 렌더러** | ✅ | 89차 — `ui/CephalopodFish.ts`가 **실사 도트 우선 · 파라메트릭 폴백**. `SQUID_STAGE_SPRITE` 14스테이지 매핑(사진 11장 공유 배정) |
...
**④ 두족류 가이드 좌표 F9 실측** — 87차 신설 9경로(펼치기·내장·펜·껍질 들춤·아가미·날개 2선·3분할 2컷)가
  근사값이다. 시메·절개·박리·부리는 스펙 §4.6 값.
  ⚠ **89차에 좌표 기준이 도마 rect → 피사체 rect로 바뀌었다** — 기존 근사값은 전부 재측정 대상이며,
  정본은 이제 SVG가 아니라 **구운 실사 스프라이트**(`squid_*`)다.
