...
> 최종 갱신 2026-08-12 (91차) · 관련 차수 14·20·41·43·45·47~63·65·67·70·74·75·78·80·82~91
...
| **무늬오징어 트리 14 → 12 → 14스테이지** | ✅ | 87차 신설 → 90차 제거 2(펼치기·결과 확인) → **91차 추가 2** — 껍질 뜯기 ②(아래로 — 90차 실측 스윕 배선) · 날개 뜯기 ①(껍질째). 아가미·닦기는 날개 뒤로 이동(완료본 실사 정합) |
| **섹션 매핑**(1:1 작업, `anyOrder:false`) | ✅ | 87차 — `SQUID_SECTIONS` 6섹션 · 91차 14작업(skin 4 · trim = finskin→fin→gill) |
| **"뜯기 전" 상태 합성 3종 + 연출 에셋 3종** | ✅ | **91차** — 사진(뽑기1/2·날개뜯기1) + 추론 합성(`deriveSkinOver` — skin_on·skin_down·finskin_on). 내장 뽑기 = 연출 2프레임(`cephAnimOverride`) |
| **peel 판정 일반화** | ✅ | **91차** — 잡기 = `tapPoint` 반경 · 당김 = 스윕 방향 투영 (구: x>0.72 + 좌로 고정 — 대각 스윕이 가이드대로 그어도 실패했다) |
...
| **두족류 렌더러** | ✅ | 89차 — `ui/CephalopodFish.ts`가 **실사 도트 우선 · 파라메트릭 폴백**. `SQUID_STAGE_SPRITE` 14스테이지 매핑(91차 — 사진 14장 + 합성 3장) |
...
**🚧 ⓪ 무늬오징어 수동 실플레이 검증 — 진행 중**
  (90차 1~3단계 실측 → **91차 4~5단계 재구성 완료**: 껍질 2회 뜯기 — 온전 상태는 영상 참고 **추론 합성**,
  마무리 스윕 배선 · 내장 뽑기 연출 2프레임 · 날개 2단 분리. 잔여 = 신설 좌표 F9 실측
  (`CEPH_FINSKIN_PATHS`·flip_skin 들추기·아가미·3분할·부리) + **합성 3종 품질 사용자 확인**.
  껍질 분리 완료본·아가미 붙은 안쪽면 실사는 계속 대기 — 오면 키 교체만)
