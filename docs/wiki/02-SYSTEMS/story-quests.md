...
- 퀘 `rewards.items`는 140차부터 실지급 — 단 **가방 2~4단계 아이템 정의(`inv_bag_*`)가 카탈로그에 없으면 경고 후 미지급**. 아이템 정의 + 픽셀 아이콘 필요.
- **선택지 손글 확장**(현재 13편 · 나머지 표준 4종) · `choice.*` 플래그를 읽는 후속 퀘 조건(현재 N18-1 1곳) · 일지 '사람들'에 우호도 눈금.
