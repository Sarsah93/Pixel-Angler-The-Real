...
| **고정 = 실동작**(받을 수 있는 할 일도 고정 · 「지금 할 일」 갱신 · 점멸 화살표) + 일지 의뢰인 초상(무발주 = 주인공) · 진행률 바 열 폭 정합 | ✅ | **177** — 구 추적 체크박스 중복 삭제 · `setTracked` 가드 `active` → `active`+`available` |
| **컷씬 대사 타이핑 + 클릭 스킵**(18ms/자 · 클릭 = 즉시 완성 → 다음 스텝 · 단락 끝 `▼`) | ✅ | **177** — 167차 이후 컷씬에만 없던 것 복원 · 차단막 `setScrollFactor(0)` |
...
### 22. 고정(추적) 체크는 두 층을 다 봐야 한다 (177차)

「고정 체크가 눌리는데 아무 일도 안 난다」는 신고는 **판정 층과 입력 층 어느 쪽이든** 원인이 될 수 있다.

- **판정**: `StoryStore.setTracked`는 `quests[id]`(수락 뒤에만 생기는 레코드)를 봤다.
  목록에 함께 뜨는 **받을 수 있는** 행은 통과하지 못하고 조용히 반환됐다.
- **입력**: 행 전체 히트를 체크칸 히트보다 **뒤에** 추가하면 행 히트가 위로 올라와 클릭을 삼킨다.
  Phaser는 같은 컨테이너에서 **나중에 추가된 것이 위**다 — 체크칸 히트는 항상 마지막에 붙인다.
- 새 체크칸을 넣을 때 **구 체크칸을 지웠는지** 확인한다. 165차는 남겨 둬서 같은 자리에 둘이 겹쳤고,
  실제로 동작하던 것은 의미가 다른 구 블록이었다.

### 23. 화면 고정 오버레이의 히트는 자식에도 scrollFactor 0 이 필요하다 (177차)

컨테이너에 `setScrollFactor(0)`을 걸어도 **렌더만** 고정된다. 히트 판정은 **자식 자신의 값**을 쓴다.
컷씬 차단막(`veil`)이 이것 때문에 스크롤된 속초 필드에서 카메라 이동량만큼 어긋나 있었고,
클릭 흡수도 클릭 진행도 한 번도 동작하지 않았다.
`DraggablePanel` 계열은 `applyScreenFixed`가 재귀로 처리하지만, **직접 만든 Container는 스스로 걸어야 한다.**
