...
- ~~통발 위판 UI(M2-01)~~ ✅147 · ~~1-11 총회 연출~~ ✅147.
  남은 것은 **1-4 구멍치기**뿐이다(149차) — 조법 자체가 없고, M1-04의 "방파제에서"는
  라벨일 뿐 강제되지 않는다(어디서 아무거나 한 마리 낚으면 통과한다 — 147차 실측).
- ~~가방 아이템 정의~~ ✅141(`QuestRewardItems.ts`). 귀속 가방 `bagSlots` 8~12는 **인벤 상한 30에 `min`** — 7행 이상은 패널 재설계가 먼저.
  ⚠ 즉 **지금 마운틴 백팩(12)과 태클 베스트(8)가 같은 효과**다 — 미구현이 아니라 *조용히 죽은 보상*이라 148차 우선순위가 높다.
