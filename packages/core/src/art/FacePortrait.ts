 ...
 *
 * 157차 — **필드 캐릭터와 같은 인상**을 유지하면서 대화창에서는 더 친근하게 읽히도록 재조정.
...
/**
 * 눈 — 157차: 필드 스프라이트의 검은 동공 바를 유지하되, 밝은 흰자·홍채·하이라이트를 4행으로 분리한다.
 * 152차의 흰자 위주 작은 눈은 화소는 많았지만 필드의 큰 검은 눈과 다른 사람으로 읽혔다.
 *  행 0 = 동공/속눈썹 바(검정) · 행 1 = 따뜻한 흰자 · 행 2 = 홍채·동공·하이라이트 · 행 3 = 아랫꺼풀 그늘.
 */
function drawEyes(g: PGrid, look: CharAppearance): void {
  ...
  for (const s of [-1, 1] as const) {
    // 눈 한 짝 = 가로 6 x 세로 4 — 작은 얼굴에서도 표정이 먼저 읽히는 둥근 인상
    const x0 = s < 0 ? CX - 7 : CX + 3;
    const x1 = x0 + 5;
    g.row(EYE_Y, x0, x1, pupil);                                  // 검은 바 — 필드의 인상은 여기서 온다
    g.row(EYE_Y + 1, x0, x1, 0xf7ead9);                          // 흰자를 완전히 흰색으로 만들지 않아 팔레트가 부드럽다
    g.row(EYE_Y + 2, x0 + 1, x1 - 1, iris[0]);
    g.put(s < 0 ? x0 + 2 : x1 - 2, EYE_Y + 2, pupil);             // 가운데 동공
    g.put(s < 0 ? x0 + 1 : x1 - 1, EYE_Y + 2, 0xffffff);          // 한 칸 반짝임
    g.row(EYE_Y + 3, x0 + 1, x1 - 1, under);                      // 아랫꺼풀 그늘
    if (look.sex === 'f') {
