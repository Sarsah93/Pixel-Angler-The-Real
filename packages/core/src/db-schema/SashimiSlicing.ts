...
export const SASHIMI_CUT_OVERRIDES: Record<string, CutPoint[][]> = {
  // dev F9 [복사] 스니펫을 여기에 붙여넣는다.
  /**
   * 엔가와 2컷 — 사용자 F9 실측 2026-08-07. 자동 배치의 **수직에 가까운 컷을 대각으로 교체**
   * (윗점이 아랫점보다 오른쪽 = 머리 쪽으로 기운 소기즈쿠리식).
   * ⚠ 절단면 렌더도 이 기울기를 따라간다 — `SashimiPanel`의 조각 분리는 사각 crop이 아니라
   *   컷 경로 기울기를 반영한 **대각 폴리곤 마스크**다(87차). 좌표를 바꾸면 절단면도 자동 추종.
   */
  'engawa': [
    [{ x: 0.419, y: 0.105 }, { x: 0.324, y: 0.924 }],
    [{ x: 0.664, y: 0.068 }, { x: 0.582, y: 0.823 }],
  ],
};
