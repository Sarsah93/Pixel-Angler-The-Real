...
## 0. 확정 시안 (141차 · 2026-09-16 사용자 결정)
...
- 지시: "이 디자인/스타일에서 조금씩 다르게 변형하는 게 좋을 것 같다." → 이후 변형(피부·머리·눈·얼굴형)은
  **이 시안을 기준선**으로 팔레트 스왑·부분 교체로 만든다. 절차 생성 룩으로 되돌리지 않는다.
- 구현 트랙(별도 차수): 시트 임포트(투명 PNG · 5열×4행 · 32×32) → `CharacterArt` 앵커 재측정 → 페이퍼돌 레이어 정합 →
  캐릭터 생성 씬 미리보기 교체. 착수 조건 = 사용자 최종 시트 PNG(남/여 2장 + 얼굴 변형 스트립).
