...
```
npx pnpm run build → ✅ 4/4 패키지 성공 (2026-08-04)
npx pnpm --filter @tra/client-pc run typecheck → ✅ 0 오류 (2026-08-04)
```

**최근 주요 변경 (2026-08-04 69차) — 사용자 피드백 8건 (뷰/좌표/방향/스테이징/아이콘/에셋/로고)** (실렌더·실 스토어 검증 PASS, 빌드 4/4·typecheck 0):
...
- **★ 작업 이어받기 (2026-08-04 기준) — 회뜨기(손질) 시스템 진행 상황 & 다음 착수 지점**:
  - **[완료 상태]** 원물 자유 손질(52~63차) · 회썰기 미니게임 2뷰(65·69차) · **사시미 접시 플레이팅 + 가격 체계**(66·68차 — 필렛 = 원물가 분할, 모듬 고정표/단품 = 괴리율 보정식(kg시세×실수율×인분마진)) · 재장착 체인(rib→pin→peel, 67차 필렛 소실 수정) · 도마 조각 스테이징 → 접시 드래그(69차).
  - **[다음 착수 — 사용자 지정 순서, 2026-08-05 오후 재개]**
    **0. 최근 변경(66~69차) 세부 추가 검토** — 실플레이 전수 확인: 도마 조각 → 접시 실마우스 드래그 감각 · 모듬/단품 가격 체감 · 방어류 탑뷰 꼬리 방향(도미만 확인됨 — 생성기 `flipTop`) · 로고 배킹 강도.
    **1. 방어류 원물 손질 전체 검토·조정** — 방어/부시리/잿방어 전 구간: 가이드 좌표 F9 재실측(돔류 기준값이 방추형과 맞는지), 실사 스프라이트 투입(`food assets/butchery/amberjack_*.png` → `pixelize_butchery.cjs`), 복면/체강/장뜨기/spine·belly 뷰 정합.
    **2. 넙치류·두족류 손질 구현** — 넙치류 = 다섯장뜨기 섹션 트리 분리(flat 분기 + 전용 뷰/좌표) / 두족류 = 전용 트리 신규(눈 위 신경·먹물·다리 — 현재 스텁). `getButcheryFamily` unsupported 해제.
  - **[전체 로드맵 위치]** 회뜨기 = **Phase 6(게임플레이 심화 — 낚시·손질) 내부 서브시스템** 하나. 완결 후 차기 대과제: **퀘스트/스토리 라인**·스시(고급 조각 '요리하기' 스텁)·불요리(화구·용기)·CookScene 실조리·경영(식당 납품/판매)·CraftScene(U)·멀티(Phase 8)·Tauri 패키징(Phase 9).
