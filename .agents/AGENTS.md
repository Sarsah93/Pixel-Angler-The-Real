...
## 9. 현재 빌드 상태 (2026-07-31 기준)

```
npx pnpm run build → ✅ 4/4 패키지 성공 (2026-07-31)
npx pnpm --filter @tra/client-pc run typecheck → ✅ 0 오류 (2026-07-31)
```

**최근 주요 변경 (2026-07-31 57차) — 박피 2단계 재설계 + 껍질 붙은 필렛 4장 부산물 + 필렛 재장착(박피부터 재개)** (사용자 캡처 2장 + 지시 — core 실판정 + 실렌더 검증, 빌드 4/4·typecheck 0):
...
- ⚠ **잔여**: 박피 3단계 유도 좌표(꼬리 칼집·단면 경계·지그재그)는 근사 기본값 — F9 실측 예정. 옆면 뷰(y plane)는 연출 이해용이라 미구현(사용자 명시). 칼 연출(살코기 위 칼 레이어)은 기존 액션 애니 재사용.
...
- **★ 작업 이어받기 (2026-07-31 기준) — 회뜨기(손질) 시스템 진행 상황 & 다음 착수 지점**:
  - **[현재 진행 중 — 사용자 수동 검증]** ① **가이드 좌표 실측 튜닝**: 신규/변경 스테이지(`fillet_1_score`·`fillet_1_ribcut`·`fillet_1_sever` 및 vessel/tail_b/rib/pin)의 유도선 좌표를 **dev F9 곡선 편집기로 실측**(현재 근사 기본값 — 코드 반영엔 core 리빌드 필요, F9 [복사] 스니펫은 opts 누락되니 항상 보존). ② **2면 스프라이트 육안 확인**: `fillet_1` 등쪽/갈비뼈끊기 = 측면 `{fam}_spine` / 배쪽 sever = 정면 `{fam}_belly` 뷰가 실플레이에서 맞는지 확인 → 어긋나면 dedicated 2면-덩어리 배쪽 뷰 분리 여부 결정.
  - **[다음 착수 — 코드, 우선순위 순]** ① **UI 전수조사 고순위 4건**(ShopPanel 그리드 스크롤 부재·EquipmentPanel/LicensePanel 이름 오버플로·ButcheryPanel 결과창 desc↔버튼 겹침 — §4 UI 레이아웃 정책 기준) ② **필렛 재장착 배선**(인벤 '껍질·갈빗대 붙은 필렛'(`inv_filletribs`/`trim_fillet_ribs`) 도마 재장착 → `ButcheryProcess.jumpTo`로 갈빗대 섹션 재개. 스키마 `ButcheryProgress`·`jumpTo`·아이템 발급 완비, **도마 수용(draggableFish/dropFishOnBoard)·progress 임베드·resume 배선 남음**) + **갈빗대 제거 단계 스프라이트 = 껍질 있는 필렛**(현재 FLESH_UP은 pickStageSprite null→순수 필렛 FISH_FILLET 폴백이라 "껍질 없는 살"로 보임 — 사용자 리포트) ③ **회 썰기(12단계) 미니게임 + 측면 뷰 에셋**(순수 필렛 재장착 → 세로 9등분 사시미/야나기바 대각 11등분 고급사시미, 소/중/대/특대 크기 연동).
  - **[전체 로드맵 위치]** 회뜨기 = **Phase 6(게임플레이 심화 — 낚시·손질) 내부 서브시스템** 하나. 회뜨기 완결 후 차기 대과제: **퀘스트/스토리 라인**·CraftScene(U)·요리(CookScene) 실조리·경영(식당 납품/판매)·해루질/통발 심화·멀티(Phase 8)·Tauri 패키징(Phase 9). 상세 로드맵은 이 문서 §"회뜨기 로드맵" 참고.
