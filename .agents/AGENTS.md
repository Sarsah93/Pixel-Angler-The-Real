...
## 9. 현재 빌드 상태 (2026-08-05 기준)

```
npx pnpm run build → ✅ 4/4 패키지 성공 (2026-08-05)
...
- **★ 작업 이어받기 (2026-08-05 야간 기준) — 진행 상황 & 다음 착수 지점**:
  - **[완료 상태]** 원물 자유 손질(52~63차 — 돔류·방어류) · **넙치류 다섯장뜨기(74차 — S자 머리·4면 포뜨기·엔가와 4장·재장착 3체인·엔가와 회썰기 2컷)** · 회썰기 미니게임 2뷰(65·69차) · 사시미 접시 플레이팅 + 가격 체계(66·68차) · 재장착 체인(rib→pin→peel, 67차) · 도마 조각 스테이징 → 접시 드래그(69차) · 도감/도움말/메인메뉴 안전망(71차) · 장비 시스템 개편(72차) · **외부 QA 수정(73차 — 접시 회전 렌더 실버그·씬 전환 검은 화면 전수조사·SceneFade 공용 안전망)**.
  - **[다음 착수 — 사용자 지정 순서, 2026-08-05 야간 갱신]** (상세 체크리스트는 IMPLEMENTATION_PLAN "🚧 다음 착수")
    **1. 광어류 가이드 검증 및 조정** — 실플레이 피드백 + **F9 실측 좌표 반영**(`FLAT_GUIDE` 전 구간이 근사 기본값: S커브·중앙선·상/하 경계 칼길·포 뜨기·내장 스윕·엔가와 경계) + 뷰/연출 수정(포뜨기 벌어짐 경계 곡선·배쪽 고정 순서 유지 여부) + 회썰기용 광어 필렛 스프라이트(현 bream 폴백).
    **2. 두족류 손질 추가 — 문어·무늬오징어·한치·갑오징어** 전용 트리 신규(눈 위 신경·먹물·다리·껍질막·연골/갑). ⚠ **한치(창꼴뚜기)는 어종 DB 미등록** — `swordtip_squid.png` 에셋만 선로드(20차) → 오라클+FISH_DATABASE+경락 매핑 4계층 등록 선행. cephalopod 전용 뷰 렌더는 PixelButcherFish flat 분기와 같은 패턴으로. trimmings 에셋은 구현 시 필요 목록 정리해 사용자에게 요청.
    **3. 해루질 관련 작업** (사용자 지정 2026-08-05 — 손질 어종 확장 완료 후 바로. NightHuntingScene/NightHuntingEngine 기반 확장).
  ...
  - ⚠ **미커밋 상태 주의**: 73차(QA 수정)·74차(다섯장뜨기+엔가와)가 main에 미커밋 — 실플레이 확인 후 커밋 필요. gh-pages 배포본은 73차 dist 포함(08-04 21:54), 74차는 미배포.
