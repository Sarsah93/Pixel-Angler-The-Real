...
**최근 변경 (2026-09-23 177차) — 홈타운 랜덤 NPC 제거 + 일지 고정 실효화 + 컷씬 대사 타이핑 복원**

- 사용자 캡처 2장·지시 4건 + 176차 확인 항목 답(`대구` → **태평양 대구**).
- **홈타운은 시골 외곽** — `AmbientNpcSystem`을 아예 만들지 않는다(속초 120명은 그대로).
- ⚠ **고정 체크가 죽어 있던 기전은 두 겹이었다** — ① `StoryStore.setTracked`가
  `quests[id]`(수락 뒤에만 생기는 레코드)를 봐서 **「받을 수 있는」 행은 조용히 반환** ②
  행 전체 히트를 체크칸 히트 **뒤에** 추가해 클릭을 삼켰다. 실제로 동작하던 것은 165차에 지우지 않은
  **구 추적 체크박스**였고, 그쪽은 메인·서브 두 칸을 한꺼번에 비우는 **다른 의미의 토글**이었다.
  → 구 블록 삭제 · 히트 순서 교정 · `available` 허용 · `pickGuideQuest`가 미수락 고정을 발주자 안내로 낸다.
- **일지 의뢰인 칸** = 30x30 슬롯 프레임 + 초상, **발주자가 없으면 주인공 얼굴**(구: 회색 점).
  **진행률 바 오버플로** = 열 합 **500 vs 행 배경 478** — `C_NAME` 상수 하나가 원인(172 → 156).
- ⚠ **컷씬에만 타이핑·클릭 스킵이 없었다**(대화창·혼잣말은 멀쩡) → 18ms/자 타이핑 +
  클릭 = 즉시 완성 / 다음 스텝 + 단락 끝 `▼`. 곁들여 차단막 `veil`에 **`setScrollFactor(0)`이 없어**
  스크롤된 필드에서 클릭 판정이 통째로 어긋나 있던 것도 해소(조작 봉쇄도 같이 복구).
- 검증: 홈타운 랜덤 NPC 0 · 속초 120 · 초상 8/8 · 고정(진행 중 메인·받을 수 있는 서브) → 추적기 2행 ·
  컷씬 타이핑/클릭/다음 스텝 전 항목 PASS · pageerror 0 · 빌드 3/3 · typecheck 0 ·
  아티팩트 Version 17 · gh-pages 17차 배포.
  상세: [177차 워크로그](../docs/wiki/03-WORKLOG/2026-09-23-177-hometown-npc-journal-pin-cutscene-typing.md).

**이전 변경 (2026-09-23 176차) — 어종 픽셀아트 6종 교체 + 참복어 신규 + 대문어 흰 매트 제거**

- 사용자 지시 2건(고해상 픽셀아트 4장 + 작업 중 대구 1장 추가). 신규 4장은 **전부 실알파 보유**라
  매트 제거 없이 타이트 크롭 + 2px 패딩만 했다 — 병어 **1523×999** · 덕대 **1513×1000** ·
  참복어 **1989×673** · 농어 **1762×878** · 대구 **1951×797**.
- **병어·덕대는 174차 저해상(473×318 / 484×288)을 대체**한다 — 백로그 Z ① "해상도 이월" 해소.
- ⚠ **참복어(자주복)는 결함이 아니라 부재였다** — 어종 DB에 `tiger_puffer`가 있는데
  `FISH_TEXTURE`·`BootScene` 어디에도 없어 `speciesId` 폴백조차 못 탔다. **신규 등록**
  (`FISH_TEXTURE` 49 → **50** · 위키 그림 230 → **231**장).
- ⚠ **문어 2장 중 대문어만 흰 매트**(RGBA인데 투명 0.0% · 참문어는 66.4%로 정상) —
  175차 대구와 **같은 결함이라 같은 도구**(테두리 BFS + 프린지 페더링, 임계 236)를 그대로 썼다
  (투명 0.0% → **44.9%** · 1729×608 → 1259×538).
- 검증: 인게임 텍스처 **7/7** 로드 · `public/fish/` 53장 **불투명 배경 잔존 0** ·
  미참조 에셋 **0** · 위키 카드 **6/6** 정확 · 이미지 깨짐 0 · pageerror 0 · 4xx 0 ·
  빌드 3/3 · typecheck 0 · 아티팩트 **Version 16**.
  상세: [176차 워크로그](../docs/wiki/03-WORKLOG/2026-09-23-176-fish-pixelart-6.md).

**이전 변경 (2026-09-23 175차) — 표준명 문치가자미 + 대구 흰 배경 제거 + 갈치 픽셀아트 교체**

- 사용자 지시 3건. ① `flounder` 표시명을 「참도다리(문치가자미)」 → **문치가자미**(174차 확인 대기 항목 확정) —
  도감·오라클 2곳. 세이브는 `speciesId`를 저장하므로 구세이브도 재로드 시 새 이름으로 표기된다.
- ② **대구는 RGBA인데 투명 0.0%** 였다 — 알파 채널만 있고 배경이 **불투명 흰색으로 구워져** 있었다.
  174차 체커보드와 같은 계열이지만 **패턴이 아니라 단색 매트**라 격자 모델이 아니라
  **테두리 BFS + 프린지 페더링**으로 충분하다(투명 0.0% → **45.5%** · 1592×656 → 1545×546 타이트 크롭).
- ③ ⚠ **갈치 교체가 빗나간 실제 기전** — `public/fish/`에 `hairtail.png`는 **존재하지 않았고**,
  `fish_hairtail`이 읽던 파일은 **`Cutlassfish.png`(대문자 C)** 였다. 폴더의 나머지 50개가 전부 소문자라
  어종 id나 소문자 통칭으로 파일을 쓰면 **게임이 한 번도 읽지 않는 새 파일이 생기고 구 이미지가 그대로 남는다**
  (빌드도 통과하고 에러도 없다). 소문자로 통일 + 신규 픽셀아트(1024×203) 교체.
  절차는 스킬 **`asset-pipeline` ①-e**에 반영.
- 검증: 인게임 텍스처 3/3 로드(갈치 1024×203 · 대구 1545×546) · 위키 카드 **8/8**(갈치·대구·문치가자미·
  도다리·강도다리·광어·병어·덕대) · 이미지 깨짐 0 · pageerror 0 · 빌드 3/3 · typecheck 0 · 아티팩트 Version 15.
  상세: [175차 워크로그](../docs/wiki/03-WORKLOG/2026-09-23-175-fish-name-cod-matte-hairtail.md).

**이전 변경 (2026-09-23 174차) — 어종 실사 7종 매핑 + 체커보드 배경 제거**

- 사용자 사진 7장(눈볼대·문절망둑·문치가자미·강도다리·도다리·병어·덕대). 7종 모두 **어종 DB에는
  이미 있는데 그림만 없던** 종이라, 그림이 붙는 순간 어획 팝업·인벤 아이콘·상세보기·위키 카드가
  `speciesId` 폴백으로 **동시에** 해소된다(42차 전수 배선).
- ⚠ 뒤 2장(병어·덕대)은 **투명을 뜻하는 회색 체커보드가 실픽셀로 구워져** 있었다
  (칸 11.9px · 두 회색 L 34 / L 102 · 전부 무채색). 은백색 어종이라 **밝기만으로는 못 가른다** —
  밝기 밴드 BFS는 어두운 윤곽선을 다리 삼아 **몸통으로 샜고**, 순정 회색만 훑으면 JPEG가 뭉갠
  칸 경계(L 45~90)에 막혀 **칸 하나에 갇혔다**(1차 제거율 22.6%).
- 최종 3단 = **격자 모델 BFS**(칸 크기·위상·반전 자동 추정 — 같은 회색이라도 피사체는 패턴을
  안 따른다) → **최대 연결요소만 보존**(얼룩 1,163개 → 1개) → **실루엣 프린지 2차 제거**.
  절차는 스킬 `asset-pipeline` ①-d에 반영.
- `FISH_TEXTURE` 42 → **49** · 위키 그림 223 → **230**장 · 아티팩트 Version 14(같은 URL).
  ⚠ 3번은 사용자가 「문치가자미(참가자미)」·DB는 「참도다리(문치가자미)」 — 학명 일치라 매핑은
  정확하고 **표시명만 확인 대기**.
- 검증: 게임 텍스처 7/7 로드 · 위키 카드 7/7 · 이미지 깨짐 0 · pageerror 0 · 빌드 3/3 · typecheck 0.
  상세: [174차 워크로그](../docs/wiki/03-WORKLOG/2026-09-23-174-fish-photo-7-mapping.md).

**이전 변경 (2026-09-23 173차) — 사진 셀 전이 편입 + 가로 시설물 선형 정렬 + 홈타운·집 내부 현대화**

- 172차가 절차 타일끼리의 경계를 고쳤다면, 여기는 **실사 셀과 절차 타일이 만나는 선**이다 —
  물·갯바위·방파제 타일은 렌더 초반 `continue`로 전이를 통째로 비껴갔다. 안벽 모서리·사석 발치를
  **물 타일 쪽에서** `'b'` 인접으로 그려, 105차에 추출만 해두고 한 번도 안 쓴
  `rubble_toe_*`·`pier_edge_*`를 처음 소비한다.
- **가로 시설물 선형 정렬** — 가로등·가로수·벤치가 해시 산포라 "왜 거기 있는지"를 설명하지 못했다.
  도로 폴리라인의 **누적 호길이 등간격**으로 스냅한다(간선 10타일 / 이면 16타일 · 좌우 보도 교대 ·
  회전교차로·서비스 골목·교차로 차도 제외). ⚖ **전부 `free`(바디 없음)** — 1타일 보도에 바디를
  세우면 그 길이 통째로 막힌다(106차 테트라포드 전례).
- **홈타운·집 내부 현대화**(사용자: "너무 올드한 에셋인 것 같아. 집 내부 또한") — 평면 색면 +
  `주방`·`저장★ 침대` 같은 개발 라벨(§8-9 위반이자 "올드한" 인상의 실제 원인)을 폐기하고
  2px 도트로 재작성했다(널마루·징두리·창광·벽시계·책선반 + 가구 10종 · 기와/용마루/처마 그늘).
- ⚠ 이번에 밟은 함정: **키릴 `а`(U+0430)가 16진 리터럴에 섞여** `TS1005: ':' expected`만 뜬다.
- 검증: 실렌더(홈타운 20오브젝트 · 집 내부 · 속초 간선 3지점) pageerror 0 · 빌드 3/3 · typecheck 0.
  상세: [173차 워크로그](../docs/wiki/03-WORKLOG/2026-09-23-173-photo-seam-street-furniture-home.md).

**이전 변경 (2026-09-23 172차) — 지형 어휘 확장 + 칠하기 순서 + 쌍(pair) 전이 규칙**

- 사용자 지적 — "뜬금없이 벽돌지형에서 모래 지형으로 바뀐다 … **지형 경계간의 이음 부분?
  그런게 전혀 고려되어 있지 않은 것 같아.**" 참고로 든 '세피리아'는 **제작자가 손으로 구조화한**
  지형이고 우리는 OSM 실측이다 — 과제는 "손으로 그린 조화를 자동 생성에서 얻는 방법".
- 근본 3건: ① **어휘가 8자뿐이고 `'.'`가 만능 통**이라 도로·광장·주차장·흙길·갯벌·농지·숲이 전부
  뭉개졌다(어휘가 없으면 전이도 없다) → **`p d t c f` 5자 신설**(OSM 태그에서 자란다 — 지어내면
  다시 임의 배치가 된다) ② **전이가 '쌍'이 아니라 '내 지형 테두리'**였다(`edgeTex`가 자기 그룹 키
  하나 · 106차가 모래 `'s'`를 **일부러 제외**해 둔 바로 그 경계) → core
  **`TerrainTransition.seamBetween(self, other)`** 가 `blob/curb/spill/revet/foam`을 결정하고
  렌더는 그리기만 한다 ③ **칠하기 순서를 지형 정의(`paint`)가 소유**해 새 지형이 렌더 코드를 안 고친다.
- ⚠ **빌더의 타일 단위 완충층은 폐기** — 32px 흙 한 줄이 **의도적인 흙길**로 읽혀 더 나빠졌다
  (런타임 알갱이 띠가 대신한다). ⚠ 숲·농지에 잔디 블롭을 붙였더니 **갈색 테두리를 두른 초록 사각형**
  → 유기 `spill`만. ⚠ `build_seamless`가 terrain.png를 **최근접 색으로 되읽어** 팔레트에 없는
  신규 색은 JSON에서 조용히 사라진다(쌍거리² ≥ 1,668로 벌림).
- 검증: 속초 실측 `c` 10,804 · `p` 4,877 · `f` 2,051 · 방파제 2,583(패스 순서 교정 후 복구) ·
  지터 고립 타일 1,077+230 디노이즈 · 전이 8종 실렌더 · 빌드 3/3.
  상세: [172차 워크로그](../docs/wiki/03-WORKLOG/2026-09-23-172-terrain-vocabulary-seams.md).

**이전 변경 (2026-09-23 171차) — 정기 지출(갱신료·조합비·어장 행사료·선박 유지비·위생 점검) + 어종 사진 정정**

- 사용자 질문 — "자격을 취득한 이후로 수수료가 발생하지 않게 되면, 밸런스가 안맞지 않을까?"
  면제 폭 자체는 작다(자격 총액 **841,000원** 중 이야기 경로 면제 **181,000원 = 21.5%**) —
  진짜 구멍은 **자격 비용이 1회성**이라는 구조였다.
- 그리고 갱신 장치는 **선언만 되어 있고 죽어 있었다**: `requiresRenewal`·`renewalIntervalDays`·
  `expiresAt`·`checkLicenseExpiry()` **호출측 0건** + `expiresAt`이 **실 벽시계 365일**이라
  발화 자체가 불가(144차 `'run'` · 147차 `AuctionEngine` · 148차 `bagSlots`에 이은 **네 번째 같은 패턴**).
- core **`rules/Upkeep.ts`** 신설 — 시계를 **인게임 일자**(`StoryStore.storyDay`)로 재정의.
  ⚖ **만료는 봉쇄가 아니라 압박**(자격이 사라지지 않고 '미갱신' — 위판만 막히고 평판 −1/일(상한 3),
  갱신하면 즉시 복구 · 149차 「닫히지 않는 목표는 만들지 않는다」와 같은 원칙).
  갱신료 = 취득가 25%(하한 2,000원) · 180일 · 전 자격 1회차 **187,000원**.
  ⚖ 조합비는 비용이 아니라 **선택**(월 12,000원 → 위판 수수료 −0.5%p · 어장 행사료 5,000 → 1,000원).
- **어종**: 170차가 md5 동일만 보고 "중복·둘 다 쏨뱅이"로 오진한 것을 사용자 사진 3장으로 정정
  (청볼락 = `dark-banded_rockfish` · 쏨뱅이 · **놀래미 = 노래미**) + 일반 볼락 종 제거 → 청볼락 흡수.
- ⚠ **재화 곡선이 아래로 이동한다** — 153차가 올린 것과 방향이 반대라 합산은 실플레이 확인 대상.
  상세: [171차 워크로그](../docs/wiki/03-WORKLOG/2026-09-23-171-upkeep-fish-photo-fix.md).

**이전 변경 (2026-09-22 170차) — 어종·요리 실사 매핑 정정 + 자격 이야기 경로 + 12차 테스트 배포**

- **dev 서버로는 테스트가 불가능하다**(사용자 `ERR_CONNECTION_REFUSED`) — 서버는 200이고 `host: true`도 이미
  켜져 있지만, 이 세션은 원격 컨테이너라 사용자 PC의 `localhost:5173`에는 아무것도 없다.
  실제 테스트 경로는 **gh-pages 재배포** — 직전 11차(09-18)라 **154~169차가 통째로 미배포**였다 → **12차 배포**.
- **매핑 정정 6건**(사용자 지적) — 쏨뱅이↔놀래미(두 파일이 **md5 동일한 중복**이고 내용은 둘 다 쏨뱅이) ·
  일반 볼락 사진 회수 · 열기·조피볼락 신규 · **돌돔 수컷 임계 40 → 30cm**(`isStripelessMale` 단일 정의) ·
  용치놀래기·돌돔 암수 **위키 2장 노출** · **문어 → 돌문어 이름 통일 + 학명 기준 카드 병합**(두족류 7 → 6).
  과증식 4종·매운탕 2종 실사 배선(`FireRecipeDef.photoKey` 신설 — 같은 family라도 그림이 다르다).
- **자격 이야기 경로 8건** — 관련 할 일을 끝내면 추천으로 **수수료·실적 요건 면제**.
  ⚖ 네 가지 주의사항을 그대로 장치로 옮겼다: 가산만(면허사무소 경로 무수정) · `once`/`event` 연결 **검증기 거부** ·
  **아는 할 일만 안내**(§8-10 R2) · **구조 게이트(선행 면허·선행 할 일)는 면제 불가**.
  ⚖ 레시피·조법은 제외(사용자 — 자유도 보호). ⚖ 면제는 `QuestRewards` 밖 별도 테이블이라 **메인 전용 제약과 만나지 않는다**.
- 검증: 빌드 3/3 · typecheck 0 · 자격 경로 0건 · 신규 텍스처 11/11 로드 · 위키 이미지 289/289 · 390px 0 · pageerror 0 · 404 0.
  상세: [170차 워크로그](../docs/wiki/03-WORKLOG/2026-09-22-170-asset-mapping-license-story-route.md).

**이전 변경 (2026-09-22 169차) — 전체 위키 도감화: 분류 버튼 · 상세 · 그림**

- 사용자 지시 3건(요리·어종·아이템에 **카테고리 버튼 + 상세 + 이미지**, 제작은 미구현이라 제외)을 반영해
  아티팩트의 목록 탭을 **분류 칩 + 세부 분류 칩 + 썸네일 카드 + 상세 시트**로 재작성했다.
- 요리 9종 = 조리법(단계·재료 범위·투입 시점) · **가치(1x 기준)** · **완성도 6구간 실지급 표**(탄 것 0원 → 별 5개) · 주재료 변형.
  수산생물 **79종을 5분류**(어류 52·두족류 7·패류 9·갑각류 4·기타 7 — 채집 18·과증식 4 신규 편입).
  아이템은 정규식 145건 → **실카탈로그 318건**(분류·가격·설명·판매처·제원).
- 그림 파이프라인 신설 — `tools/gen_wiki_images.py`(BootScene 텍스처 + `PixelIconArt` → 썸네일 215장) ·
  `tools/dump_wiki_items.mjs`(esbuild로 클라 `WikiCatalog.ts` 덤프). ⚠ `tools/wiki_img/`는 gitignore —
  **그림 단계를 건너뛰면 조용히 그림 없이 구워진다**.
- 검증: 실클릭 pageerror 0 · 그림 100/100 로드 · 390px 가로 스크롤 0 · 빌드 3/3 · typecheck 0. 게임 코드 무변경.
  상세: [169차 워크로그](../docs/wiki/03-WORKLOG/2026-09-22-169-wiki-catalog-categories-images.md).

**이전 변경 (2026-09-22 168차) — 장면 생성기 재작성 · 채집 스팟 거대 사진 수정 · 전체 위키 아티팩트**

- 사용자 지적 「'이름없는꾼' 멘트와 '혼잣말' 멘트가 겹친다」 → `generatedScene` 재작성: 혼잣말 = 나레이션 문장만, NPC = 발주자 `offer`/`done`만(없으면 침묵),
  플레이어 = 짧은 응답, 자막 = 지금 서 있는 곳. 손글 3편(N19-1 사용자 각본 · M4-11 탁만수 동석 · M6-05 조행록 `give`).
- 「사진 속 천막」 캡처의 이물질 = 채집 스팟이 아이템 아이콘 사진(1254px)을 키 충돌로 1881px 렌더 → `forage_dot_<id>` 분리 + `SPOT_MAX_PX`. 속초 34지점 스윕 이상 0.
- 전체 위키 아티팩트 재발행(조행록 「장면」 절 + 요리·어종·제작·아이템·시스템 탭) · `tools/dump_quest_scenes.cjs` 신설.
- 검증: 100편 정적 0결함 · 실플레이 12/12 · 빌드 3/3 · typecheck 0. 상세: [168차 워크로그](../docs/wiki/03-WORKLOG/2026-09-22-168-scene-generator-render-audit-wiki.md).

**이전 변경 (2026-09-22 167차) — 퀘스트 장면 게이트: 클릭으로 닫히던 목표 100건을 컷씬으로 · N18-6 「위반 증거」 사용자 각본 · 아이콘 2건**

- 「클릭만 해도 클리어」의 정체 = `DialoguePanel` 생성자의 `talk` 이벤트(79건 — 대화창을 여는 순간 닫힘) + `[이 자리에서 마무리]` 클릭(수동 21건) +
  대화·선택 계통 행동 키의 선택 클릭(8건). 셋 다 지웠다. `StoryEvent 'scene'`이 **그 목표 하나만** 닫고, 컷씬이 끝나야 발행된다.
- `data/QuestScenes.ts` — 나레이션 층에서 장면을 조립하는 생성기 + 손글 표 `QUEST_SCENE_OVERRIDES`. 상대가 필드에 없으면 발주자가 입구,
  상대는 임시 배우로 걸어온다. 컷씬 스텝 `moveTo`·`faceTo`·`fade`·`remove`·`give`·말풍선 `...` 신설.
- N18-6 ① = 사용자 각본 그대로(경매장 동쪽 벽에 숨은 수상한 사람 → [F] → 숨어 관찰 → 6줄 교차 → 명부 전달 → 퇴장 → 혼잣말). ③은 도현수 대화 행.
- 혼무시·감성돔 바늘 도트 매핑 + 구세이브 `iconTexture` 백필. ⚠ 다른 지역 퀘스트는 입구 NPC가 없어 아직 열 수 없다.
- 검증: 실렌더 7항목(목격·보고·손글·생성·행동 선택·ESC·열기만) · 빌드 3/3 · typecheck 0 · pageerror 0. 상세: [167차 워크로그](../docs/wiki/03-WORKLOG/2026-09-22-167-quest-scene-gate.md).

**이전 변경 (2026-09-22 166차) — 마을 NPC 체계 교체: 구 gem 프롭 삭제 · 마을 사람 자율 배회 · 스토리 NPC 3x3 자유 행동(낚시 루프)**

- 구 gem NPC(할아버지·경찰관·관광객 정지 그림)는 `SeamlessChunks` 보행자 산포와 편집기 팔레트에만 남아 있었다 → 산포·`PROP_DEFS`·매니페스트 로드 전부 삭제.
  마을 사람은 `scenes/field/FieldNpcSystem.ts`가 `characterOf('amb_*')`로 결정적으로 만들어 보도·맨땅에 세우고(속초 120명 · 건물 반경 6타일 마을 편향) 스스로 걷는다.
- 스토리 NPC 7인은 `StoryNpcs.behavior`대로 앵커 3x3 안에서 움직인다 — **낚시(도현수·강두철)** = 캐스팅→대기→회수 탑다운 루프(찌·원줄·그림자) · 좌판(정옥선) · 순찰(어촌계장) · 배회(탁만수·배누리·바람).
  ⚖ 낚시는 낚시와 관계있는 인물에게만 · 바쁨 미적용([F] 대화 가능) · 컷씬 중엔 AI가 손을 뗀다 · `n.x/n.y`가 좌표 정본.
- ⚠ 도현수·강두철 앵커는 주차장 한복판(반경 26·33타일 안 물 없음)이라 물가로 옮겼다(534,151 · 567,138). N18-6 보고 트리거 동행.
- 아이콘 29장 점검 — `it_float_zero_v2` 미매핑 → 전용 도트 교체. 에셋 없는 잔여 = 혼무시·감성돔 바늘 3호(이모지).
- 검증: 실렌더 gem 오브젝트 0 · 낚시 상태 전이 전부 · 3x3 이탈 0 · 이름표/마커 동기 · 빌드 3/3 · typecheck 0 · pageerror 0. 상세: [166차 워크로그](../docs/wiki/03-WORKLOG/2026-09-22-166-field-npc-behaviors.md).

**이전 변경 (2026-09-22 165차) — 컷씬 런타임 재작성 · 얼음 나르기 실플레이 · 「지금 할 일」 메인/서브 · 일지 고정 · 지도 툴팁**

- Codex 163·164차의 핀트를 정정했다. 컷씬은 검은 폴백 무대를 버리고 **실제 필드 위**에서 배우·이름표가 움직이고
  말풍선·HUD 대사창(20px)이 순서대로 흐른다. ESC는 건너뛰기만 하고 일시정지 메뉴로 새지 않는다(`skipCinematic`).
- M1-02 얼음 나르기 = 수락 즉시 상자 지급 → 하역 표시 [F] → 추적기 `얼음 나르기 (완료!)` + 데이터 안내(`afterKo`).
  목표 계약에 `howToKo`·`afterKo`·`guidePlaceKey`를 신설해 퀘스트별 하드코딩을 회수했다.
- 「지금 할 일」은 메인 → 서브 2블록(Ⓜ/Ⓢ · 사용자 지정), 일지 첫 컬럼 고정 체크(메인 1·서브 1 · `StoryStore.pinned`),
  전체 지도 마커 호버 이름, 가짜 웜 = 고무 재질·신선도 무영향 분류, 행동 계통 중복 매칭 가드, EN 잔존 0.
- 검증: 실렌더 pageerror 0 · overflow 0 · 빌드 3/3 · typecheck 0. 상세: [165차 워크로그](../docs/wiki/03-WORKLOG/2026-09-22-165-field-cinematics-quest-ux.md).
  ⚠ 잔여 = 원격지 컷씬 실발화(인천 맵 대기) · 문서 사본 드리프트.

**이전 변경 (2026-09-20 164차) — 수동 custom 목표 27개 행동화·6계통 이벤트 배선·계통별 선택/우호도 분기**
...
- 사용자가 준 명제는 **「레시피 ≠ 요리」** 였다. 154차는 **레시피 1개 = 아이템 1개**라 우럭을 넣든 광어를 넣든
  결과가 늘 같은 `매운탕`이었다 — **낚시 게임에서 조리는 어획의 종착지인데 어획이 결과에 남지 않았다.**
  착수 범위도 사용자가 지정했다: *"매운탕 3종 → DishInstance → 상세보기 → 요리 도감까지만 먼저 완성하고,
  정상 작동을 확인한 뒤 나머지 7개를 확장"* — 이번 차수는 **기획서 Stage 1~6**까지다.
- **core 5파일 신설** — `FishCookingProfiles`(지방감·식감·비린 향·풍미·국물 기여 + 라벨 + 중량 밴드) ·
  `RecipeLore`(설명·조리/재료 요령 + 어종별 변형 설명) · `FoodEffects`(효과 8종) · `DishDiscovery`(도감 id·이름·후보) ·
  **`DishFactory.createDishInstance`**(프로필 × 중량 × 신선도 × 154차 품질 → 이름·회복치·효과·판매가).
  `TUNING.cook.{ingredientQuality, dishEffect, naming, pricing}` 신설(**mockup — F8 조율 대기**).
  ⚖ **154차 조리 엔진은 한 줄도 안 고쳤다** — `finishCook`이 낸 `DishData` 위에 얹는 층이다.
- 실측: 같은 900g·같은 신선도에서 **감성돔 ₩31,800·허기 +29 / 우럭 ₩30,600·+27 / 광어 ₩29,100·+25**,
  효과 지속은 지방감 비례 32/29/28분. 중량 350 → 1,300g: 허기 18 → 31 · ₩19,700 → 33,600(상하한 클램프).
  신선도 0.92 → 0.55: 품질 92 → 55 · ₩24,700 → 21,200 · 수식어 소멸. 60분 뒤 총점 94 → 67 · ₩19,500.
- ⚖ **개체마다 새 아이템을 만들지 않는다** — `InvItem.dishInstance`에 실려 저장되고 이름·회복치·가격·효과가 거기서 나온다
  (카탈로그·상점 불변 · `dishInstance` 없는 154차 요리는 기존 분기 = **구세이브 호환**).
  ⚖ **실기전 효과는 포만감 하나**(나머지 7종은 표시 데이터 — 생존 밸런스를 한 번에 8축 열지 않는다).
  ⚖ **수식어 최대 1개**(탄 → 일품 → 싱싱한 → 기름진). ⚖ **도감은 (레시피 × 주재료) 1건**(개체마다 등록하면 도감이 로그가 된다).
- **요리 도감 탭(N)** — 레시피 목록 + 변형 카드. 매운탕 **3 / 50** · 전체 **발견 3 / 57** · 미발견은 `??? 매운탕` + 「다른 어종을 사용해 보세요」.
- ⚠ **실측 결함 3건**: ① 발견 토스트가 **내부 id를 그대로 노출**(`inv_dish_stew_red_1` — §8-9). `addItem`이 모든 아이템을
  `record('item')`하는데 개체형 id는 위키 카탈로그에 없어 `displayNameOf`가 id를 돌려준다 → **카탈로그 밖 아이템은 알리지 않는다** +
  요리는 전부 `kind: 'dish'`로 통일 ② **효과 지속 문장이 개체마다 달라**(`32분 동안 …`) 정확 일치 사전을 비껴갔다(131차 함정의 변종)
  → i18n **런타임 규칙**(효과 DB의 Ko/En 쌍에서 숫자 자리만 캡처로) 신설 ③ 비린 향 라벨은 4단계인데 프로브가 3개라 **`약함`이 빠졌다**.
- 상세: `docs/wiki/03-WORKLOG/2026-09-18-156-dish-instance-cooking-codex.md`

**이전 변경 (2026-09-18 155차) — 인지 UX 전면: 퀘스트 화살표·추적 + 전체 지도(M)·핀 + 채비 고정 + 획득 토스트 + 사시미 별 5개 + 3끼 생존 재보정 + 초상 정합 + 혼잣말 퀘 우호도 정정** (사용자 지시 10건 + 캡처 추가 3건 — 실렌더 전 항목 · EN 잔존 0 · overflow 0 · pageerror 0 · 아티팩트 v9, 빌드 3/3·typecheck 0):

- **우호도 이질**(사용자: *"혼자하는 일인데, 왜 우호도+0.03이 적용되고 있지?"*) — `choicesFor`가 무발주 퀘(M1-01·M7-08)에도 메인 톤 세트를 폴백해
  `affinity['']`에 쌓였다. **혼잣말 전용 선택지**(효과 없음) + `applyOutcome` 발주자 가드 + 로드 시 빈 키 제거 + **검증기 확장**
  (무발주 = 우호도·거절 금지 · `affinityOther` ≠ 발주자) → **데이터 오류 3건 적발**: N13-3·N05-5 발주자 이중 가산 · **N09-2 발주자 오기**(대사는 한봄) → 정정.
- **인지 UX**(테스터: *"얼음을 어떻게 운반해야 하는 지 모르겠다"*) — core `rules/QuestGuide.ts`가 목표마다 「방법」 문장을 내고
  (일감 = `정옥선에게 [F] → 「일손이 필요하진 않으세요?」 → 일감 「얼음 나르기」`) 대화창·일지·**필드 추적기**가 같이 쓴다.
  **점멸 화살표**(추적 → 활성 메인 → 수령 가능 순) + 일지 「이 할 일 추적하기」(`story.tracked`) · **우측 획득 토스트**(아이템·재화 — `onGained`·`onCoins`) ·
  인벤 NEW 점·귀속 금테. 내부 id는 `GUIDE_NAMES`로 이름 치환(§8-9).
- **전체 지도(M)** `ui/FullMapPanel.ts` — 큰 창 + 지역명 · 휠 줌(맞춤~×8)·드래그 · 마커(랜드마크·상점·식당·NPC·퀘스트) · 좌하단 범례 16종 ·
  **빨간 점멸 현위치** · 피어 파란 점 · **우클릭 핀 설정/제거**(퀘스트 추적과 별개 · 청록 화살표) · **휠 = 내 위치 중심** · 좌→우 드래그 = 지도 왼쪽으로(`DRAG_DIR`). 미니맵은 **타이틀바 [−][+]**로 크기 순환.
- **채비 고정** — U 채비 탭 **우측 하단** [채비 고정]/[고정 해제](사용자 캡처 위치 · 캐스팅 전 팝업은 정정으로 제거 — 고정은 관문이 아니다) ·
  `InventoryStore.rigLocked` 세이브 · 고정 중 소켓·루어·모드 변경 무시 · 던질 때 소모품(미끼)만 줄어든다. **1인칭 결과 후 항상 탑다운 복귀**(`recast` 폐기).
- **사시미 별 5개** core `SashimiQuality.ts`(신선·컷·식감(칼)·구성(4방위 균형)·완성도 + 감쇠 — 정석 5/92 → 5h 뒤 2/51 · 별 판매 배율 0.45~1.2 · `(별 N개)` 이름).
  **생존** 드레인 하루 3끼 기준(전형 하루 허기 105 / 수분 134) + core `FoodNutrition.ts`(g·kcal·ml → 회복치 · `ItemVitals`는 파생 · 설명문 손수 수치 6곳 제거).
- **초상 정합** — 초상 렌더러가 필드와 다른 문법(항상 눈썹 · 흰자 2행 눈 · 콧수염 · 모자 미덮음)을 썼다 → 앞머리·동공 바·턱수염·`crown(bot)`으로 필드 규칙에 맞춤(시트 불변).
- ⚠ 하네스 함정 2건 신규(스킬 `verify-render`): 헤드리스 rAF 300ms↑ → 시간 갱신은 메서드 직접 호출 · `update()`가 `nearWater`를 매 프레임 재계산.
- 상세: `docs/wiki/03-WORKLOG/2026-09-18-155-quest-guide-fullmap-rig-lock.md`

**이전 변경 (2026-09-18 154차) — 불요리: 화구 조합 설치 + 실시간 조리 시뮬 + 맛 별 5개 + 걸어두기** (로드맵 다음 단계 + 사용자 지시 3건 — 실렌더 매운탕 5별 95점 · EN 잔존 0 · 세이브 왕복 · pageerror 0, 빌드 3/3·typecheck 0):

- **조합 설치**(사용자: "하나의 아이템을 단순 설치가 아니게") — 인벤 [화구 설치] → 스토브 + 캐니스터(끼우면 소모·강불 55분) +
  용기(코펠/양수/팬/석쇠 — 나중에 올려도 됨) → 뭍 3타일 클릭. `[F]` 조리 / `[Shift+F]` 회수(연료 소실). 집 주방 가스레인지 = 연료 무한·수돗물.
  골격은 121차 통발(`TrapFieldSystem`) — GameState 소유·wall-clock·피어 공유는 통발 채널 `kind:'stove'` 편승(서버 무수정).
- **리얼리즘** — 레시피 8종(매운탕·지리·생선구이·조림·오징어볶음·조개탕·전복죽·장어구이). 필수/부가/택1/한도(넘기면 완성도 하락) ·
  소금·설탕 **양**(0.25큰술 단위 → 싱겁다/적당하다/짜다) · 단계 순서 · 열 모델(100°C 캡+증발·팬 150°C·바람 감쇠·용기 열관성) ·
  익음(`rateExp 2.2`)·탐(burnC/무뒤집기/졸아붙음). ⚖ **창작 요리 배제** — 레시피에 없는 재료는 거부.
- **맛 별 5개** = 간·온도·식감·신선도·**완성도**(앞 넷 조화 × 구성 × 순서 × 불 조절 × 스킬 — 넷 중 하나라도 모자라면 다섯째 별 없음).
  완성 직후부터 **시간 감쇠**(온도 별 20~30분·신선 240분) → 상세보기가 「지금」/「완성 직후」 별점을 함께 보인다. 탄 요리 0원.
- **걸어두기** — 실 10초 = 조리 1분(`timeScale 6`). 탑다운 배지 `끓는 중 N%`/`완성`/`탔다!` · 약불 = 보온(졸지도 타지도 않음) ·
  강불 방치 = 시뮬 33분에 탐 · 오프라인 갭은 세이브 로드 시 `lastTickMs` 보정(정지).
- ⚠ 실측 6건 — 코펠 용량이 고형물을 1:1 부피로 세어 감성돔 한 마리가 안 들어갔다(`solidVolumeFrac 0.6`) ·
  **회수하면 스토브·용기가 증발**(`recoverPlaceable`이 홈타운 설치물만 알았다 → `COOK_CORNER` 폴백) · 패널 정적 층이 세션 변화를 못 따라옴(레이아웃 키) ·
  상세보기 재료 행이 패널 왼쪽으로 삐짐(3개씩 행 분할 + 값 클램프) · 큰 고기 거부 사유 · **vite가 core를 `@fs` 소스로 서빙해 dist 빌드 실패가 조용히 묻힌다**.
- 도움말 「불요리」 3토픽 · 픽셀 아이콘 68 → **83** · 식자재마트 코너 28종 · `life_cook` wired · N23-6 `cook` auto.
- 상세: `docs/wiki/03-WORKLOG/2026-09-18-154-fire-cooking.md`

**이전 변경 (2026-09-18 153차) — 대화 전용 70편 실목표 배선 + 아티팩트 장 접기 + 「할 일」 용어 통일** (사용자 지시 2건 — 무결성 0건 · 추적 불가 자동 목표 0건 · 아티팩트 pageerror 0, 빌드 3/3·typecheck 0):

- **대화 전용 70편(38%)을 전부 조업으로 바꿨다.** 152차는 그것을 **정직하게 표기하고 품삯을 0.25배로 낮췄을 뿐**
  이었다 — 라벨이 무엇을 약속하든 실제 조작은 대화 [다음 단계] 한 번이었기 때문이다(134차 `manual` 설계).
  메인 25·서브 45에 **추적기가 실재하는 목표만** 심었다: `catch`·`release`·`butcher`·`sashimi`·`gather`·
  `craft`·`sell`(위판)·`trap`·`custom`(`job:*`·`buy:*`). 분포 **대화 70 → 0**(수월 26·보통 112·까다로움 44·고난도 4).
- ⚖ **닫히지 않는 목표는 만들지 않는다** — 진행 중 세이브를 막기 때문이다(149차 규칙).
  ⚠ **`spotKind: 'boat'`는 어디서도 발화하지 않는다**(`standingSpotKind()`는 `breakwater|shore`만, 1인칭은 `hole|shore`만)
  → **선상 조건을 단 목표는 하나도 만들지 않았다.**
  ⚠ **M3-07 「종묘방류」의 `communityWork`는 속초 전용 일감(`job:coop_work`)이라 무대인 포항에서 영원히 안 닫혔다**
  → **치어 방생 5회**로 교체(152차가 "유지"로 남겨 둔 항목의 실제 해법).
  추적기가 아직 없는 종류(`cook`·`boatTrip`·`holdPosition`·`survive`·`furnish`·`farm`·`mine`)의 `manual` 줄은
  **이야기 장면 표시로 남기고** 그 옆에 실목표를 붙였다 — 바뀐 것은 "대화만으로는 끝나지 않는다"는 사실 하나다.
- ⚖ **난이도 구간 재조정** 1/4/8/13 → **1/3/6/8**. 70편이 점수를 갖자 분포가 통째로 올라가(중앙값 3 → 5)
  구 경계로는 「고난도」가 영영 비고 「보통」이 3분의 2를 먹는다. `custom` 가중치도 **0 → 1** —
  manual은 이미 제외되므로 여기 남는 `custom`은 **실제로 발화하는 사건**(품삯 일감·지정 물건 구입)뿐이다.
  ⚖ **「대화」 구간은 비어도 남긴다**(표기는 계약 — 앞으로 그런 임무가 생기면 그때도 그렇게 적혀야 한다).
  ⚠ 70편이 품삯 0.25배 구간을 떠나므로 **Ch2~Ch7 재화 곡선이 위로 이동한다**(152차가 의도한 방향).
- **지시에 없던 죽은 배선 2건** — ① `CraftingStore`가 `GameState.grantXp(activityXp('craft', …))`를 **직접** 불러
  `addActivityXp`를 비껴갔다. 그래서 **제작 `activity` 이벤트도, 140차 제작 숙련도도 한 번도 오르지 않았다**(호출측 0건 실측)
  → `addActivityXp` 경유로 복구(XP 산식 동일) ② **면허 요구사항이 내부 id를 그대로 화면에 출력**(§8-9 R1 위반) —
  `• 퀘스트 완료: quest_first_catch_and_cook` · `spotId` · `fishId` → 이름 조회로 교체(노출 0건 실측).
- **아티팩트 색인** — 열람 중인 장이 접히지 않던 범인은 `if (!openCh.size && cur) openCh.add(cur.chapter);` 한 줄
  (마지막 장을 접으면 즉시 되열렸다). 자동 재개방을 지우고 **`모두 접기 / 모두 펼치기` + `7장 · 186편`** 을
  색인 상단 고정 줄에 — *"평시에 챕터가 총 몇 개인지 눈으로 보는"* 상태가 기본이 됐다(사용자 원문).
  접힌 장에는 **금색 바 + 점 + `열람 중 — …`** 흔적을 남기고, 다시 펴면 그 행이 강조된다.
  ⚠ 내가 쓴 `keepCurrentVisible()`의 `offsetTop` 계산이 sticky 색인에서 어긋나 **선택 행이 55px 밖에 남았다**
  (실측 `selTop 884 > idxBot 878`) → `getBoundingClientRect` 기준으로 교체.
- **용어 임무 → 「할 일」 전수**(사용자가 든 예시 그대로) — 일지 패널·도움말·단축키·피어 정보·
  HUD 로그 `[퀘스트]` → **`[할 일]`** ×6·면허/스킬 요구사항·아티팩트 전문. 남긴 예외는 152차에 사용자가 직접
  이름 붙인 **`조행록 퀘스트 항로도`(제호)와 `퀘스트 정보 개요`(인포박스 머리)** 둘뿐.
  ⚠ `[퀘스트] … 수락/완료/거절/미룸/달성/보상` 6종은 **영문 규칙 자체가 없어 영어 화면에 한국어로 남아 있었다** → 규칙 신설.
- 검증: 무결성 0건 · **자동 목표 186퀘 전수 추적 가능성 0 오류** · 나레이션 `obj` 길이 불일치 0 ·
  나레이션 EN **855/855** · 선택지 EN 1048/1048 · 인게임 '임무' 잔존 0 · 아티팩트 실클릭(접기·재개방·모두 접기·
  색인 스크롤) · 1280/390px 가로 스크롤 0 · pageerror 0.
- 상세: `docs/wiki/03-WORKLOG/2026-09-18-153-real-objectives-chapter-collapse.md`

**이전 변경 (2026-09-17 152차) — 퀘스트 클리어 난이도 + 선택지 영문 1,048줄 + 대화 초상 재렌더 + 아티팩트 수정 4건** (사용자 실검증 지시 5건 — 영문 커버리지 1048/1048 · 아티팩트 pageerror 0, 빌드 3/3·typecheck 0):

- **"대화만 했는데 재화를 지급" — 버그가 아니라 134차 설계가 드러난 것이다.** `StoryObjective.manual`은
  자동 추적기가 없는 목표를 대화 [다음 단계]로 닫는 장치이고, `StoryStore.event()`가
  `if (o.manual && ev.kind !== 'talk') return;`으로 **실제 행위 이벤트를 전부 무시**한다.
  라벨이 무엇을 약속하든 요구되는 조작은 대화 한 번이었다. 실측: manual 목표 84건 중 77건은 `talk`(무해) ·
  **실제 문제 7건** · 라벨에 개발 상태가 노출된 것 **112건**.
- **core `rules/QuestDifficulty.ts` 신설** — **수행 활동만** 세는 클리어 난이도
  **대화 / 수월 / 보통 / 까다로움 / 고난도** 5단. ⚖ **`manual` 목표는 0점**(강제되지 않는 목표를 세면
  표기가 곧바로 거짓말이 된다) · ⚖ **요구 레벨·보상 크기는 제외**(레벨은 도달하면 사라지는 관문이고,
  보상은 난이도의 결과지 원인이 아니다) · 조건 한 겹(어종·크기·계절·조법·장소·자가어획)마다 +1 ·
  수량 2~3회 +1 / 4~7 +2 / 8+ +3 · 기한 +1. 분포 **대화 70 · 수월 33 · 보통 79 · 까다로움 4**.
- ⚖ **품삯에 난이도 배율**(`payChoiceCoins` — 대화 **0.25배** / 수월 0.6 / 보통 1 / 까다로움 1.3 / 고난도 1.6).
  **0으로 만들지는 않는다** — 대화로 끝나는 임무도 이야기를 진행시킨다. 다만 **손을 쓴 임무와 같은 액수를 주지 않는다.**
- **manual 해제 4건**(추적기 실재 확인): M3-03 `license` · N23-2 `visit` · N20-3 `visit` · N23-3 `sashimi`.
  ⚠ **auto `visit`은 `placeKey`가 없으면 영원히 안 닫힌다**(`match()`가 `o.placeKey === ev.placeKey`) —
  둘 다 `region:{지역id}`를 함께 채웠다. **유지**: M3-04 craft(불요리 미구현)·N23-6 cook(호출처 0)·
  M3-07 communityWork(`coop_work`가 속초 전용인데 무대는 포항). 라벨 개발 상태 접미 **112건 제거**(§8-9 R1).
- **선택지 영문 1,048줄** — 151차가 `labelEn = labelKo` 폴백으로 남긴 빚. 나레이션과 같은 **짝 파일**
  `db-schema/StoryChoicesEn.ts`(524엔트리) + `mergedNarrative()` 합본 → **UI 호출부 변경 0**(i18n은 원문이 곧 키).
  `choiceEnCoverage()` **1048 / 1048 · 누락 0** · 사전 전수 `ko == en` **0건**.
  ⚠ **키에 세트(`c` 완료 / `o` 발주)를 넣는다** — 한 퀘스트에 같은 id의 발주·완료 선택지가 함께 있다(실측 4건 충돌).
- **대화 초상 재렌더** — 150차는 게임 시트의 얼굴 창(16x16)을 잘라 NEAREST로 키웠는데, 그 격자의
  **실제 얼굴은 가로 10 · 세로 9줄**이라 몇 배로 키워도 눈은 사각형 두 개고 코가 없었다
  (사용자: "단순 확대가 아니라 픽셀 단위와 그래픽 퀄리티를 높여야 함"). core **`art/FacePortrait.ts`** 신설 —
  **같은 `CharConfig`로 32x32 전용 격자에 다시 그린다**: 흰자·홍채·동공·하이라이트·눈꺼풀선 각자의 화소,
  코(콧대 그늘+하이라이트+콧방울+인중), 입술 2행, 귀·턱 그늘·수염결·앞머리 술·눈썹 각도·모자 챙.
  ⚠ **필드 스프라이트는 한 줄도 안 건드렸다** · **표시 크기 동일**(scale을 32 격자 배율로 환산) ·
  텍스처 키 `face_*` → `face2_*`(런타임 생성 · 세이브 무관).
- **아티팩트 4건**(같은 URL 재발행): 장 표기 `제n장 · 챕터이름 — 지역` · **장별 드롭다운 색인**
  (7장 186편을 한 줄로 펼치면 5장을 보려고 끝없이 굴려야 한다) · `임무 정보` → **`퀘스트 정보 개요`** ·
  **겹침 해소** — 범인은 인포박스의 `float:right`였다(본문이 박스 아래로 파고든다) → **2열 머리 그리드**로 교체,
  실측 인포박스 하단 714 < 첫 섹션 887 · 390px 가로 스크롤 없음 · pageerror 0. 인포박스에 **난이도 행** 추가.
- ⚠ 사용자 판단으로 닫힌 항목: **시드 낚싯대 기본 착용** — "착용 해제하면 되니까 관계없을 것 같네."(조치 없음)
- 상세: `docs/wiki/03-WORKLOG/2026-09-17-152-quest-difficulty-en-choices-portrait.md`

**이전 변경 (2026-09-17 151차) — 영문 나레이션 186편 + 장비창 페이퍼돌 착용 반영 + 퀘스트 아티팩트 위키형 재발행** (사용자 지시 3건 — 영문 커버리지 829/829 · 아티팩트 데스크톱/폰 pageerror 0, 빌드 3/3·typecheck 0):

- 백로그에 이름만 올라 있던 빚 셋(O1·O2·O4)을 한 번에 갚은 차수다.
- **영문 나레이션 186편** — 141차가 헤더에 "영문은 비워 둔다 … 사전 편입은 후속"이라 적고 미룬 것.
  한국어 원고와 **같은 키를 쓰는 짝 파일** `db-schema/StoryNarrativeEn.ts`를 신설하고
  `narrativeOf()`가 `mergedNarrative()`로 합본을 낸다. 화면에 닿는 경로는 **i18n 런타임 사전**
  (`allNarrativeLines()` → `buildRuntimeDict` — **원문이 곧 키**)이라 **UI 호출부를 한 줄도 안 고쳤다**.
  커버리지 점검기 `narrativeEnCoverage()` 신설 — **829 / 829줄 · 누락 0**.
  ⚠ 잔여 = **선택지 영문 510개**(`complete`/`offerChoices`는 여전히 `labelEn = labelKo` 폴백).
- **장비창 페이퍼돌 착용 반영** — 138차가 페이퍼돌(레이어 14종)을 세웠는데 **인벤토리의 착용 상태를
  읽는 곳이 한 군데도 없었다**(`refreshCharacterLook()` 호출측 0). 미구현이 아니라 **끊어진 배선**이다 —
  `GameState.character.outfit`은 생성 때 한 번 정해지고 아무도 바꾸지 않았다.
  `client/data/EquipOutfit.ts` 신설(아이템 id 표 우선 + `subCategory`·이름 낱말 추론 + 손 도구 → `held`),
  렌더 호출측 5곳(장비창·필드 스프라이트·혼잣말 초상·멀티 외형)을 **`characterLook()`** 으로 교체,
  `inventory-changed`·장비창 `onChanged`에서 재굽기.
- ⚖ **덮어쓰기지 대체가 아니다** — 생성 때 고른 옷차림이 **기저**로 남고 착용분만 위에 덮는다.
  전면 파생으로 만들면 시드 옷(모자·조끼·바지·신발)이 **기본 미착용**이라 새 캐릭터가 벗고 시작한다(실측).
  ⚠ **기저는 절대 수정하지 않는다** — 매번 기저에서 다시 덮으므로 여러 번 불러도 결과가 같다.
  기저를 갈아치우면 두 번째 호출부터 "벗은 장비가 안 벗겨진다". 세이브 스키마 변경도 없다.
- **퀘스트 아티팩트 위키형 재발행**(같은 URL) — 150차에 사용자가 PDF로 지정한 타르코프 위키 골격
  (제목+리드+목차 → 인포박스(유형·위치·의뢰인·연관 임무) → 대화 → 요구사항 → 목표 → 보상 + 클리어 대화 →
  안내(물품 표 + 공략) → 참고 → 여담)으로 재작성. 생성기 `tools/gen_quest_wiki_data.mjs --html` +
  템플릿 `tools/quest_wiki_template.html` — **수치를 손으로 적지 않고 `dist`에서 그대로 굽는다**
  (재실행 결과가 발행본과 **바이트 동일**).
- ⚠ 실측으로 잡은 것 2건: **390px 가로 넘침**의 범인은 표가 아니라 `grid-template-columns:1fr`
  (= `minmax(auto,1fr)`이라 색인의 min-content가 트랙을 밀어낸다 → `minmax(0,1fr)`) ·
  초기 렌더의 `scrollIntoView`가 **첫 화면에서 제호를 밀어냈다**(넘어갈 때만 스크롤하도록 인자 분리).
- ⚠ 부작용: 시드 낚싯대가 기본 착용이라 **필드 캐릭터가 늘 대를 들고 다닌다**(138차 `held` 설계 의도 —
  육안 확인 대상) · 멀티 외형은 **이름 확정 시 1회만** 전송하므로 그 뒤 갈아입은 것은 피어에게 안 간다.
- 상세: `docs/wiki/03-WORKLOG/2026-09-17-151-en-narrative-paperdoll-quest-wiki.md`

**이전 변경 (2026-09-17 150차) — 실검증 피드백 12건: 표현형 손질 + 스포일러 차단 + 표현 규칙 상시화** (사용자 지시 12건 + 참고 자료 3건 — 실렌더 47/47 · pageerror 0, 빌드 3/3·typecheck 0):

- 관통하는 지적은 개별 버그가 아니라 **"표현형이 AI가 함축된 텍스트로 direct하게 표시하려는 느낌"** 이었다.
  마지막에 사용자가 **"내가 지정한 부분들은 규칙으로 정해놔"** 라고 해서, 이번 산출물은 코드 + **상시 규칙**이다 —
  **§4 「인게임 표현 규칙」 R1~R10** 신설 · **§8 금지 9~11** 추가 · CLAUDE 7번 · `ui-panel` 스킬.
- **일지(J) 전면 재작성** — 구 구조는 186퀘를 챕터 트리로 통째로 펼쳤다(조행록 17장의 지표 어종·계절,
  인물 23명 이야기까지). 사용자: *"유저들의 재미를 다 떨어트리는 행위적 구현"*.
  타르코프 임무 화면으로 이식 — 좌 **표**(의뢰인 얼굴·임무·지역·상태·**진행률 N% + 바**) /
  우 **상세**(얼굴 초상 · **자체 스크롤 나레이션** · 목표 바 · 보상 카드) + **이야기 탭**
  (붉은 점선 척추 · 체크박스 · **현재 위치 ▼**). **기본 노출 1 / 186**(활성만) ·
  `완료한 임무 표시`·`잠긴 임무 표시` 토글은 **둘 다 기본 꺼짐** · 잠긴 임무는 **이름도 주지 않는다**.
- **대화창 전면 재작성** — 클릭 즉시 의뢰서가 펼쳐지던 것을 폐기. **좌→우 타이핑** · 넘치면 앞 줄이
  **위로 밀리는 로그** · 단락 끝 **흰 역삼각형 ▼** · 본문 **20px** · **표준 선택지 3종**
  (`내가 도와줄 수 있는 게 있을까요?` / `물건을 볼 수 있을까요?`(= `StoryNpcDef.shopId` 있을 때만) / `나가기`) ·
  초상은 전신이 아니라 **얼굴 확대**(`ensureFacePortrait` — 증명사진).
  **오프닝 혼잣말 6단락**(홈타운 첫 진입 1회 · `MonologuePanel`).
- ⚠ **한글이 입력되지 않았다** — `ev.key.length === 1`로 글자를 모으면 IME 조합을 못 받는다
  (조합 중 `key`는 `'Process'`, 완성 글자는 `input`/`compositionend`로만 온다).
  숨김 DOM 입력 **`ui/TextInput.ts`** 신설 → 캐릭터 만들기 이름 · 지역 채팅에 배선.
- ⚠ **도로 위 횟집의 기전** — 106차 `trimBuildingsOnRoads`는 **타일**만 걷어냈고, 화면의 아스팔트는
  101차 이후 **도로 벡터 밴드**가 그린다. 타일은 `'.'`인데 그림은 차도인 자리가 생기고,
  `resolvePoiDoor`의 차도 회피는 타일 문자만 봐서 못 걸렀다 → `SeamlessChunks.nudgeOffRoad`로
  **문 좌표**를 민다(스프라이트가 아니라 — 거래 판정·NPC·미니맵 핀이 한 좌표를 공유한다).
  속초 실측 **199곳** 이동.
- ⚠ **타일 건물 뒤 캐릭터가 가려졌다** — 145차 반투명은 POI 프리팹만 봤다 → 청크 지붕도 편입.
  원점이 제각각(프리팹 0.5/1 · 지붕 RT 0/0)이라 판정을 **경계 상자(`getBounds`)** 로 바꿨다.
- 차량 배율 ×1.4 / ×1.2 — 주행 **52.78px** · 주차 **66px** > 캐릭터 몸통 52px(사용자 지정 1.2~1.4배 밴드).
  별불가사리 `abundance` −30% · 장비창 구형 캐릭터(`man-idle-front`, 138차 폐기분) 교체 +
  이모지 고스트 16종 → **픽셀 아이콘**(48 → **68종**).
- ⚠ **세로 경계는 자동 교정이 없다** — `enforceTextBounds`는 가로만 고친다.
  `auditTextBottom`(`DraggablePanel.auditBottom()`) 신설 → 팝업 8종 전수 감사 offenders 0.
  면허 패널 '조건 미충족'(실측 484~497 vs 박스 492)은 사유 줄을 버튼 **위**로 올려 해소.
- **용어 치환**(R1) — `자격 사다리`·`서브 아크`·`메인 라인`·`회귀 고리`·`N03-6` 같은 **내부 명칭/ID**와
  `활용 (Utilization)`·`스테이터스`를 사람 말로. i18n 키가 원문이라 EN 사전도 함께 교체.
- 상세: `docs/wiki/03-WORKLOG/2026-09-17-150-ux-overhaul-spoiler-gate.md`

**이전 변경 (2026-09-17 149차) — 구멍치기(테트라포드·사석) + M1-04 장소 게이트** (로드맵 다음 단계 — 실렌더 47/47 · pageerror 0, 빌드 3/3·typecheck 0):

- **조법 자체가 없었다** — 학습 태그(`holeFishing`·`tetrapodSafety`)·일지 라벨·사이소 저가 릴대·
...
- core **`HoleFishing.ts`** 신설 — 구멍 깊이(테트라포드는 블록이 겹쳐 연안 수심보다 깊은 수직 굴 ×1.15 /
  사석은 완만해 ×0.85 · 만조 보정 · 1.5~9m 클램프) · 수면 거리 1.2~2.0m · **밑걸림 배율 ×2.2 / ×1.7** ·
  구멍 어종 가중(볼락 2.6·우럭 2.4·쏨뱅이 2.2·노래미 2.0 ↔ 방어·부시리·삼치 0.1) · 미끄러짐.
  `TUNING.hole` 15키 + F8 슬라이더 5종(**mockup — 실플레이 조율 대기**).
- ⚖ **새 단축키를 만들지 않았다** — `F` 체인은 NPC > 오브젝트 > 건물 > 불가사리 > 채집 > 통발로 이미 꽉 찼고,
  **채집 스팟이 테트라포드 발밑에도 생겨** 넣으면 서로 잡아먹는다. 낚시 행위는 마우스에 둔다:
  블록 위에서 **짧게 떼면 구멍치기 · 꾹 누르면 캐스팅**(`TUNING.hole.tapMs` 320ms — 1인칭 호핑/릴링과 같은 문법).
  같은 자리에서 두 조법을 다 쓰므로 **기존 캐스팅을 빼앗지 않는다**.
- **M1-04 장소 게이트** — core `StorySpotKind`(breakwater/hole/shore/boat) + `StoryObjective.spotKind` 신설.
  ⚖ **라벨이 약속한 만큼만 강제한다** — 라벨이 "방파제에서"라 `breakwater`를 걸고 구멍치기(`hole`)가
  이를 만족하게 했다(`spotKindSatisfies`). 구멍치기를 **강제하지 않은 이유** = 라벨보다 더 요구하면
  계약 위반이고 **진행 중인 세이브가 막힌다**. ⚖ 조건**만** 어긋난 어획은 조용히 버리지 않고
  `방파제에서 낚아야 인정됩니다`로 이유를 말한다.
- ⚠ **실측으로 잡은 것 2건**: ① 기본 `SeabedProfile`은 발앞을 `maxDepth × 0.16`으로 깎아
  **2.2m 구멍이 0.8m로** 그려졌다 → 생성자에 `{ floorRatio, alwaysRock }` 추가(구멍 = 발앞이 곧 바닥·전 구간 블록)
  ② 매 프레임 도는 구멍 판정이 `resolveCastDepth`의 `[수심] …` HUD 로그를 도배 → **타일 캐시 + `quiet` 인자**.
- 도움말 **구멍치기 토픽 4페이지**(조작·대상·저가 장비·안전) + EN 사전(잔존 0).
- ⚠ **하네스 함정 2건**(신규): 헤드리스는 rAF 간격이 300ms를 넘어 **실마우스로 "짧은 탭"을 만들 수 없다** ·
  **`Math.random` 전역 스텁은 Phaser를 깨뜨린다**(1인칭 씬 생성이 `reading 'context'`로 죽는다 — 제품 무관).
- 상세: `docs/wiki/03-WORKLOG/2026-09-17-149-hole-fishing-spot-gate.md`

**이전 변경 (2026-09-17 148차) — 인벤토리 윈도우드 스크롤 + 가방 사다리 2~5단계 실효화** (로드맵 다음 단계 · 사용자 선택 "윈도우드 스크롤" — 실렌더 27/27 · pageerror 0, 빌드 3/3·typecheck 0):

- ⚠ **가방은 이미 다 있었다** — 141차가 `bagSlots` 3·5·8·10·12를 실물로 발행했고 `gridCapacity()`가
  그 값을 소비하고 있었다. 막고 있던 것은 **`GRID_CAPACITY_MAX = 30` 한 줄** — 25+8·25+10·25+12가
  전부 30으로 클램프되어 **원정 가방·가이드 가방·항해 가방이 서로 같은 효과**였다.
  미구현이 아니라 **죽은 보상**이다(144차 `'run'`·147차 `AuctionEngine`에 이은 세 번째 같은 패턴).
- ⚠ **그 30도 밸런스가 아니라 렌더 한계였다** — 135차 주석이 그대로 적어 두었다:
  "인벤 패널 세로 여백의 실측 한계다(7행이면 하단 상태줄을 침범한다 — **더 늘리려면 패널 재설계가 먼저**)."
  그래서 148차의 실제 작업은 가방이 아니라 **`InventoryPanel` 재설계**다.
- **윈도우드 스크롤**(사용자 선택) — 보이는 행만 생성 + 휠(`containsPointer` 게이트) + 스크롤바(썸 드래그) +
  **`n–m / N행` 표기**(조용한 잘림 금지) + **드래그 중 가장자리 자동 스크롤**(8행짜리 그리드는 양 끝이
  한 화면에 없다). ⚠ **마스크는 쓰지 않는다** — Phaser는 마스크로 **입력을 클립하지 않아** 스크롤아웃된
  셀의 팬텀 히트가 남는다(54차 ShopPanel 전례 · `ui-panel` 스킬 규칙).
- 상한 30 → **37**(= 25 + 최대 `bagSlots` 12) → **25 / 28 / 30 / 33 / 35 / 37** 여섯 상태로 분화.
  뷰포트 416px에 **가방 없음 5행 · 착용 6행**(행 간격 7 → 4px). 그리드 하단 **515 ≤ 선택 요약 상단 524** 실측.
- **용량 밖 칸 = '잠긴 칸'**(어두운 바탕 + 주황 테두리) — **꺼내기는 되고 넣기는 거부**.
  조용히 안 그리면 사라진 것처럼 보이고, 넣게 두면 가방을 벗는 순간 또 갇힌다.
  이로써 135차 dev 경고가 가리키던 **시드 tackle 4건(slot 25~28)이 처음으로 화면에 나온다**.
- **칸 축소 가드 일반화** — 135차 `bagUnequipBlocked`(벗기)를 **`bagShrinkBlocked(bag, target)`** 으로
  확장해 **작은 가방으로 갈아타기**도 막는다(S5 §5가 "가방 2단계 이상이면 필요하다"고 예고한 항목).
- ⚠ **i18n 사전을 고쳤으면 dev 서버를 재시작하고 검증한다**(120차 함정 재현) — HMR `?t=` 분화로
  하네스가 게임과 다른 사전 인스턴스를 물어 **"전 항목 미번역"으로 보인다**. 재시작 후 잔존 0.
- 실측: 사다리 28/30/33/35/37(유니크 5) · 한 번에 하나만 착용(자동 스왑) · 확장 칸 점유 시 해제·교체 차단 ·
  스크롤 2행에서 좌상단 히트 = **slot 10** · 뷰포트 밖 클릭 = **−1**(팬텀 히트 0) · 패널 밖 텍스트 0 · EN 잔존 0.
- 상세: `docs/wiki/03-WORKLOG/2026-09-17-148-inventory-scroll-bag-ladder.md`

**이전 변경 (2026-09-17 147차) — 위판(경매 현장) + 어촌계 총회 연출 + 기한 초과 평판 감점** (로드맵 다음 단계 · 사용자 결정 5건 — core 31/31 · 총회 10/10 · 체인 10/10 · 영문 잔존 0 · pageerror 0, 빌드 3/3·typecheck 0):

- ⚠ **착수 조사에서 라벨과 실제가 어긋난 네 건**이 나왔고 그것이 범위를 정했다.
  **"통발 D2"는 이미 121차에 끝나 있었다**(S14 §4 D2a~D2d 전부 ✅ — PLAN의 `현재 지점`은 스테일 표기) ·
  **`AuctionEngine` 332줄이 사문**(클라 소비처 0 — 144차 `'run'`과 같은 패턴) ·
  M2-01 목표 라벨이 문자 그대로 `위판 UI — 준비 중` · **M1-04는 구멍치기 없이도 지금 통과**된다
  ("방파제에서"가 강제되지 않는다). 통발에서 막혀 있던 유일한 것 = **잡은 것을 팔 데가 없다**.
- ⚠ **사문의 기전은 "안 쓰임"이 아니라 쓸 수 없는 방향** — `AuctionEngine`은 플레이어를
  **중매인(사는 쪽)** 으로 모델링했다(`calcPlayerAuctionTotal` = 총 *비용*). 위판은 파는 쪽이라
  **구매자 측 계약을 한 줄도 고치지 않고** 개폐장 스케줄·`AuctionLot`만 공유하는
  `ConsignmentAuction.ts`를 신설했다(나중에 "경매장에서 사기"를 만들 때 쓸 자리로 보존).
- **위판 창구 = 건물 종류가 아니라 능력**(`ShopDef.auctionWindow`) — 속초는 직판장이 겸하고,
  전용 위판장 POI가 있는 지역이 생기면 **데이터만** 바꾼다(사용자 결정 — "지역에 따라 분리해서
  지정하기 어려운 경우도 있을 수 있으니"). 직판장 4탭 수용을 위해 탭 폭 자동 축소(120 → 103, 444 ≤ 446).
- ⚖ **평판은 수수료율에만** 건다(0 = 6% → 100 = 3%). 낙찰가에 걸면 시장 가격이라는 **자유 콘텐츠에
  스토리 진행도가 샌다**(S22 §13) — 실측으로 못박음: 평판 0↔80의 **낙찰 총액 105,730원 동일**,
  실수령만 99,386 → 101,924원. ⚖ **위판 자격은 `enforceRodSell` 토글과 무관하게 항상** 본다
  (위판장은 벌칙이 아니라 계원 창구). ⚖ `canConsign`에 **생산자 요건**(`caughtByPlayer`) 추가 —
  `canSell`은 구매품 되팔기를 허용하지만 위판에 열면 재정거래 구멍이 된다.
- **M1-11 총회 연출** — 좌석 24석 거수 표결. ⚖ **부결은 없다**(138차 §3 원칙): 찬성률만 평판을 따르고
  (평판 0 → 13/24 · 100 → 23/24) 과반 초과분이 신뢰 보너스가 되어 **곧장 위판 수수료로** 이어진다
  (평판 소비처 ①②가 여기서 만난다). **M2-01 `sell` 목표 `manual` → auto.**
- ⚖ **기한 초과는 재화가 아니라 평판 감점**(사용자 결정 · `missRepPerDay` 1/일) —
  M1-06~M1-11 구간은 현금이 품삯뿐(일당 상한 73,000원)이라 재화를 빼앗으면 회복 불가 상태가 된다.
  `TUNING.story.missCostKrw`는 하드코어 프리셋용으로 남기고 배선하지 않는다.
- ⚠ **합성 문자열이 사전을 비껴가는 131차 함정이 재발** — 붙이기 전 `t()` 번역으로 해소하되,
  단위어(`시간`·`분`·`건`)는 **전역 사전 오염**을 피해 로케일 분기로 처리했다.
- 실측: 경매 완주 낙찰 총액 194,568 − 수수료 11,674(6.0%) = **실수령 182,894원** ·
  유찰분 `returnedItemId` 회수 · 파장 시 출품 거부 · 계원 자격 전 위판 가능 0건 → 후 통발 2건만.
- 상세: `docs/wiki/03-WORKLOG/2026-09-17-147-consignment-auction-general-meeting.md`

**이전 변경 (2026-09-17 146차) — 유저 간 거래(공증) + 정보 보기 + 플리마켓 계약 + 팝업 중 동기화 결함 수정** (사용자 지시 3건 + 의견 요청 — 서버 13항목 · 2인 실렌더 거래 완주 · pageerror 0, 빌드 3/3·typecheck 0):

- **피어 우클릭(한 칸 안) → 정보 보기 / 거래하기.** 거래는 **서버 = 공증인**(인벤은 각자 로컬):
  제안 → 수락창 → 편집 → 양쪽 잠금 → 양쪽 확정 → `committed` 도장 → 각자 적용 → `applied`.
  **제안을 고치면 양쪽 잠금이 풀리고**, 도장 뒤엔 취소 불가, 적용 전 튕기면 **이어하기 때 다시 내려준다**(7일).
  예외(바쁨·거래 중·거리·이탈·시간 초과)는 전부 채팅 로그 문구(사용자 지시 "대상자가 바쁩니다").
- ⚖ **퀘스트 지원 우려는 구조적으로 작다** — 목표 24종이 전부 행위 이벤트라 인벤토리를 읽는 목표가 없다
  (`deliverFree`도 manual). 유일한 구멍 `earn`(재화 누적)은 **`addCoins(…, quiet)`** 로 막고, `bound`를
  **판매 금지 → 양도 금지**로 확장. 퀘스트 아이템 탭은 **`deliverFree` auto화 시점**에(백로그 R⑥).
- ⚖ **정보 보기 공개 = 레벨·장비·면허·최대어·출조 / 비공개 = 재화·인벤·퀘스트·생존·스킬·우호도**(서버에도 안 올라간다).
- ⚖ **경매·플리마켓 = 계약은 지금, 원장은 나중.** P2P 거래와 마켓이 같은 `MpTradeItem` 스냅샷을 쓴다.
  마켓은 등록 = 서버 소유 이전(에스크로). ⚠ 로컬 세이브가 수정 가능하므로 **서버가 아이템에 서명**하기 전엔 열지 않는다.
- ⚠ **팝업이 열리면 멀티 동기화가 통째로 멈추던 잠복 결함**(145차부터 — `uiBlocked` 가드 뒤에 있었다) 수정.
  ⚠ "보냈다" 플래그가 요청이 나는 사이 뒤집혀 프로필이 영영 안 올라가던 경쟁 → 실은 객체로 판정.
  ⚠ 받은 개체 id가 내 dev 시드와 겹쳐 한 스택으로 합쳐지던 것 → `_t<시각><순번>`.
- 상세: `docs/wiki/03-WORKLOG/2026-09-17-146-p2p-trade-profile.md`

**이전 변경 (2026-09-17 145차) — 멀티 세션 세계: 설치물 공유 + 공용 시드 + 외형/활동 + 피어 밀어내기 + 건물 반투명 + 지역 채팅 + 이어하기** (사용자 결정 7건 + 의견 요청 3건 — 2인 실렌더 6항목 · 영속 재시작 · pageerror 0, 빌드 3/3·typecheck 0):

- **통발은 놓는 순간 남에게도 보인다**(사용자 지시 — "서로 안보이면 같은 세션 내 타일, 자원을 같이 쓰는 중인데
  충돌이 생길 것 같음"). 서버가 설치 목록을 들고, **같은 칸 중복 설치 거절 · 남의 통발 회수 거절**.
  안에 든 것은 보내지 않는다 — 포획물·분실 롤·조례 적발은 놓은 사람 클라이언트의 몫이다.
- **공용 시드** `mpWorldSeed(세션시드, 종류, 키, 슬롯)` — 서버 왕복 없는 순수 함수.
  ⚠ **시드만 맞춰선 소용이 없었다** — 구 보일링은 발생 지점을 **플레이어 ±26타일**에서 골라
  기준점이 다르면 다른 자리가 나왔다 → **맵 격자 칸(24타일) × 45초 슬롯의 성질**로 재작성
  (양쪽 계산 완전 일치 실측). 과증식은 표류를 **벽시계**로 옮기고 배치 시각부터 되감아 재생 ·
  홈타운 날씨 1시간 슬롯 · 교통은 **초기 편성만**(걸음이 프레임 dt라 위치는 갈린다 — 배경에 대역폭을 쓰지 않는다).
- ⚖ **퀘스트 게이트 규칙**(사용자 지시): 진행도가 조건에 끼어드는 무작위는 **`MpSharedRngKind`(닫힌 유니온)에 넣지 않는다.**
  아직 그 단계가 아닌 사람 화면에 뜨거나, 같은 단계라도 한 사람이 먼저 가져가면 다른 사람의 목표가 사라진다.
- **외형 동기화** — 서버가 `CharConfig`를 보관한다(구 `characterOf('mp_'+id)`는 모두에게 일관됐을 뿐
  **본인이 만든 얼굴과는 무관**했다). **활동 배지 4종**(낚시/거래/실내/자리비움) + 필드 밖은 알파 0.55 + 밀어내기 제외.
- ⚖ **피어 밀어내기는 내 바디만 민다** — Y-정렬은 이미 있었고(플레이어·피어·건물·NPC 전부 `20 + y*0.001`),
  남은 건 밀어내기뿐이었다. **피어에게 물리 바디를 달지 않는다**(1초 폴링 스냅샷에 끼인다) ·
  막힌 칸 방향 금지 · **완전 겹침(0벡터)이면 `playerId` 비교로 반대 방향**(실측으로 잡음).
- ⚖ **건물 반투명은 트리거 대신 기하 판정** — POI 187곳에 트리거를 다는 대신 프리팹 배치에 이미 쓰는
  사각형에 발끝을 넣어 본다(새 물리 바디 0개). 알파 1 → **0.38** · **0.2초** `Sine.easeOut` · 6px 이력.
- **지역 채널 실채팅**(presence 편승 — 왕복 안 늘림). ⚠ 입력 중에는 **Phaser 키보드를 통째로 끈다** —
  `stopPropagation`으로는 `keydown-B`가 안 막혀 **채팅에 `b`를 치면 쿨러가 열렸다**(실측).
- **이어하기** — 서버가 이름이 아니라 **`userId`로 사람을 알아보고** 세션을 디스크(`.mp-sessions/`)에 남긴다.
  프로세스를 완전히 죽였다 켜도 **시드·마지막 자리·통발이 복원**된다(실측). ⚖ 배포 모델은 **호스트-클라이언트 P2P** —
  서버를 없애는 게 아니라 호스트로 옮기는 것이라 코드 경로는 그대로다. 저장 소유권 = **호스트는 세계 / 각자는 캐릭터**.
- ⚠ 이름은 접속을 끊어도 **7일간 잠긴다**(자리를 남기므로) · **`userId`는 localStorage**라 브라우저를 바꾸면 다른 사람이 된다.
- 상세: `docs/wiki/03-WORKLOG/2026-09-17-145-mp-shared-world.md` · 시스템 페이지 S24 갱신

**이전 변경 (2026-09-17 144차) — 달리기(Shift) + 캐릭터 만들기 프리뷰 회전·동작 버튼** (사용자 지시 2건 — 실렌더 전 항목 PASS · pageerror 0, 빌드 3/3·typecheck 0):

- **달리기는 이미 core에 있었다** — `VitalsActivity`의 `'run'`은 125차부터 있었고 `drainOf()`가
  `TUNING.vitals.drainRun`을 반환했는데, **어떤 호출측도 `'run'`을 넘긴 적이 없어** 사문이었다
  (`tickVitals`가 `moving ? (isMounted ? 'bike' : 'walk') : 'idle'` 3분기). → 새 활동 종류를 만들지 않고
  **비어 있던 분기를 채웠다**(소비처가 없던 값이라 `drainRun` 재조율도 세이브·밸런스 회귀가 없다).
- **Shift 홀드 + 방향키 = ×1.55**(심리스 210 → **326 px/s**). 위계는 **걷기 210 < 달리기 326 < 자전거 420**으로
  고정 — 달리기가 자전거를 넘으면 `drv_bike` 스킬 계통과 자전거 아이템이 죽는다.
  `drainRun` [16,26,32] → **[22,34,44]**(피로 100%까지 걷기 8.3h ↔ **달리기 2.3h** ↔ 자전거 5.0h).
- ⚖ **소모값만으로 벌하지 않고 '못 뛰는 상태'를 만든다** — 피로 **85% 초과** 또는 허기·수분 임계 미만이면
  숨이 차서 걷기로 강등 + 12초 스로틀 경고. 짧은 스프린트는 피로가 거의 안 들어 소모값만으로는
  사실상 무료가 되기 때문이다(132차 슬랙 체급 게이트·133차 `safeFloorFrac`과 같은 원리).
- `CharacterSprite.update(dt, moving, **paceMult**)` 신설 — 달리기는 **같은 걷기 4프레임을 1.55배로** 돌린다.
  ⚠ **전용 달리기 포즈는 없다**(아트 후속 — 백로그 P①).
- **캐릭터 만들기 프리뷰 재구성** — 미니어처 4종(정면·좌·우·후면) 폐기 → 프리뷰 아래 **◀ 방향명 ▶ 회전 행**
  + **정지/걷기/달리기 동작 행**(하단 인셋 104 → **146**으로 확장해 미니어처 자리를 흡수).
  **자동 회전(2.4초)도 폐기** — 방향은 버튼이 전담한다. ⚠ '기타 행동'은 없다(시트 모션이 대기·걷기뿐).
- 검증: 속도 ×**1.550** · 드레인 run/walk ×**3.52**(이론 3.67) · 피로 90%에서 차단(×1.000·`running=false`) ·
  회전 4방위 · 걷기/달리기 프레임 16회 ↔ 11회 · 레이아웃 500~587 ≤ 인셋 616 ≤ 패널 628 · 영문 사전 7/7.
- ⚠ **`handleMovement`가 `tickVitals`보다 먼저** 호출되어야 활동 종류가 같은 프레임 이동과 맞는다
  (`update()` 순서 `handleMovement → syncPeers → tickVitals` 유지).
- 상세: `docs/wiki/03-WORKLOG/2026-09-17-144-run-sprint-charcreate-preview.md`

**이전 변경 (2026-09-16 143차) — 싱글/멀티 진입 플로우 + 대화창·팝업 통일 + 필드 라벨 정리 + 문구 다듬기** (사용자 지시 4건 + 후속 1건 — 실렌더 2인 교차 확인 · pageerror 0, 빌드 3/3·typecheck 0):

- **멀티플레이 신설(S24)** — core `types/Multiplayer.ts`(**클라이언트·서버 공용 계약**) · 서버 `/mp/*` REST +
  인메모리 `SessionRegistry` · 클라 `net/MultiplayerClient.ts`(1초 폴링) · `MultiplayerLobbyScene`.
  순서는 사용자 지정 그대로 **게임 시작 → 혼자 하기/여럿이 하기 → 로비(방 열기·코드 참가) → 슬롯 3칸 → 캐릭터 만들기**.
  `[이대로 생성하기]`가 **중복 여부 확인중 → 통과 → 생성 중 로딩 → 접속중**을 띄우고, 겹치면 **빨간 글씨**로 중단.
  같은 지역이면 서로의 스프라이트 + 머리 위 이름표(실측 A→`돌섬` / B→`푸른바다` 교차 확인).
- ⚠ **멀티에서 어긋나는 것들은 워크로그 143 §8에 전부 적었다** — 요지: 공유되는 것은 **지도·시간·서로의 위치/이름뿐**이고
  퀘스트·우호도·인벤·설치물·어획은 각자 로컬이다. 보일링·과증식·차량은 클라이언트마다 다른 난수로 돈다
  (채집 스팟만 `forageSeed(mapKey, Date.now())`로 이미 결정적).
- **대화창** 초상 열이 박스 **상단에만 매달려** 아래 70px가 늘 비어 있었다(실측 바 y=170·라벨 184 vs 박스 260)
  → 스택 높이를 재고 세로 중앙 정렬. **`DraggablePanel` 프레임을 `paintHudPanel`로** 교체 —
  139차 "게임의 모든 창은 `paintHudPanel` 문법"을 **팝업 베이스만 비껴가 있었다**(전 팝업 동시 적용).
- **`ui/TextQuality.ts` 신설** — Text 내부 래스터 2배 + **텍스트 텍스처만 LINEAR**(스프라이트·타일은 NEAREST 유지).
  ⚠ 상한은 **프레임버퍼 1280x720** — 창이 그보다 크면 캔버스 전체가 CSS 확대다(31차).
- **필드 바닥 이름표·점 기본 끔** — 설정 **'화면' 탭** 토글(기본 끔). 속초 실측 거래처 **187** · 이름만 있는 곳 **121**이라
  글자가 타일을 덮었다. 대신 미니맵이 인물(`mm_npc`)·장소(`mm_poi`)·상점·퀘스트 `?`/`!`를 떠맡는다
  (표시 하한 150 = 의뢰·인물 / 250 = 물품 상점 / 350 = 전부). 픽셀 아이콘 42 → **44종**.
- **AI 메모성 문구 제거** — 캐릭터 만들기 우상단 배너 삭제 · 부제 **'당신은 어떤 사람인가요?'** ·
  `(추후 구현)`류 괄호 8곳 · **UI 이모지 8곳**(§8-8 위반분) · 메뉴 한국어화 · 표준 퀘스트 선택지 7편 어투 교정
  (경구체 → 생활 말투. `outcome`은 한 줄도 안 바꿨다 — 무결성 0/0).
- 상세: `docs/wiki/03-WORKLOG/2026-09-16-143-multiplayer-ui-tone-labels.md` · 시스템 페이지 **S24 `multiplayer.md` 신설**

**이전 변경 (2026-09-16 142차) — 캐릭터 룩 시안 정합(머리카락·눈·팔·아웃라인) + NPC 이름표 머리 위 배치** (사용자 지시 2건 — 실렌더 간격 7/7 · pageerror 0, 빌드 3/3·typecheck 0):

- **시트 임포트는 택하지 않았다** — 생성 시트 실측이 열 3.05배 ↔ 행 3.5배로 격자가 어긋나고, 프레임 일관성이 없으며,
  무엇보다 구운 그림에는 **옷을 입힐 수 없고**(페이퍼돌 상실) NPC 41인 결정적 분화가 불가능하다.
  → 시안이 시안처럼 보이는 네 가지를 **`CharacterArt.ts`의 그리기 규칙으로** 옮겼다(계약·앵커는 불변).
- **머리카락이 이마를 덮는다**(`HAIR_CAP_ROWS 4` · 옆머리 y0+5) + 정수리 볼륨 행 + 2단 인셋 = **둥근 실루엣**.
  모자 왕관도 같은 인셋을 따른다(안 그러면 머리보다 넓은 막대가 떠 보인다).
- **눈 = 위 행 동공(검은 바) / 아래 행 홍채**(`EYE_ROW 5` · 2px × 2행). **눈썹 행 폐기** —
  앞머리에 가리는 것이 정상이고, 이마가 드러나는 `buzz`·`topknot`에서만 그린다. 입·수염도 축소(수염 = 턱선 한 행).
- **부위색 아웃라인**(`OUTLINE_MIX 0.68` — 이웃 4픽셀 평균과 혼합) · 팔 3px → 2px(어깨 14) + `armSeam` 1px 그늘.
- **이름표는 셀 상단 기준**(`charTopFromFeet` = −58 · 여백 6) — 구 `y−40`은 머리(y−52~y−34)를 12px 파고들었다.
  ⚠ 머리 꼭대기가 아니라 **셀 상단**을 쓰는 이유 = 비니 방울·챙·곱슬이 머리보다 3행 위로 나온다.
  플레이어 [F] 힌트·오브젝트 힌트·토스트도 같은 기준으로 정렬.
- ⚠ 아웃라인 색이 바뀌어 **모든 캐릭터 텍스처가 새로 구워진다**(런타임 생성 — 세이브 무관).
  **캐스트 해시 규칙은 불변**이라 기존 세이브의 "누가 누구인지"는 그대로다.
- 상세: `docs/wiki/03-WORKLOG/2026-09-16-142-char-look-silhouette-name-label.md`

**이전 변경 (2026-09-16 141차) — 퀘스트 나레이션 186편 + 메인 해금형 보상(귀속·기술·상점) + 선택지 비공개 공개 + 발주 정책(once/event)** (사용자 지시 4건 + 추가 3건 — 무결성 0/0 · 하네스 6항목 · pageerror 0, 빌드 3/3·typecheck 0 · **main 푸시(사용자 허가)**):

- **나레이션 층 `StoryNarrative.ts`**(186/186) — 주인공 1인칭 서두(육하원칙·정의문 금지) · NPC 발주/진행/완료 대사 · 메인 목표 문장 라벨 ·
  **내용 기반 완료 선택지 2~3개**(응답 문장이 보상을 드러낸다) · 에필로그. 우선순위 나레이션 > 정의 > 표준. 구 손글 13편 이관.
- **메인 = 해금형 보상 / 서브 = 진행 보조**(검증기가 겹침 거부) — `rewards.items[].bound`(귀속 장비 20 · 판매 제외) · `skillUnlocks` 7
  (스킬 `unlock: { kind: 'quest' }`) · `shopUnlocks` 5(`ShopEntry.unlockKey` — 플래그 전엔 목록에 없음) · 재화 · 면허. XP 계약 §8-1 불변.
- **보상은 고른 뒤에만** — 대화창 힌트 제거 · 응답 화면 = NPC 응답 → 받은 것 → 이 답으로 → 다른 답 `???` → 에필로그 · 일지도 완료 후 공개.
- **발주 정책** — `once`(거절 = 영구 소멸) 5편 · `event`(계절·레벨 구간 · 거절 시 쿨다운) 9편 · 세이브 `declined`·`offerCooldown` · 일지 `거절함`.
- 아티팩트 v5(같은 URL — 나레이션·발주/완료 선택지 전문·해금 표·정책 표) · **캐릭터 시안(스타듀 계열 시트) 기본 룩 채택**(구현은 후속 — S23 §7).
- 상세: `docs/wiki/03-WORKLOG/2026-09-16-141-narrative-unlock-rewards-offer-policy.md`

**이전 변경 (2026-09-16 140차) — 얼굴형·머리 축소·측면 프로파일 + NPC 대화창(선택지 분기) + 우호도 + 숙련도 + 아티팩트 v4** (사용자 지시 6건 + 질문 1 — 실렌더 pageerror 0, 빌드 3/3·typecheck 0 · **main 병합·푸시(사용자 허가)**):

- **머리 9행×10px**(구 10×12 — 남성 몸통 10px보다 넓어 버섯 실루엣) · `FaceShape` 3종(계란/둥근/사각 — 턱선 깎기 `L_headShape`) ·
  측면 다리 **행별 기울기**(구: 사각형 전체 이동 + 골반 대비 1px 뒤 → "뒤로 휨") · 코 1px·인중 들임·뒤통수 채움 · 눈 2행.
- **대화창 재작성** — 하단 박스 + 딤 + 초상(실시트 ×5) + 우호도 게이지 + 선택지(↑↓ Enter). **모든 퀘가 발주 톤 + 완료 답을 고른다**:
  서브 표준 4종(품삯 / 가르침 = 숙련도 XP / 사양 = 우호도·항구 신뢰 / 친밀 전용 청탁 = SP+1) + 손글 13편.
  **메인은 XP 배율·아이템·SP 금지**(`validateStoryChoices`) — 톤·우호도 ±0.1·소액·평판만.
- **우호도 −1~+1 5티어**(`rules/Affinity.ts`) — 서브 보상 ×0.7~1.3 · **−0.4 미만 서브 발주 거절** · 품삯 ×(1+0.15a) · 메인 재화 1±0.1.
  자유 콘텐츠(상점·낚시·이동)에는 걸지 않는다(**자유 플레이 보장 규칙** — 워크로그 140 §4).
- **숙련도** — 기술형 22스킬은 배운 뒤 **행위로 채워야 효과**(임계 15/45/100/200/360 · 배율 0/.35/.6/.85/1/**1.15**). 훅 9종.
  ⚠ 구세이브의 해당 스킬은 효과 0으로 시작한다(설계).
- `.agents/CHARACTER_SPRITE_PROMPT_SPEC.md`(GPT Image 프롬프트 스펙) · 아티팩트 v4 같은 URL 재발행.
- 상세: `docs/wiki/03-WORKLOG/2026-09-16-140-face-shape-dialogue-choices-affinity-proficiency.md`

**이전 변경 (2026-09-16 139차) — 캐릭터 만들기 UI 재정비 + 스토리 아키텍처 v4** (사용자 지시 2건 — 실렌더 pageerror 0, 빌드 3/3·typecheck 0):

- **생성 씬은 138차에 기능만 세운 초안이었다** — 패널이 평면 `fillRect`(게임의 모든 창은 `paintHudPanel` 문법) ·
  **색을 `3 / 10` 숫자로 골랐고**(픽셀 팔레트에는 이름이 없어 무엇을 고르는지 알 수 없다) ·
  12행이 한 줄로 이어져 그룹 경계가 없고 · 748x560 패널의 우측 하단이 비어 있었다.
- **HUD 패널 3장 + 2열 5섹션**(체형 / 얼굴 / 머리 / 복장 / 이름) · **실제 색 견본 스트립**(직접 클릭 ·
  수염은 앞에 '없음' 사선 칸) · 선택 강조는 **색 + 좌측 금색 바**(메인 메뉴 관례 — 폰트 교체는 레이아웃을 흔든다) ·
  미리보기 스테이지(바닥 그림자·발판·배율 8 -> 10·`이름 · 성별` 캡션) · 안내 홈 · 푸터 `paintHudSlot` 버튼.
  방향 회전이 매번 전체 리프레시를 돌리던 것 -> `setFrame`만 교체.
- **`.agents/STORY_SPEC_v4.md` 신설(481줄) = 스토리 계통 정본.** v3의 기형(§0.5 코드 정합 노트가
  본문보다 우선)을 **본문 승격**으로 해소하고 134~138차 구현을 반영했다 — 퀘 **186**(메인 68·서브 118) ·
  아크 **23** · XP **2,809,150** · 목표 종류 24 · 학습 태그 66 · 법 강제 3단계 · 회귀 고리 · 생활 계통 ·
  캐릭터 계약 · 구현 현황표(✅🔶⬜) · 부록 A(v3 -> v4 변경 10건). **수치는 전부 `dist`에서 산출**(추정 없음).
  v3는 **원안 산문 보관본**으로 남기고 헤더에 안내 배너를 달았다. 충돌 규칙 = **코드 > v4 > S22 > v3**.
- ⚠ **캐릭터 아트(머리 크기·눈·입)는 이번 차수 대상이 아니다** — 사용자 검토 중이라 명시적으로 보류했다.
- ⚠ 앞으로 **퀘 수·XP·id를 바꾸면 v4 §2·§3 표도 같이 고친다**(코드가 정답 — 표를 고친다).
- 검증: 우열 우단 **1226 ≤ 패널 안쪽 1228** · 안내 홈 506~612 · 버튼 654~690 ≤ 푸터 704 ·
  견본 클릭(머리 색 6번째) 반영 · 무결성 0건 · 문서 200자 초과 줄 0.
- 상세: `docs/wiki/03-WORKLOG/2026-09-16-139-charcreate-ui-story-v4.md`

**이전 변경 (2026-09-16 138차) — 바닐라 베이스 캐릭터 + 페이퍼돌 장비 + NPC 41인 분화 + 과증식 해양생물 4종 + 퀘스트 28편** (사용자 지시 6건 — 무결성 0건 · 실렌더 pageerror 0, 빌드 3/3·typecheck 0):

- **구 캐릭터의 정체**: 외부 고해상 출력물을 런타임 축소해 쓰고 있었다(`man-idle-front` 260x467 · **26,817색** → 52px = **x0.111**).
  실측 결함 3건 — ① **idle/walk 실효 해상도가 2.3배 다르다**(x0.111 vs x0.261: 걷기 시작하면 또렷해지고 폭이 29 → 32px 널뜀)
  ② 화면 그레인 3종 혼재(지면 2px / gem NPC 1px / 캐릭터 무그레인) ③ **장비 착용을 원리적으로 표현할 수 없다**
  (구워진 사진에 모자를 씌울 수 없다 — 72차 장비창을 만들고도 캐릭터는 아무것도 안 입었다)
  ④ 스토리 인물 38인을 **NPC 텍스처 5장으로 돌려막음**(탁만수 = 강두철 = `npc_grandfather`).
- **core `art/CharacterArt.ts` 신설 — 아트 격자를 코드가 소유한다.** 셀 **32x32 아트 px**(몸 26행) · **정수 x2** = 몸 52px
  (구 `PLAYER_DISPLAY_H`와 동일해 **레이아웃 1px도 안 바뀐다**) · 그레인 2px = Kenney 지면과 동일 격자.
  레이어 14종이 `pose()`의 앵커 사각형만 참조 — **신규 장비 = 그리기 함수 1개**.
- **바닐라 = 나체 + 언더웨어**(여성은 상의 속옷 포함). 상의·하의·신발은 **전부 장착 아이템**이고 생성 직후 스타터 한 벌이 입혀진다.
- 걷기 **4프레임 접지/통과** + 대기 1 = 방향당 5프레임. ⚠ **접지 2프레임은 좌우 체중 이동(sway)으로 구분**한다 — 안 그러면 같은 그림이 된다.
  얼굴 = 2x3 눈 + 하이라이트 · 눈썹 · 입 4종 · 볼 홍조, **여성 속눈썹 1px**(같은 도트 예산에서 성별이 가장 크게 읽히는 신호).
- **`art/CharacterCast.ts`** — 인물 41인의 성별·연령·역할만 표로 두고 나머지는 **id 해시로 결정적 분화**(고유 41 / 중복 0).
  POI 배경 NPC도 역할 기반 고유 인물로 교체. 구 gem NPC 5종은 **배선만 제외**(파일 보존).
- **`CharacterCreateScene` 신설** — 새 게임 시 성별·피부·머리 모양/색·눈동자·입·눈썹·수염·홍조·기본 상하의 색·이름(12행).
  미리보기는 **실제 게임 시트를 그대로** 쓴다(별도 프리뷰 렌더러 금지 — "여기선 예쁜데 인게임은 다른" 상태 방지).
- **과증식 해양생물 4종**(`MarineNuisanceDatabase` — 보름달물해파리·노무라입깃해파리·별불가사리·아무르불가사리):
  수면 아래를 바다색으로 눌러 **빙산의 일각**처럼 걸치고, 해파리는 **해안선 6칸 밖**에서 5분에 한 칸 표류한다.
  수거는 **씬 전환 없이 훌치기** — 착수 지점에 걸어 릴링으로 끌어오고 발 앞 한 칸에서 끝난다(무게가 클수록 느리다).
  해안에 밀려온 불가사리는 [F] 채집. 어종 DB와 분리한 이유 = 낚시 스폰 후보가 아니고, 가치가 **구제 수매가**이며, 시간 축이 과증식이라서.
- **퀘스트 158 → 186** (신규 아크 4 — N20 유하랑 버튜버 · N21 오세찬 과증식 · N22 하늬 생활 · N23 채수림 어종 관리).
  신규 목표 `cull`(자동 추적) · `farm`·`mine`·`furnish`(manual). ⚠ **manual 목표는 챕터당 2편까지** — 쌓이면 "대화만 하는 퀘스트"가 된다.
  누적 XP 2,415,600 → **2,809,150**(기존 보상은 한 건도 줄이지 않았다).
- **만렙 200에서도 XP가 쌓인다** — 레벨만 200에서 멈추고 경험치는 누적(퀘스트가 계속 느는데 만렙에서 보상이 증발하면 후반이 무보상이 된다).
- 건물 프리팹 11장이 도시 전체를 덮던 문제 → **POI별 벽 색·차양·간판 변형**(`BuildingVariant` — 2px 그레인 유지).
- ⚠ **비정수 배율 금지**: 배치는 항상 `setScale(정수)`. `setDisplaySize`로 맞추면 그레인이 무너진다(구 캐릭터 실패의 원인).
- ⚠ **캐스트 해시 규칙을 바꾸면 기존 세이브의 NPC 얼굴이 전부 바뀐다**(외형 미저장 · id로 재생성).
- 어종 실사는 **픽셀화 예외로 유지**(사용자 결정 — 게임성 우선).
- 상세: `docs/wiki/03-WORKLOG/2026-09-16-138-character-base-nuisance-quests.md` · 시스템 페이지 **S23 `character-art.md` 신설** · S22 갱신.

**이전 변경 (2026-09-15 137차) — 퀘스트 증설 38편(회귀 고리·인물 연결) + 지역/인물 색인 + 일지 서브 트랙 스크롤** (사용자 지시 3건 — 무결성 0건 · 실렌더 PASS · pageerror 0, 빌드 3/3·typecheck 0):

- **맹점의 정체**: 챕터 = 지역 1:1이라 **다음 지역이 열리면 이전 맵을 다시 갈 이유가 없었다**(사용자 지적).
  실측 — 120퀘 중 챕터 무대 밖 편은 20, 그중 2~5챕터의 속초 복귀는 **0편**. Ch1에서 세운 인물 7명이 Lv20 이후 사라졌다.
- **증설 38편** — 메인 3(**M2-11 되돌아온 상자**: 부산 상자 규격을 속초 좌판으로 / **M4-11 속초의 손님**:
  낚시어선업 배로 강두철·탁만수 승선 / **M6-09 고래마루의 마지막 손님**: Ch3 배가 팔린다) + 서브 35(챕터당 5)
  → **메인 68 · 서브 90 = 158퀘** · 누적 XP 1,972,850 → **2,415,600**.
  ⚠ 메인은 `mainRows`가 배열 순서로 체이닝하므로 **각 챕터의 마지막(해금) 퀘스트 앞**에 넣는다.
- **회귀 고리 13편** — 새 자격·새 배를 들고 옛 항구로. 챕터 무대 밖 편 20 → **33**.
  특히 **N19가 매 챕터 속초로 불러들인다**(Ch1·2·3·4·6·7).
- **신규 아크 2종** — **N18 도현수**(라이벌, 7챕터 각 1편 · 메인 전용에서 아크로 격상) ·
  **N19 정옥선·탁만수**(사용자 요청 "npc간 서로 좋아하는 설정" — 끝내 말하지 못하고 **심부름꾼은 늘 플레이어**).
  그 밖에 강두철↔고만석(서른 해 전 같은 배) · 배누리 진로 · 이수연 식당.
- **일지 서브 트랙 = 윈도우드 렌더 + 휠 스크롤** — Ch5가 12아크 15편으로 늘어 필요 573px > 예산 470px.
  간격 압축(136차)으로는 못 담는다. 아래로 삐져나가는 행은 그리지 않는다.
- **아티팩트** — "인물 라이브러리를 확인할 수 없다"의 정체는 **페이지 맨 아래 + 길잡이 없음**이었다(섹션은 있었다).
  **섹션 네비 4칸** + **지역별 퀘스트 11카드**(회귀 배지 33) + **인물 다중 아크**(탁만수 = N02·N19) + 하드코딩 수치 제거.
- ⚠ **id 번호는 발행 순서, 배열은 챕터 순서** — 기존 id를 다시 매기면 세이브(`story.done`)가 진행을 잃는다.
- ⚠ 실렌더 함정: 패널 하단 측정 시 **origin 0.5 베이스 플레이트**를 같이 재면 항상 `panelH × 1.5`(984)가 나온다.
- 상세: `docs/wiki/03-WORKLOG/2026-09-15-137-quest-expansion-return-loops.md` · 시스템 페이지 S22 갱신.

**이전 변경 (2026-09-15 136차) — 스풀·베일(전유동·흘림) + 밑걸림 대처 + 장비 고장 + 일지 2트랙 + NPC 퀘스트 마커** (사용자 지시 6건 — 실렌더 전 항목 PASS·pageerror 0, 빌드 3/3·typecheck 0):

- **스풀·베일**(core `SpoolSystem.ts`): 채비에 **원줄 길이 `lineOutM`** 상태를 신설. 풀려난 줄보다 채비가
  멀어질 수 없고, 팽팽해지면 반지름 `lineOutM` 구면에 갇혀 **호를 그리며 끌려온다**(진자).
  **R 홀드 = 베일 개방**이면 줄이 계속 나가 채비가 조류를 타고 흘러간다 — 전유동·흘림이 비로소
  반유동과 **다른 조법**이 됐다. 실측: 닫힘 60초 거리 20 → **9.56m** ↔ 개방 60초 **20m 유지 · 원줄 34.7m · 수심 12m 바닥 도달**.
  ⚠ 자유 방출에는 **페더링 상한**(`maxSlackM 4`)이 필요하다 — 없으면 조류가 끌어가는 것보다 빨리 내줘
  슬랙이 무한 누적된다(개방 30초 37m 실측). 넘으면 방출이 막히고 **백래시(원줄 꼬임) 신호**가 뜬다.
- **파이팅 줄 주기**: R = `FightInput.spoolOpen` → 요구 장력 **×0.18**(텐션 54 → **5.2**) · 거리 19.7 → **27.2m**(도주).
  **바늘털이(jump) 정대응 / 여 박기(dive) 오대응**으로 판정한다. 대가는 거리 손실 + 제압도 정체 + **슬랙 바늘빠짐 위험**.
- **밑걸림 대처 선택창**(사용자 확률표): 끌어당기기 **10 / 10 / 30 / 50**(완전 회수 → 미끼 → 바늘+미끼 → 전량) ↔
  뒤로 당겨 끊기 **50:50**(도래 위 생존 / 전량). ⚠ **절지 파단은 라인이 로드 등급의 1.5배를 넘을 때만** 굴린다 —
  모든 밑걸림에 15%를 걸면 초반 장비가 몇 번 만에 폐기된다(무작위 징벌 → 채비 매칭 학습으로 전환).
- **장비 고장 8종 + 내구도**(core `GearDurability.ts`): 초릿대·절지 파단·원줄 꼬임·베일 휨·핸들·
  찌 부력 변성/깨짐·루어 파손. 확률은 **저가 장비 기준**이고 `gearQualityMult`(지수 −0.35)로 고급일수록 준다.
  내구도는 "못 쓰게 되는 시계"가 아니라 **고장이 잦아지는 계수**(사이소 200회 = 1.0 → 3.5).
  수리점 탭(직판장·생활용품점) · 자가 수리 · **슬롯 우하단 경고 배지** · 사용불가 고장은 손에서 자동 해제 + 캐스팅 차단.
- **일지(J) 전면 재작성**: 120퀘 단일 트리 폐기 → **레일(챕터 7 진행 막대) / 본문(메인 스파인 · 서브 아크) 2트랙**
  + 퀘스트 상세 3단 항법. **경험치는 목록에서 빼고 상세 '보상'에만**(사용자 지시). 사람들(17아크) 색인 신설.
- **NPC 퀘스트 마커**: 미니맵 상점 카테고리 8종 + **빨간 느낌표(새 의뢰) / 노란 물음표(지금 해결 가능)**,
  탑다운 필드는 NPC 몸통 우상단(좁으면 좌상단). 메인·서브는 구분하지 않는다(사용자 결정).
  픽셀 아이콘 29 → **42종**(생성기 가변 크기 지원).
- ⚠ **스풀 구속은 모든 조법의 채비 위치에 개입한다** — 착수 시 원줄 = `hypot(castDist, 0.3) + 0.6m`라
  장거리는 자연히 여유가 생기고(20m 착수 → 5m 침강 시 필요 20.62 vs 보유 20.60) 단거리·심수에서만 세게 걸린다.
- **136-b 후속** (사용자 지시 2건 — 도움말 ko/en 실렌더 PASS·한국어 잔존 0·pageerror 0):
  ① **줄 주기가 곧 바늘빠짐이던 문제** — R로 주면 텐션이 5.2까지 떨어져 임계 6 아래라
  **바늘털이 정대응이 1.5초 뒤 확정 바늘빠짐**이었다. `slackOpenGraceMult 0.4`로 유예(→ **3.75초**) +
  **`FightStatus.slackRisk` 경고 바**(텐션바 아래 · `줄이 느슨합니다 — 바늘 빠짐까지 N초`).
  ② 수심 패널 스풀 행에 **나간 줄 길이 상시 표기**(`줄 24.7m · 여유 3.9m` / `· 방출` / `· 팽팽` / `· 백래시!`).
  ③ 밑걸림 선택창에 **확률표 노출**(감추면 "왜 잃었는지 모르는" 선택이 된다).
  ④ 도움말 **스풀 · 밑걸림 · 장비 고장 3토픽**(ko/en · 실캡처 4장 × 2 = 8장) + 영문 사전 `i18n/en_gear.ts`.
  ⑤ `TUNING.spool` 9키 **확정값 승격**. ⚠ `annotate_help_images.py` — `hangul_ok()`가 Pillow 11에서
  항상 False였고(`tobytes()` 제거), 한글 글리프를 가진 **wqy-zenhei**를 폰트 후보에 넣어 131차 잔여 해소.
- 상세: `docs/wiki/03-WORKLOG/2026-09-15-136-spool-snag-durability.md` · 시스템 페이지 S3·S12·S22 갱신.

**이전 변경 (2026-09-15 135차) — 미완성 접시 저장 + Ch1 실시스템(품삯·사이소·가방) + 법 강제 3단계** (사용자 지시 3건 — 실렌더 22/22 + 35/35 · pageerror 0, 빌드 3/3·typecheck 0):

- **미완성 접시**: 플레이팅 도중 접시를 내리면 **미완성 접시 아이템**(`plateWip`)으로 보관 —
  상세보기 `플레이팅 진행 6 / 16점 (38%)` · 슬롯 **좌하단 % 배지** · 도마에 다시 올리면 방위·회전까지 복원 ·
  우클릭 **[해체하기]**(접시 + 조각 전량 반환). **판매 0원**(사용자 결정 — 가격표가 만석을 전제한다),
  섭취는 가능하고 이때 **식기는 회수**한다. 신선도는 **가장 오래된 조각 계승**(`msToSpoiled` 기준 — 라벨 서열 아님).
- ⚠ **점검에서 드러난 실버그 6건**(전부 수정): ① `finalizePlate`가 완성 접시를 **항상 `'fresh'`** 로 굳혀
  판매 배율 0.10인 '나쁨' 조각 16점을 **정가 접시로 세탁**할 수 있었다 ② 미완성 접시만 보유하면
  도마 사시미 영역이 안 열려 **이어담기가 막다른 길**이었다(`hasPlate`가 빈 접시만 셌다) ③ **부패 조각**도 배치됐다
  ④ HUD D-day가 기한 초과 시 **`D--3`**(음수 문자열 결합) ⑤ 미완성 접시 섭취 시 **접시까지 소멸**
  ⑥ 진행률 배지가 이름 라벨을 **3px 침범**(실측).
- **Ch1 manual 목표 실시스템화**: core **`DayJobs.ts`**(품삯 3종 — 얼음 12,000×2 · 그물 8,000×3 ·
  공동작업 25,000×1, **행동력 소모 + 하루 상한**) · `StoryStore.work()` + 세이브 `jobs` ·
  대화창 **일감 스트립** · **생활용품점 사이소**(`daily` — 저가 장비 한 벌, 속초 POI 17곳) ·
  **가방 사다리 1단계**(도면 `bp_backpack_rough` → `bagSlots: 5` → 인벤 **25 → 30칸**, 등 슬롯, 확장 칸 점유 시 해제 차단) →
  M1-02·04·08·10 목표를 `manual` → **`auto`** 로 교체.
- **법 강제 3단계**(`TUNING.law.enforceRodSell`): **0 = 끔 / 1 = 항상 / 2 = M1-06 완료 후**(기본 **2**).
  규칙을 가르치는 퀘스트 전에 벌하지 않고, 스토리를 진행하지 않은 구세이브는 **회귀 0**.
  ⚠ M1-06을 끝내면 어획물 판매가 전면 막힌다(낚싯대 영구 · 통발도 비어업인은 자가소비) — `reported_fishery`는
  **M1-11 총회**에서 나오므로 그 사이 현금은 품삯뿐이다(일당 상한 **73,000원** · 피로 54 < 상한 88).
- ⚠ **시드가 기본 25칸을 넘는다**(실측 — `tackle` 29개): slot 25~28이 그리드에 안 그려진다.
  기능은 살아 있고(채비 선택창·상점은 slot을 안 본다) 가방을 쓰면 6행이 열려 보인다 → `createSeedItems`에 dev 경고.
- 상세: `docs/wiki/03-WORKLOG/2026-09-15-135-wip-plate-dayjobs-law.md` · 시스템 페이지 S4·S12·S22 갱신.

**이전 변경 (2026-09-14 134차) — 스토리 「조행록」 정립: 퀘스트 120 · 법 규칙 5조 · 일지·NPC 대화 · 플레이어 시나리오** (사용자 스펙 v3 업로드 + 지시 "플레이어 시나리오로 정립하고 인게임 요소를 최대한 반영" — 무결성 0건 · T1~T7 · 실렌더 12/12 · pageerror 0, 빌드 3/3·typecheck 0):
...
- **무게 모델 교체**: `W = weightFactor × L³`(지수 3 고정) → **`W = lwrA × L^lwrB`**(FishBase 표준형) + `bodyForm`.
  ...
  **56종 전수 재보정** — 감성돔은 사용자 제시표를 회귀해 **a 0.0179 · b 2.95**(30cm 408g / 40cm **953g** / 50cm 1.84kg / 60cm 3.15kg — 전 구간 적중),
  갈치는 FishBase *T. lepturus* **a 0.00032 · b 3.17**.
- **비만도(condition factor)** 신설 — "같은 길이라도 계절·산란 상태에 따라 무게가 다르다"(사용자):
  겨울(11~2월) **+7%** · 산란기(금어기 대용) **+9%** · 산란 직후 **−8%** · 개체차 N(0, 5%) · 클램프 0.82~1.22.
  상세보기에 **비만도 행**(`115% · 아주 통통함`) 표기.
- **체형 7종 파이트 배수**(`TUNING.fightPhys.form` — 어종군 `burst`와 **직교**: 군 = 힘의 크기 / 체형 = 힘의 성질):
  **장어·리본형**(갈치·붕장어·갯장어·학꽁치) = 정적하중 ×0.55 · 추력 ×0.62 · **1.2Hz 요동(진폭 45%)** · 바늘빠짐 ×1.45 —
  리서치 근거 "몸을 비틀어 요동치고 바닥·틈에 박는다" / **납작형**(광어·가자미·양태) = 정적하중 **×1.9**(판자 저항) ·
  체고형 ×1.2 / 방추형 = 거리 저항 ×1.15 / 두족류·구형 = 추력 낮고 요동 약간.
- ⚠ **드랙 모델 결함 발견·수정**(132차 백로그 I의 실체): 상한이 **"릴을 안 감을 때"에만** 걸려
  **감는 순간 요구 장력이 그대로 실렸다** → 감성돔 50cm·1.5호 랜딩 **0/300**. 실제 드랙은 감는 중에도
  스풀이 역회전하며 미끄러진다 → `reelDragCapFrac 0.97` 신설, **충격 하중(`shockBreakFrac 2.8` 초과)만 그대로 파단**.
  유영 부하 `pullIdle` 0.22 → **0.10**, `reelLoadKg` 0.6 → 0.25.
- ⚠ **체급 보정 안전대**(`safeFloorFrac`) — 안전대 30~80은 절대값이라 3kg 원줄의 24g 복섬은 영영 그 아래였다
  (탈출 배수 상시 2.7배·제압 보너스 28%). **못 거는 장력을 못 걸었다고 벌하지 않는다**(132차 슬랙 체급 게이트와 같은 원리).
- 검증: 시뮬(안내대로 = 감성돔 40cm **87%** / 50cm 85% / 참돔 83% / 광어 85% / 부시리 43% ↔ **라이트 채비 대물 18% · 부시리 2호 목줄 0%** 유지) ·
  실렌더(복섬 10m **10.0s** / 20m **15.0s** = 사용자 시나리오 목표 정합 · 갈치·붕장어 요동 안내 표시 · 광어 텐션 50.7~85% ↔ 장어형 8~13% ·
  132차 회귀 릴링 단조 0.00000m · 진행도 정합) · ⚠ **판매가·수율이 무게 비례로 함께 이동한다**(구세이브 어획물은 저장값 유지).
- 상세: `docs/wiki/03-WORKLOG/2026-09-11-133-lwr-bodyform-fight.md`

**이전 변경 (2026-09-11 132차) — 파이트 거리 정합: 랜딩 = 거리 / 제압 = 별도 축** (사용자 리포트 "발앞인데 진행도가 한참 뒤쳐진다" — 시뮬 + 실렌더 검증·pageerror 0, 빌드 3/3·typecheck 0):

- **원인 확정**: `progress`(랜딩 진행도)가 **거리와 무관한 자체 시계**였다 — 릴링 중 `+11/s`라 체감 9초 고정인데,
  `distM`은 `−1.35 m/s`에 **하한 1.2m**라 10m 착수는 7초에 발앞에 닿는다. 그래서 **물고기가 발 앞에 붙은 채
  진행 바만 차올랐고**, 30m에서는 반대로 진행도가 먼저 100이 되어 **18m 지점에서** 끌어오기가 시작됐다.
- **재정의**: 랜딩 진행도 = **거리 진행도**(`1 − (distM−landRange)/(hookDist−landRange)`)로 주입
  (`FightInput.landProgress`), core `FightingPhase`의 내부 축은 **제압도 `subdue`** 로 분리
  (피로 잔여 기저 + 패턴 대응 누적). `landed`는 **발앞 도달** 그 자체.
- **파이트 3D 운동 `stepFightKinematics` 신설** — 횡(`fleeLatM`)·거리(`distM`)·수심(`fightDepthNorm`)을 한 곳에서.
  **도주는 언제나 플레이어 반대쪽**(수평면 도주각 ±`runSpreadDeg`, 전진 성분 `max(0, cos)`) →
  수평뷰는 왼쪽/가운데 위/오른쪽, 수심 패널은 왼쪽으로 동시에 보인다. **무대 좌표(f2dPos)는 여기서 파생**
  (구 `updateFight2DSim`의 별도 적분 = 이중 시뮬 제거 — 세 뷰가 따로 놀던 근본 원인).
- ⚖ **릴링 중에는 거리가 한 프레임도 늘지 않는다**(`reelHoldCap` — 사용자 요구). 손을 놓으면 상한이 사라져
  줄이 나간다. 저항은 절대 m/s가 아니라 **회수 속도 대비 비율**(`resistFrac`)이라 체급이 달라도
  "영영 못 감기 / 저항 없이 끌려오기"가 생기지 않는다.
- ⚠ **24g 복섬은 바늘털이에 정대응(슬랙)하면 확정 바늘빠짐**이었다 — 최대 요구 장력이 3kg 원줄의 2%라
  게이지가 임계(6%) 아래를 못 벗어난다(시뮬 67%). **체급 게이트** 신설. 탈출 확률에도 **피로 배수**를 걸어
  먼 거리 파이트가 탈출 폭증이 되지 않게 했다(30m 30% → 21%).
- `TUNING.fightDist` 18키 + `fightPhys` 제압도 6키 + F8 슬라이더 13종. 패턴 안내에 "릴링을 멈추고" 보강.
- 검증: 시뮬 복섬 10m **8.1s** / 20m **13.0s**(무대응 10.1 / 15.7 — 정대응이 항상 빠르다) ·
  실렌더 랜딩 시 **distM 2.44~2.49m = landRange 2.5** · 릴링 중 프레임당 최대 증가 **0.00000m** ·
  진행 바 27.9% = 기대 27.9%. ⚠ **헤드리스 벽시계로 파이트 시간을 재면 10배 부풀려진다**(rAF 2fps + dt 클램프)
  → `fp.update(now,16.67)` 수동 스텝 + `fightElapsedSec`.
- 상세: `docs/wiki/03-WORKLOG/2026-09-11-132-fight-distance-coherence.md`

**이전 변경 (2026-09-11 131차) — 도움말 라이브러리 현행화(제작 카테고리 신설 · 생존 ready 전환 · 스킬 확장 반영) + 제작 UI 영문화** (사용자 지시 "반영된 모든 내용을 도움말 라이브러리에 내포" — 실렌더 ko 10/10·en 13/13·pageerror 0, 빌드 3/3·typecheck 0):

- **문제의 정체**: 도움말은 `data/HelpContent.ts` **데이터 한 곳**이라 기능 차수와 자동 동기화되지 않는다.
  123차가 스펙 기준으로 깔아 둔 `status: 'planned'` 6토픽(생존 지표·음식·상태이상·기절/사망·병원·상태 패널)이
  **125~129차에 전부 구현됐는데도 게임 안에서는 "준비 중"** 이었고, '준비 중 콘텐츠'엔 129차에 실구현된
  **'제작 (계획)' 스텁**이 남아 있었다.
- **도움말**: **'제작' 카테고리 신설**(기본 = U 창 제작 탭 / 고급 = 설치 제작대 [F] · 성공률·재료 절약·Shift+F 회수) ·
  생존 6토픽 **planned → ready 전면 재작성** · 스킬 트리 토픽 **3페이지화**(트리 읽는 법 / **해금 조건** /
  **시너지 히든 `???`**) · 레벨 토픽에 **Σ 215 = 만렙 200 + 면허 15** 등식 · 단축키 `U`·`K` 현행화 + **`Shift+F`** 추가.
  카테고리 11 · 토픽 44(`planned`는 실제 미구현 3종만 잔존).
- **본문 수치는 전부 코드에서 읽어 적었다** — XP 10~400·체장 0.5~3.0·첫 발견 ×5·준법 방생 ×0.5 ·
  Lv200 누적 5,636,469 · 도면 14(hand 8/workbench 6) · 제작대 180,000원 · 진료비 45,000원 ·
  임계 20%·HP −2/분·28℃·5℃ · 사망 −15%·기절 80% · 보양식 30~40 + 드레인 −25~32%.
- ⚠ **129차 제작 UI는 영문 사전에 전혀 없었다**(실렌더 확인 — 도면 이름·그룹 헤더·재료·버튼·로그가 en에서 한국어).
  `I18n.buildRuntimeDict`에 **`CRAFT_BLUEPRINTS`·`CRAFT_GROUP_LABEL` 합류**(데이터의 `nameEn`이 정본) +
  사전 156항목 + 규칙 16종(제작 결과·재료 줄·스킬 잠김·해금 조건·설치/보건소 로그).
  **130차가 바꾼 스킬 포인트 헤더 형식 때문에 122차 정규식이 조용히 죽어 있던 것**도 여기서 잡았다.
- ⚠ **합성 문자열은 사전을 비껴간다** — 도움말 팁이 `` `Tip · ${tip}` `` 이라 정확 일치가 깨지고, 구분자 분해도
  **팁 원문 속 ' · '** 때문에 어긋났다 → **붙이기 전에 `t()`로 번역**. 전수 스캔 356개 **미번역 0**(착수 시 98).
- 캡처 8장(제작 탭·고급 제작대·스킬 트리·상태 패널 × ko/en) + `annotate_help_images.py`에 콜아웃 20 ·
  **`hangul_ok()` 가드 신설**(한글 폰트가 없으면 두부 대신 축소본만 — 이 환경은 폰트 없음·egress 차단이라
  **한국어 콜아웃은 잔여**, 영문판은 정상 생성).
- 상세: `docs/wiki/03-WORKLOG/2026-09-11-131-help-library-craft-progression.md`

**이전 변경 (2026-09-11 130차) — 스킬 트리 확장 (a)(c)(d)(e) + Σ 등식·티어 제약 재정의** (사용자 지시 — core 43/43·실렌더 24/24·pageerror 0, 빌드 3/3·typecheck 0):

- **(a) 생존 지표 최대치** — 대식가·큰 물통·강단 3노드. `VitalsState`에 **`maxHunger`/`maxHydration`** 신설
  (⚠ `*_drain`(소모 속도)과 **다른 축**이다 — 이건 그릇 크기). **임계 20%는 절대값 → 비율**로 재정의.
- **(c) 행동력 확률적 미소비** — '요령' 4%p/랭크. 비용을 깎는 게 아니라 **가끔 아예 안 드는** 모델
  (129차 재료 절약과 동일 — 소수점 소모를 만들지 않는다). `applyVitalsAction`이 면제 여부를 반환.
- **(d) 해금 조건** — `SkillUnlockCond`(레벨·면허·카테고리 숙련). 선행 랭크와 **직교하는** 관문이고,
  기존 7노드(보트 조종·토지 거래·온실·문어 붙잡기·주식·로드 빌딩·대물 운)에 부여. 패널이 남은 조건을 그대로 표시.
- **(e) 시너지 히든 5종** — **비용 0 · maxRank 1 · requires 2개 이상**. 조합을 채우면 **자동 습득**되고,
  미해금 동안은 `???`로만 보인다. `canLearnSkill`이 히든을 거절 — 포인트로는 살 수 없다.
- ⚖ **등식 재정의: Σ(유료) 200 → 215 = 만렙 200 + 면허 보너스 15.** 세이브는 랭크만 저장하고 포인트는
  DB 비용으로 매번 재계산하므로, **기존 노드 비용을 깎으면 이미 찍은 세이브의 사용 포인트가 소급 변동한다** —
  그래서 좌변이 아니라 **우변(예산)을 늘렸다**. 새 표어 = **"만렙 + 전 면허 취득 = 전 스킬 마스터"**.
- ⚖ **티어 수용 6 → 8** — 124차는 행 간격만 줄여 7노드에서 **6.7px 겹쳤다**(실측). 이제 `nodeMetrics`가
  **노드 높이까지 줄이고**(하한 46px) 압축된 열은 설명을 생략한다(설명은 하단 상세 스트립이 갖는다).
  티어는 여전히 0~3 — 5열은 패널 폭 788px에 안 들어간다.
- 실배선 33 → **43/92**(생존 최대치 3·행동력 면제·위생·응급처치 정정·시너지 5).
- 상세: `docs/wiki/03-WORKLOG/2026-09-11-130-skill-tree-expansion.md`

**이전 변경 (2026-09-11 129차) — 성장·생존 P7: 제작 시스템 + 구급품·보건소 + 피로 회복 음식** (사용자 지시 — core 32/32·실렌더 36/36·pageerror 0, 빌드 3/3·typecheck 0):
...
- **규칙 정정(사용자)**: 이모지 금지의 본뜻은 **① 이모지 ② "[아이콘] [타이틀]"식 텍스트 장식**이고,
  **시각 아이콘 자체는 금지 대상이 아니다**. 126차의 텍스트 약어 배지(`식`/`FP`)는
  "유저가 해석해야 하는 UI"라 폐기 — **통일 칩 프레임은 유지하고 내용물만 그림으로**. §4·§8-8 재작성.
- **픽셀 아이콘 파이프라인 신설** — `tools/gen_pixel_icons.py`(**16x16 손그림 매트릭스** 13종 + 검사) →
  `data/PixelIconArt.ts`(자동 생성) → **`ui/PixelIcon.ts`**(캔버스 텍스처 굽기 · 정수 배율 배치).
  허기(빵)·수분(물방울)·식중독(생선 가시)·생물중독(해파리)·설사(변기)·출혈·골절·오한·감기(마스크)·
  독감(바이러스)·이상고열(체온계)·탈진·기절.
- core **`StatusBadge {ko,en,color}` → `{icon,color}`** (소비처는 `RegionHud` 한 곳 — 세이브 무관) ·
  스트립 칩·허기/수분 라벨을 아이콘으로 교체(**로케일 무관** — 사전·폭 흔들림 문제가 사라진다).
- **호버 팝업 = 커서 추종 폐기 → 앵커 확장**(`anchorTip` — 여유 공간 큰 쪽) + **매초 갱신**(`tipRefresh`).
  허기·수분 툴팁은 `62% / 100%`, 체력·피로도·경험치는 `현재 / 최대 (비율)` 유지.
- ⚠ 실렌더에서 잡은 것: 앵커를 **칩 하나**에 붙이면 팝업이 형제 칩을 덮어 다음 상태로 커서를 못 옮긴다
  → **스트립 전체 박스**를 앵커로. 아이콘은 16px 실사이즈로 육안 확인해야 한다(10배 미리보기는 다 예뻐 보인다 —
  마스크·뼈·악취선 3종을 그 때문에 다시 그렸다).
- ⚠ 하네스 함정 2건: **헤드리스에서 `scene.time` 타이머가 발화하지 않는다**(신규 프로브도 0회 —
  `hud.updateStatus()` 직접 호출) · **`GameState.vitals`는 게터가 새 객체를 만든다**(`commitVitals` 사용).
- 상세: `docs/wiki/03-WORKLOG/2026-09-10-128-pixel-icons-hover-tip.md`

**이전 변경 (2026-09-10 127차) — 성장·생존 P5·P6: 날씨 캐스팅 물리 + 스킬 실배선 24/83 + 조준 홀드 가이드** (사용자 지시 3건 — core 28/28·실렌더 19/19·pageerror 0, 빌드 3/3·typecheck 0):

- **core `CastWeather.ts` 신설** — **풍속 3m/s 초과부터** 맞바람 비거리 감소(9m/s ×0.73 · 실비행 **466px → 332px**) ·
  옆바람 산포 ×1.6 + 착수점 밀림 · **비 = 유속 ×1.27 · 밑걸림 ×1.33**(소나기 ×1.45/×1.55).
  `TUNING.castWeather` 13키 + F8 슬라이더 8종(mockup — 실플레이 조율 대기).
- ⚖ **비거리 배율은 완력이 아니라 수평 속도 전체에 먹인다** — `CastLaunchParams.speedMult` 신설.
  완력 기여가 초기 속도의 15%뿐이라 완력에만 곱하면 −7%에 그친다(실측 466 → 431px).
  **맞바람 손실(`distanceMult`)과 횡풍 밀림(`crossWind`)은 분리** — 한쪽에 몰면 이중 계산.
- **P5 스킬 실배선 13 → 24/83** — 생활 6종(허기·갈증·수면·면역·내한·회복)·채집 속도·정투·바람 읽기 +
  `GameState.rollStatus`(상태이상 확률 × 면역력)·`resetSkills` + **리스펙 아이템**('조업 재교육 이수증' 20만원).
  ⚖ 바람 보정은 **손실의 20%만**(`fish_wind` 랭크당 +0.40 → **+0.10**, 상한 0.30 — 사용자 "아주 미비").
  ⚖ `skillMult`는 **선형 합산**이라 스펙의 `0.7^rank`는 그대로 못 쓴다(`fish_scatter` −0.22/랭크 = 3랭크 0.34).
- **P6 캐스팅** — 조준 **400ms 홀드 = 필요 파워 눈금**(`solveCastPower` 이분 탐색 역산 · 사거리 초과 경고) ·
  착수 무작위 산포(거리 비례 × 날씨 − 정투) · 산포 링 · 바람 화살표 · 비행 카메라 팔로우/복귀.
- ⚠ **core 재빌드 후 vite dev 사전번들이 옛 core를 물고 있다** — `packages/client-pc/node_modules/.vite` 삭제 후
  dev 재시작하지 않으면 실렌더 검증이 **조용히 옛 코드를 본다**(127차에 실제로 밟음).
- 상세: `docs/wiki/03-WORKLOG/2026-09-10-127-cast-weather-skill-wiring.md`

**이전 변경 (2026-09-10 126차) — 성장·생존 P3·P4: 상태 패널 개편 + 기절·사망 FSM + UI 이모지 금지 규칙** (사용자 지시 3건 — 실렌더 29/29·pageerror 0, 빌드 3/3·typecheck 0):

- **UI 특수문자·이모지 금지 규칙화**(사용자 지시) — 15차 "텍스트 접두사 금지"를 **아이콘·디자인 전체로 확대**.
  §4 전용 절 + §8-8 금지 사항 + CLAUDE.md + 스킬 `ui-panel`에 명문화(허용 대안 3종 · 예외 `✕ ◀▶ ◱◐`).
  125차에 들어갔던 `StatusEffects.icon` 이모지 11종 → **`badge{ ko 1자, en 2자, color }`** 교체.
- **P3 상태 패널 '대' 개편** — 경험치 바(6px·`Lv.n`) · 우측 컬럼 **허기/수분 컴팩트 바** ·
  **수치 호버 팝업**(상시 숫자 없음) · **상태이상 스트립**(패널 밖 하단 좌정렬 · 색 칩 + 라벨 · 4개 초과 시 2행 ·
  이름/남은 시간/치료 수단/설명 Ko·En 툴팁). **'중/소' 회귀 0** · 패널 높이 불변(배지 행만 +16px 하강).
- **P4 기절·사망 FSM** — `ui/CollapseOverlay.ts` 신설(눕기 90° + 비네트 0.5초 + 2초 후 팝업) ·
  `TUNING.collapse`(재화 15%·인벤 유지·HP/허기/수분 50%·피로도 80% 캡 — 하드코어 프리셋은 주석 보존) ·
  1인칭은 채비 회수 후 **필드 씬이 연출 전담**.
- ⚠ 신규 함정: 게이지 rect는 **`barRects()` 단일 소스**(렌더·히트 공유) · 쓰러짐 정리는 create/shutdown 양쪽 ·
  **부활은 저장하지 않는다**(침대 저장 정책 불침범) · ERASE 블렌드는 게임 화면까지 지운다(그라데이션 텍스처로 우회).
- 상세: `docs/wiki/03-WORKLOG/2026-09-10-126-vitals-panel-collapse-fsm.md`

**이전 변경 (2026-09-10 125차) — 성장·생존 P2: 생존 지표(허기·수분·피로) + 상태이상 11종 + 세이브** (스펙 §11 P2 — core 44/44·실렌더 12/12·pageerror 0, 빌드 3/3·typecheck 0):

- **core `Vitals.ts`·`StatusEffects.ts` 신설** — 활동 드레인 6종(대기/앉기/걷기/자전거/달리기/채집) ·
  행동 비용 8종 · 임계 20%(이동 −20%·피로 ×1.3) · 0% → HP −2/분 · 온도 계수 · 수면(피로 0·HP +50%) /
  상태이상 11종(진행 체인 오한→감기→독감 · 자연치유 · 재발 롤 · 효과 합산).
- ⚖ **시계 = 활동 시간만** — 저장 구조에 타임스탬프를 두지 않고, 씬 `update()`의 **일시정지·모달 가드 뒤**에서만
  `tickVitals`를 부른다. 그 자체가 오프라인 정지(로드 시 보정 코드 불요).
- **배선 9곳** — 필드/1인칭 드레인 · 캐스팅/파이팅/손질/회뜨기/출조 비용 · 음식 회복치 14종(생수·주먹밥 신설) ·
  침대 수면 · **채집 부상 → 골절/출혈/생물중독 승격** · '나쁨' 음식 섭취 허용 + 식중독 롤.
- `GameState`: `maxHp = 100 + ⌊Lv×0.5⌋ + stamina_max − 상태이상` · `moveSpeedMult`·`canRideBike` ·
  `SaveData.vitals`(구세이브 = 만복 시작). **HP 원본은 `player.stamina`** — 별도 보관 금지.
- 잔여: 허기·수분 UI(P3) · 기절/사망 FSM(P4 — `fainted` 신호는 이미 반환) · 치료 아이템·병원(P5·P7).
- 상세: `docs/wiki/03-WORKLOG/2026-09-10-125-vitals-status-effects.md`

**이전 변경 (2026-09-10 124차) — 성장·생존 P1: 레벨 200 곡선 + 활동 XP + 스킬 트리 83노드(Σ=200) + 제작 설계 정정** (사용자 착수 지시 — core 42/42·실렌더 ko/en 9/9·pageerror 0, 빌드 3/3·typecheck 0):

- **제작 설계 정정(사용자)**: 123차 ⚖#8 "제작대 = [F]"는 오독 — **기본 제작 = U 창 '제작' 탭 신설**(핸드크래프팅
  보드, 조건 없음 — 재료 보유 시 활성·수량 조절) / **고급 제작 = 설치된 고급 제작대 [F] → 팝업**. 스펙 §2-5 재작성.
- **core `types/Progression.ts` 신설** — `MAX_LEVEL 200` · `xpToNext = round(25n^1.5)+75`(Lv200 누적 5,636,469) ·
  `catchXp`(희귀도 기본 × 체장/평균 0.5~3.0) · `activityXp` 5종 · `TUNING.xp`. `GameState.grantXp`(만렙 캡 ·
  구세이브 무마이그레이션) + 배선 4곳(어획 첫 발견 ×5 · 손질/회 ×등급 · 채집 · 준법 방생 ×0.5).
- **스킬 트리 44 → 83노드 Σ=200pt**("만렙 = 전 스킬 마스터" 등식) — 39노드 증설(전부 `wired:false` 예정) +
  **제작 카테고리 신설(잠금·열람만)** + DB 무결성 검사(Σ·고아·티어당 ≤6). 기존 44노드 id·비용 보존(세이브 호환).
- ⚠ 배치 제약: **티어 0~3 · 티어당 ≤6노드**(SkillTreePanel 행 압축 한계) · XP는 반드시 `grantXp` 경유.
- **착수 순서 확정**: 스펙 P2~P9 선행 → 통발 D2 → 불요리 → 농장(농사 배선·타입은 농장 단계 이연).
- 상세: `docs/wiki/03-WORKLOG/2026-09-10-124-progression-p1-level200-skilltree.md`

**이전 변경 (2026-09-10 123차) — 성장·생존 통합 스펙 v2(122차 정합판) + 도움말 '성장 · 생존' 선반영** (사용자 원안 업로드 — 실렌더 ko/en·pageerror 0, 빌드 3/3·typecheck 0):

- **`.agents/PROGRESSION_SURVIVAL_SPEC.md` 신설** — 원안(레벨 200·스킬 88노드 200pt·생존 4지표·
  상태이상 11종·기절/사망·퀘스트 7챕터)을 122차 실코드에 정합한 **v2 실행 스펙**. 원안 대비 결정
  14건(⚖): effect.key 아키텍처 유지 · **스킬 44노드 보존 + 39 증설 = Σ 200pt**(제작 카테고리 신설) ·
  실시간 시계 재기저(행동 비용 1축·오프라인 정지) · 병원 입원/사망 페널티 완화 · 제작대 = [F] ·
  체력 기저 100 유지 · 영문화 규칙 §10 · 구현 순서 P1~P9(백로그 H — 착수 순서는 사용자 결정 대기).
- **도움말 라이브러리 선반영**: '성장 · 생존' 카테고리(ready 4 — 레벨/스킬/면허·단속/일지 ·
  planned 6 — 지표/음식/상태이상/기절·사망/병원/상태 패널) + 낚시 `cast-scatter` + 낡은 스텁 정리
  (`p-gather` 삭제·`p-craft` [F] 갱신). **영문 사전 56항목** — en 실렌더 한국어 잔존 0.
- 로컬 잔재 정리: 미커밋 구 100차(레거시 해루질 배선)는 121차 인-맵 구현이 대체 — 폐기(백업 보관).
- 상세: `docs/wiki/03-WORKLOG/2026-09-10-123-progression-survival-spec-v2.md`

**이전 변경 (2026-09-09 122차) — 121차 피드백 5건: 상호작용 E → F · 영문 잔여 · 팝업 ✕ 통일 · HUD 명패 · 면허/스킬/일지 패널** (사용자 스크린샷 6장 — 실렌더 하네스 41/41·pageerror 0, 빌드 3/3·typecheck 0):

- **E = 장비창 전용 · F = 상호작용**(오브젝트 > 건물 > 채집 홀드 > 통발 수거) — 검증 중 홀드 폴링이 `keyE`로 남아 F 홀드가 즉시 취소되던 것도 수정.
- **면허(L)·스킬(K)·일지(J)** — `Skills.ts`/`SkillDatabase`(카테고리 6 · 스킬 40 · **실배선 13** · 농사 잠김 · 레벨당 1pt) · 면허 카테고리 6군 + 7종 ·
  `LicensePanel` 재작성(윈도우드 22행) · `SkillTreePanel`(티어 열 트리) · `JournalPanel`(스토리·메인/서브 골격). 시스템 페이지 **S21 `progression.md` 신설**.
- **영문**: 도감 라벨·지역/구역 설명(`descriptionEn/descEn/detailsEn`)·출처 화면(`providerEn/serviceEn/usageEn`) 데이터화 + `en_panels.ts` · 금액 규칙 숫자만.
- **팝업**: 도감·출처 하단 버튼 제거 → 우측 상단 ✕ · `openPopup`이 **새 팝업을 밴드 최상단**으로(`raiseToTop`) → ESC LIFO 정합 · HUD 명패 `locale-changed` 재배치.
- 상세: `docs/wiki/03-WORKLOG/2026-09-09-122-feedback-fkey-i18n-panels-skills.md`

**이전 변경 (2026-09-09 121차) — 인-맵 채집(해루질) + 어촌계 어장 실폴리곤 + 강원 조례 + 통발 아이템 설치/수거** (사용자 지시 "실제 해루질 요소를 현재 맵 안에 발현" + 결정 7건 — 실렌더 하네스 2종·pageerror 0, 빌드 3/3·typecheck 0):

- **별도 씬이 아니라 속초 맵 위에서** — 지형 규칙(섬/암초 갯바위 · 사석/TTP 발밑 · 항만 안벽 · 웅덩이)으로 후보 3,836타일 →
  시간 시드 스팟 ≤220 → 야간은 헤드랜턴 반경(800lm ≈ 4.4타일) 안만 보임 → **[E] 홀드** → 쿨러/인벤 '채집물'.
  판정은 전부 core `ForagingEngine`(도구 게이트·맨손 부상·문어 도주·법정 크기 방류) + `TUNING.forage`(mockup).
- **어촌계 어장 실폴리곤** — 국립해양조사원 어장정보 SHP(EPSG:5179)를 `tools/extract_fishfarms.py`로 클립 →
  속초 3폴리곤(마을어업속초9·협동양식속초5·6). 원본 114MB는 `docs/reference/gis/`(gitignore), 파생 5KB만 커밋.
- **강원 조례 전부 재현** — 어장 안 전복·해삼·성게·홍합·문어 = 적발 롤 → 압수 + 벌금(재화 30%·상한 30만) ·
  채집물 **판매 금지 3경로** · 산란기 도루묵 통발 · 스쿠버 없음(후속 `ForageAccess` 자리만).
- **통발 = 아이템** — 직판장 8종 · T/인벤 '통발 놓기' → 통발×미끼 패널 → 물 위 클릭(4타일·육지 거리 ≤3·실측 수심) →
  wall-clock 침지 → [E] 수거(분실 롤 1회 → 포획 → 통발 반환·내구도). 심화 면허 등급 게이트.
- ⚠ 실측: **실데이터 파고 2.1m가 채집을 막았다**(설계대로 — 하네스는 임계 상향) · 스팟 상한 40은 맵에 흩어져 화면에 0개 → 220.
  해루질 입문 허가의 '거제 방문' 조건 제거(심리스 속초에선 달성 불가).
- 상세: `docs/wiki/03-WORKLOG/2026-09-09-121-inmap-foraging-fishfarms-traps.md`

**이전 변경 (2026-09-08 120차) — 영어 잔여 마무리: 상호명 `nameEn` + 지명 필드 구조화 + 콘텐츠 산문 사전** (119차 잔여 3건 사용자 결정 — 전수 스캔 잔여 0·pageerror 0, 빌드 3/3·typecheck 0):
...
- **HUD 타이틀바(캡션바) 규칙 신설** — 상태 패널에 밴드(`HUD_HEADER_H = 20`, 제목 '상태' + ◱ ◐)를 두고 컨텐츠를
  ...
  축소 단계(k=0.5)에서 밴드가 11px로 줄어 버튼이 로그를 침범 → 최소 높이 하한. **실측 여유 11px / 5px.**
- **지역 채널 3단계 표시** — `LogEntry`(시각·머리말·본문 분리) + `formatLogLine`: 큰 `[HH:MM] [이벤트] 본문` /
  중간 = 시각 제거 / 최소 = 본문 10자 초과 시 `...`(짧은 내용은 그대로).
- **월드맵 겹침 = 클릭 탈취였다** — 홈 버튼이 참조 없이 생성돼 지역/스팟 뷰에서도 남아 **'← 전국 지도' 클릭을
  depth 122 히트렉트가 삼켰다**. 부품 보관 + 뷰별 표시 토글 + 타이틀/힌트/목록을 버튼 아래로.
  구역 카드·스팟 툴팁은 **고정 y → 실측 흐름 배치**(wordWrap 줄 수 가변).
- **랜딩 후 수심 정보** — result 상태에서 조류·바람 정지(`lastTidal` 고정·tide 0) + `LANDED_PULL_MPS`로 발앞 0.3m까지
  당겨 붙임(구: 반탄류가 계속 더해져 **뒤로 갔다**) + 결과창 좌클릭 릴링 차단. 거리축 물고기 아이콘 **좌우 반전** +
  챔질 후 **1.2배·희귀도 색**(괴물급 ≥2.4×도 대물 빨강으로 표시 — 서프라이즈 유지, 픽셀 실측 확인).
- **i18n 근본 수정** — `translateOnce`가 `s.replace(re, '$1…')`이라 **캡처 안 한국어가 재번역되지 않았고**,
  포괄 규칙 `[HH:MM] 본문` 하나가 채팅 로그 전체의 번역을 막고 있었다 → `applyTemplate`(캡처 재번역) +
  구분자/콜론 분해 확장 + **탐욕적 금액 규칙을 맨 끝으로**. 사전 **656항목 추가**(아이템 227·채비/요리 339·도움말 90) +
  `places.ts` 지명 + 해양생물·루어·오라클 런타임 합류 + 괄호 별칭 짧은 이름. 요일 `(Tue)`·`[System]`·`Sokcho` 확인.
- **캡처 주석 영문판** — 영어 로케일로 21장 재촬영 + `annotate_help_images.py --lang en`(`LABEL_EN` 80종) →
  `help_*_en.png`, `compose_fight_guide.py en` → `guide_fight_6_en.png`. 패널이 로케일로 선택(없으면 한국어판 폴백).
- 단축키 버튼 '단축키' → **'도움말'**(EN `Help`) · 아이템 슬롯 수량 `x숫자` **우측 상단**.
- ⚠ 신규 규칙: **타이틀바 전용 영역** · **wordWrap 아래 고정 y 금지(흐름 배치)** · **참조 없는 고정 버튼 금지**
  (`.claude/skills/ui-panel/SKILL.md`, `.agents/AGENTS.md` §4, `docs/wiki/02-SYSTEMS/ui-framework.md` §6).
- 상세: `docs/wiki/03-WORKLOG/2026-09-08-119-hud-caption-i18n-depth.md`

**이전 변경 (2026-09-08 118차) — 117차 피드백 8건: HUD 실버그·캐스팅 힌트 조건·영어 설정·도움말 실캡처** (실클릭 하네스 4종·pageerror 0, 빌드 3/3·typecheck 0):

- **HUD 실버그 2**: `statusC.add(statusC)`(자기 자신에 add → 상태 패널이 아예 안 그려짐) → `this.add(statusC)`. 지역 채널은
  `destroyLogPanel()`(파츠·마스크·씬 입력 핸들러 해제) 뒤 **k 스케일 실치수로 재생성**(좌하단 고정). 최저 투명 0.05.
  ⚠ 하네스가 상태값만 보고 통과시켰다 — **스크린샷 육안 확인 없는 UI 검증은 검증이 아니다**(verify-render 스킬 반영).
- **캐스팅 힌트 입력 조건**: 씬 `pointerdown(p, over)` — `over.length > 0`(HUD·버튼·팝업 위)면 return. 맵 직접 클릭만 캐스팅.
- **영어 설정(i18n)**: `client/i18n/I18n.ts` — `Phaser.Text.setText` 프로토타입 훅으로 **모든 Text가 사전을 통과**(원문 = 키,
  정확 일치 → 규칙 → 구분자/글리프/태그 분해), `setLocale`이 살아 있는 Text를 원문으로 재-set해 즉시 전환. 어종명은
  `FISH_DATABASE.nameEn`. ESC 메뉴 [환경설정] → `SettingsScene(returnScene)`. ⚠ 새 코드에서 `text === '한글'` 비교 금지.
- 희귀도 `[1.14×, 평균]`(common '보통'→'평균') · `TextFit.enforceTextBounds`(패널 우측 경계 강제 + `[TextOverflow]` 경고) ·
  dev 훅 `__FP.devForceBite()/devForceFight(pattern)` → 도움말 실캡처 21장(`tools/annotate_help_images.py` 688px 표시 크기에
  라벨을 최종 픽셀로) + 파이트 튜토리얼 합성 삽화(`tools/compose_fight_guide.py`).
- **후속(같은 날)**: 상태 패널 3단계를 **scale → 정보량 축소**로 재설계(글씨 크기 고정 · 중간 = 아이콘만 + 아이콘 호버
  툴팁 · 최소 = HP·피로·시각만 + 패널 호버 시 날씨 전체 툴팁, 커서 우측 하단) · 단축키 버튼 **재클릭 = 닫기** +
  ⚠ **`useHandCursor` 객체 파괴 시 Phaser가 커서를 기본값으로 되돌린다**(실측) → `restoreHandCursor(scene)` 신설.
- 상세: `docs/wiki/03-WORKLOG/2026-09-08-118-hud-fix-i18n-help-captures.md`

**이전 변경 (2026-09-08 117차) — 도움말 라이브러리 + HUD 재배치 + 외부 테스터 피드백 7건** (사용자 대형 지시 3부 — 실클릭·휠·드래그 검증, 이미지 13/13 로드, pageerror 0, 빌드 3/3·typecheck 0):

- **도움말 라이브러리**: `data/HelpContent.ts`(8카테고리 › 섹션 › 39토픽 › 페이지) + `ui/HelpLibraryPanel.ts`
  (1080×656 · 좌측 3단 트리 윈도우드 렌더 · 우측 GeometryMask 본문 · depth 890 비모달). **F1 / 우하단 단축키
  버튼**(64×64 둥근 사각 — 구 힌트 바 대체). 이미지 = `tools/annotate_help_images.py`(실캡처 + 번호 콜아웃) 13장.
- **HUD**: `RegionHud` 상태 패널 = 래퍼 컨테이너 크기 3단·투명 4단 / 지역 채널 = **크기마다 재생성**(로그 보존).
  ⚠ **중첩 컨테이너 + GeometryMask + 인터랙티브 자식 = 포인터 이동 시 렌더러 크래시**(실측) — 래퍼 금지.
- **파이팅 물리 텐션**(`FightingPhase` 물리 모드 + `simulation/FishRarity.ts`): 텐션% = 체중×(정적 0.35 + 어종군
  가속×패턴×피로×**대응 배수 0.6/1.35**) ÷ 라인 강도(원줄·목줄 약한 쪽 — 카본 1.8kg/호 · PE 9kg/호). 릴 안 감으면
  드랙 상한 0.85. `TUNING.fightPhys` + F8 5종(**mockup — 실플레이 조율 대기**). 시뮬: 잿방어 1.2kg/1호 = 파단(의도).
- 희귀도 `fishRarity` 6단계(≥2.4 검붉 · ≥2.0 빨강 · ≥1.6 주황 · ≥1.2 노랑 · 흰 · ≤0.8 초록, 돔·방어류 보통→우량 하한) —
  어획 팝업 제목색·인벤 요약·상세보기 행. 입질 튜토리얼 = **첫 입질 1단계 진입 순간** 정지 + [다시 표시하지 않기].
- 채비: `InventoryStore` 루어 전용 `_lureLine/_lureLeader`(세이브 + 구세이브 시드) · 직판장 원줄·목줄·바늘 판매 ·
  바늘 소켓에 루어면 원투 봉돌 면제 · dragIn `dt×0.55` + "◀ ← 키 + 릴링" 화살표.
- 상세: `docs/wiki/03-WORKLOG/2026-09-08-117-help-library-hud-feedback.md`

**이전 변경 (2026-09-08 116차) — 조도 시트 실제 위치 재적용** (사용자 캡처 2장 "sheet.png를 인게임 조도 위치에 그대로" — 실렌더 4지점·pageerror 0, 빌드 3/3·typecheck 0):

- ⚠ **115차는 엉뚱한 섬에 적용했었다** — (274,532)는 청초호 쪽 초지 섬. 실제 조도 = `lights.json`
  **조도등대 (794,427)** 옆, 성분 994타일·bbox 41×33. 래스터 `'r'`이 섞여 `computeIslets`의 `wild`
  조건에서 빠져 섬 플래그 0이었던 탓에 "큰 섬 성분" 검색에 안 잡혔다. 백업으로 원복.
- **면적 기준 재적용**: 그리드 **72×60** → 사진 본섬 898셀 bbox **41×33 = 인게임 41×33**.
  도구에 `--islet-max`(994타일 치환 허용) + 배치 기준을 **본섬 최대 성분 무게중심**으로(잡석 bbox 쏠림).
- **`computeIslets` 상한 600 → 1200** — 시트 적용 후 뭍 927타일이 상한에 걸려 섬 판정이 빠지자
  '.' 밑 얕은 물 베이스가 안 깔리고(투명에 tan) 항만 규칙이 어선 4·크레인 1을 섬에 세웠다(실측).
- 원본 사진은 바탕화면에서 `Desktop/voi_홍희준/…/map tiles/`로 이동돼 있었다(2048×1716 검색).
- 상세: `docs/wiki/03-WORKLOG/2026-09-08-116-jodo-relocate-real-site.md`

**이전 변경 (2026-09-03 115차) — 조도 드론 사진 → 갯바위 타일 시트** (사용자 사진 1장 + 지시 "타일셋 느낌으로 32×24 정도 그리드화, 경계 와이어링, 그대로 적용" — 실렌더 3지점·pageerror 0, 빌드 3/3·typecheck 0):

- **`tools/pixelize_islet.py`**: 사진 → ½ BOX 리샘플 → HSV 분류(물/초지/암반) → **휘도 순위 균등화**로
  해안 rock 램프 배정(중앙값 124 — 분위 정규화는 175로 너무 밝았다) → NEAREST ×2 → 시트 + `patch.json`
  병합(정본·런타임 둘 다). 물 픽셀은 **투명**(게임 물이 비친다). 그리드 32×27(종횡비 보존).
- **경계 와이어링**: 셀 뭍 ≥ 0.5 = `'.'`(걷기) · 뭍 ≥ 5% = 그림만(여). 본토 타일은 절대 안 덮는다.
- **시트 1장 슬라이스**: `ISLET_SHEETS` + `ensureSheetCell`(`ts_jodo_r{r}c{c}` 캔버스) — 셀 241개를
  파일로 싣지 않는다. 다음 섬 = 도구 실행 + 이름 1개.
- 가드 3곳: 사진 셀 물 타일엔 절차 바위 산포·**타일 변 포말**(격자 이음선 원인) 생략 · 섬엔 크레인 금지.
- 상세: `docs/wiki/03-WORKLOG/2026-09-03-115-jodo-photo-islet.md`

**이전 변경 (2026-09-03 114차) — 방파제 단면 자동 분류 + 항로표지 등대** (사용자 캡처 3장 "위성처럼 테트라포드+등대, 속초 전부" — 실렌더 6지점·pageerror 0, 빌드 3/3·typecheck 0):

- **단면 분류 `computeBreakwaters`**: `'b'` 성분마다 물까지 거리로 dw ≤ 2 = 테트라포드(TTP 시트) ·
  dw 3(폭 넓은 것만) = 사석 · 안쪽 = 상판. 상판 자리 `'.'`도 편입(구멍 메우기) → 콘크리트.
  방파제 판정 = 외해 접수 비율 ≥ 0.35(안벽은 종전 렌더). 전수: 상판 2,136 · 사석 806 · 피복 2,860.
- **등대**: `tools/extract_lights.py`(OSM seamark → `lights.json`, 지형 재빌드 없음) + `RegionLight` ·
  `hasLights` 플래그 · 절차 베이크 5종(홍색=우현 · 백색+녹돔=좌현 · 등주는 물 위 free). 속초 5/5.
- 피복 위 **가로등·계선주 금지** — 가로등은 상판 가장자리만(`breakwaterClassAt`).
- ⚠ 106차 "자동 규칙 폐기"와 다른 점: 시트 전체가 아니라 **단면 거리로 3종만** 배정한다.
- 상세: `docs/wiki/03-WORKLOG/2026-09-03-114-breakwater-section-lighthouse.md`

**이전 변경 (2026-09-03 113차) — 차량 배치 재설계** (사용자 리포트 4건 — 실렌더 10지점·pageerror 0, 빌드 3/3·typecheck 0):

- **주차장 구역 한정**: `computeParkingLots` — 도로 밖 `'r'` 성분 중 40~700타일 ∧ 채움률 ≥ 0.5 ∧
  진입로 5타일 ∧ 섬 제외 = **40구역 4,553타일**(구 16,630의 27%). 구역별 **점유율 0.40~0.70** 랜덤.
  112차가 광장·매립지·항만 에이프런까지 차로 덮은 것이 "너무 많은 차"의 실제 원인이었다.
- **도로변 평행 주차 폐기** — 연석 안쪽 = 주행 차로라 "도로 위 정차"로 보였다. 전 지점 **도로 위 차량 0**.
- **노상 밀도** 0.80대/화면(10지점 평균 — 0대 7곳·상가 밀집지 3~4대).
- ⚠ **조도의 정체는 차가 아니라 어선**(실렌더 확인) — `computeHarbor`가 섬 둘레를 항만으로 잡고
  앵커가 36px 뭍으로 올라탔다. 섬 둘레 2타일 제외 + 앵커 재계산(침범 ≤ 8px).
- ⚠ **`computeParkingLots`는 `indexRoads()` 뒤에서 호출해야 한다** — `roadBand`가 `roadsByChunk`를
  쓰므로 앞에 두면 접근성 판정이 전부 false가 되어 구역 0개(차량 전멸). 실측으로 발견.
- 상세: `docs/wiki/03-WORKLOG/2026-09-03-113-vehicle-placement-rework.md`

**이전 변경 (2026-09-03 112차) — 밀도 트랙 4건 일괄** (사용자 지시 순서 그대로 — 실렌더 6지점·FPS 55·pageerror 0, 빌드 3/3·typecheck 0):

- **주차 열 주축**: 도로 밖 `'r'` 5×5 침식 성분의 bbox 종횡비(h ≥ 1.35w = 세로 행) → 전역 1회 계산이라
  청크 경계 정합(111차 보류 사유 해소). 세로 행은 측면 프레임 · 가로 행은 정면/후면.
- **콘크리트 광장**: `kn_plaza_0/1`(2px 그레인) 을 L1이 `'r'` 위에 얹고 벡터 밴드가 회랑을 덮는다 —
  밴드 밖만 콘크리트 · 광장엔 아스팔트 반점 금지.
- **항만 디테일**: `computeHarbor`(4방향 광선 ≥ 3 뭍 = 둘러싸인 물) → 정박 어선 열(E-W 접현 3타일 ·
  N-S 선수 접안 2타일) · `'.'/'w'/'r'` 안벽 계선주 · 하역 크레인 절차 베이크(`smx_crane`).
  ⚠ 안벽 판정에 **래스터 `'r'`** 을 넣지 않으면 규칙이 침묵한다(실측 어선 1·크레인 0).
- **도로변 평행 주차**: 축 정렬 세그먼트만(회전 금지) · 끝단 3타일 물림 · 점유 0.25 · 연석 안쪽.
  주행 차량과 시각 겹침 가능(위험).
- 상세: `docs/wiki/03-WORKLOG/2026-09-03-112-density-track-batch.md`

**이전 변경 (2026-09-02 111차) — 주차장 차량 열 절차 배치** (밀도 트랙 1순위 — 실렌더 3지점·FPS 60, 빌드 3/3·typecheck 0):

- 109차 래스터 'r' 광장(도로 밴드 밖 ∧ 3×3 'r')에 **행 r%4·칸 c%2 전역 위상**으로 결정적 주차 열 —
  점유 0.66 · TopDown 차량 6종 전면/후면 프레임 · 충돌 있음. 동명항 어시장·항만 안벽에서
  참고 이미지 밀도 정합. 잔여 = 열 방향 주축 판정·도로변 평행 주차·'r' 지면 콘크리트 톤.
- 네이버 위성 픽셀아트 직접 동봉은 **라이선스 위험으로 배제**(사용자 결정) — 절차 배치로 대체.
- 상세: `docs/wiki/03-WORKLOG/2026-09-02-111-parking-lot-rows.md`

**이전 변경 (2026-09-02 110차) — 교차부 마킹 회랑 클리핑** (사용자 캡처 리포트 "교차로 선들이 난잡하게 겹침" — 실렌더 3지점·FPS 59·pageerror 0, 빌드 3/3·typecheck 0):

- **원인**: `markingPieces`는 그래프 정점 공유 교차만 절단 — 정점 없이 기하적으로만 겹치는
  교차(이중도로 oneway 쌍의 상대 도로·회전교차로 링·중간 관통)는 마킹이 교차로 안을 관통했다.
- **해법(수작업 불요)**: `SeamlessChunks`에 **마킹 회랑 클리핑** — 세그먼트 공간 해시 +
  `markSuppressedAt`(다른 도로 회랑 halfW+0.15 안 또는 회전교차로 원 안 = 억제 · **준평행 23°
  면제**로 이중도로 쌍 자기 마킹 보호) + `clipMarking`(0.5타일 샘플 분할). 전 선형 마킹 +
  횡단보도 배치 가드에 적용.
- 검증: 사용자 캡처 지점(회전교차로 449,152) 재현 — 관통 마킹 전부 소멸.
- 상세: `docs/wiki/03-WORKLOG/2026-09-02-110-junction-marking-clip.md`

**이전 변경 (2026-09-02 109차) — 래스터 보강 Phase 1·2 실행 (Sentinel-2 → terrain 병합)** (사용자 결정: Antigravity 대신 직접 실행 — 좌표 정합 98.84%·회귀 0·FPS 60·pageerror 0, 빌드 3/3·typecheck 0):

- **신규 도구 2종**: `tools/fetch_region_raster.py`(Process API — UTM 원격자 10m + 사이드카 meta,
  rasterio 회피 = tifffile+pyproj) · `tools/classify_raster.py`(역방향 샘플링·NDWI/NDVI·피복·
  얇은 런 b — 스펙 §3-2 검출식 2종은 실데이터 불일치로 폐기·정정).
- **병합 패스**(`build_osm_tilemap.py merge_raster` + `--no-raster`): OSM 기본값 `'.'`만 채움 —
  속초 식생 58,194·포장 46,276·모래 880·내수면 75·b 497. **b diff = 합의 833/래스터만 497/
  추론만 1,750 유지(삭제 0 — 개정안 §2)**. `--no-raster` 재빌드 = 커밋 기준선과 동일.
- **모래 임계 실측 확정**: `B04 > 0.22 ∧ (B04−B02) > 0.06`(모래 0.290/0.149 vs 콘크리트
  0.105/0.015) — 기본값이면 항만 안벽이 전부 모래가 된다.
- 검증: 도달성 5지점 O · b 3,080타일·103성분·바다 비접촉 0 · sea ratio 불변 · 패치 321 보존 ·
  실렌더 4지점(FPS 60). © Copernicus Sentinel data 크레딧 + 메인화면 푸터 갱신.
- 잔여: **사용자 육안 확정**(도심 'r' 체감) · Phase 3 DEM 도엽 재확보 · b/s 보너스 배선.
- 상세: `docs/wiki/03-WORKLOG/2026-09-02-109-raster-phase12-exec.md`

**이전 변경 (2026-09-02 108차) — 래스터 보강 스펙 개정안 문서화** (107차 §6 잔여 착수 — 코드 변경 0·문서 전용):

- **`.agents/RASTER_UPLIFT_AMENDMENT.md` 신설**(개정 정본 — §8-7 교체 시 발판 79% 상실 실측) +
  후속: 사용자 제공 스펙 전문을 **`.agents/RASTER_UPLIFT_SPEC.md`로 레포 배치**(§0.5 개정 노트 연결).
  ⚠ 블로커: 루트 `.env` 부재(CDSE 자격증명 — step 1 전 사용자 배치 필수).
- 개정 3건: **Phase 1 재정의**("b 5→400 발굴" 폐기 — b는 이미 2,583타일·118성분, 위성↔추론
  diff 검증·정밀화로 격하) · **§6 "래스터 b vs 추론 b" 병합 4케이스**(+patch 최우선·diff 리포트·
  회귀 기준) · **§8-7 개정**(교체 폐기 → 현행 유지 + `isFishableStandTerrain` 스폰/입질 보너스).
- 잔여: b/s 보너스 배선(코드 — 사용자 승인 후) · 발주 실행(사용자).
- 상세: `docs/wiki/03-WORKLOG/2026-09-02-108-raster-amendment-doc.md`

**이전 변경 (2026-09-02 107차) — 래스터 보강 발주 선행 정정** (스펙 §0-2 + 사용자 지시 — 빌드 **3/3**·typecheck 0):
...
- **입자 정합 실측**: 맵 규칙은 2px(Kenney 16px ×2)인데 실사 셀만 1px이라 인접 대비가 4~20배
  (잔디 0.85 ↔ 상판 3.2~8.6 ↔ 사석 15~20). **밝기는 원래 정합**(상판 149.5 ↔ 콘크리트 143)이었다.
  → `_clean(grain=2)` 신설로 해안·TTP 리베이크 + **Gemini 생성본 도트화**(22색·2px — 15k~24k색 해소).
- **해변↔포장**: 45° 삼각 톱니 + 빗변선 + 검정 접경선 3중 → **선 전면 제거 + 노이즈 모래 스필**
  (`drawSandSpill` — 2px 알갱이·최대 ≈1/3타일)로 경계를 녹였다.
- **해변↔바다**: 접경 풀에서 코너·`edge_still`(좌열 투명) 제외 → **포말 2종 + 접경축 미러 4변형**.
  절차 젖은 모래 6px 띠는 접경 셀이 있으면 생략(연석형 세로줄 제거). 수심 버킷 경계는 알갱이 전이.
- **편집기(F7)**: 시트 셀 **126 타일 + 오브젝트 27**을 카탈로그(`TileCatalog.ts` 자동 생성)로 노출 ·
  **회전·반전(R/X/Y)** · **겹침 허용(O)** 자유 배치(0.25타일·충돌 없음) · `RegionPatch.tileTex` 신설.
- ⚠ **`'b'` 자동 규칙 폐기** — 상판 변형 선택만 남기고 사석 자동 분류·접경 오버레이·**자동 테트라포드
  피복**을 제거(사용자: "개별 타일로 구축하라고 했지, 한 타일에 전부 적용하라고 한 게 아니다").
  외해측 소파블록은 편집기에서 겹쳐 쌓는다. 테트라포드 스프라이트도 **1타일 이하**(32/24/18px).
- **후속(같은 날 — 리포트 2건 + 지시 3건)**: 이중도로(oneway 쌍) **황색 중앙선**(`dualBoundaryOffset`) ·
  골목 접속부 마킹 무절단(`otherHalf ≥ max(1.25, halfW/2)` — 주기적 정지선 해소) · 횡단보도/정지선/
  스크램블 **아스팔트 클립**(`onAsphalt` — 직선 횡단보도 우선·반쪽 X는 통째 생략) · 서비스 골목(w2)
  중앙선 제거 · **차도 안 건물 타일 274개 런타임 정리**(`trimBuildingsOnRoads` — 저장 diff 제외) ·
  편집기 **호버 고스트**(실물 반투명 미리보기 — 회전·반전 반영). Q&A: **겹침 허용(free) 오브젝트는
  이동도 안 막는다**(충돌 바디 자체가 없음 — 막을 구조물은 겹침 끄고 배치).
- 상세: `docs/wiki/03-WORKLOG/2026-09-02-106-tile-grain-editor-catalog.md`

**이전 변경 (2026-09-01 105차) — 해안 시트 3장 에셋화 + 방파제 실사 타일·갯바위 산포** (사용자 에셋 3장 + 지시 — 실렌더 4지점·60FPS·pageerror 0, 빌드 4/4·typecheck 0):

- **입력 3장의 성격이 달랐다** — `돌 방파제 그리드`(실사 항공사진 + 사용자 격자 96셀) /
  `방파제 바위…모서리`·`부두 플랫폼 모서리`(검정 테두리 셀만 타일, 제목·라벨 제외).
  ⚠ 바위 셀의 "흰 배경"은 **알파 0** — RGB로 테두리를 찾으면 셀 전체가 검정으로 잡힌다.
- `tools/extract_tileset_assets.py coast` 신설 → **51 스프라이트**(상판 4+이음 · 사석 4 · 두부 2 ·
  사석 발치 2 · 부두 모서리 4 · 바위 20 · Rock-Water 4 · 물 상세 7). **그룹 톤 정규화** 신설
  (실사 셀 휘도 123~153 → 149대 — 안 맞추면 바둑판 이음매).
- **방파제 `'b'` 렌더 전면 교체** — 물 비접촉 = 상판 / 외해측·3면 물 = 사석 사면 / 항내측 = 상판
  + 물 타일 쪽 안벽 모서리 오버레이. 분류 실측: 상판 1,876 · 사석 628 · 이음 79.
- **갯바위 산포** = 절차 사각형 → 실사 바위 스프라이트(조도 둘레 2타일).
- 잔여: 물 상세(코스틱·그라데이션) 편입 여부 · `submerged_*` 셀 재선정 · 두부 끝단 배치 ·
  부두 코너 셀. **조도 위성 픽셀 시트는 사용자 작업 중**.
- 상세: `docs/wiki/03-WORKLOG/2026-09-01-105-coast-tileset.md`

**이전 변경 (2026-09-01 104차) — TTP 타일 시트 그리드화 + 테트라포드 피복·해변 접경·외해 마스크** (사용자 에셋 2장 + 지시 — 실렌더 6지점·pageerror 0, 빌드 4/4·typecheck 0):

- **시트 2장은 격자가 아니었다** — 라벨 목업 스크린샷(셀 55×60/64×64 불균일 · 72×72 스프라이트에
  고유색 1,531개). 실측 크롭 → area 리샘플 → k-means 양자화 추출기 신설
  (`tools/extract_tileset_assets.py ttp` → `public/tileset/ttp/` 25장, 타일 32px = TR 1:1).
- **테트라포드** = 구 한 덩어리(gem 124px 축소) → **소형 유닛 2~3개 클러스터 + 2타일 두 번째 밴드**
  (3크기 × 좌우 플립). F7 편집기 프롭도 새 스프라이트.
- **해변 접경** = 접경 셀 4방위 회전 + **바다 색 투명 오버레이**(시트 청록을 통짜로 깔면 접경마다
  사각 색 패치 — 실렌더 확인) → 103차 절차 서프 밴드 대체.
- **외해 마스크**(`computeOpenSea`) — 구 "바깥 4·7타일 수심 ≥ 5"는 청초호 석호도 통과(인접 물
  75타일 중 41 실측). 맵 경계 물에서 `waterDist ≥ 8`만 타고 확산해 석호·항 내측을 뺀다.
- 잔여: 방파제(호안 캡)·갯바위 시트 = **사용자 제작 중** — 도착 시 `'b'`·암반 렌더 + 콘크리트/모래
  지면 타일을 한 벌로 교체.
- 상세: `docs/wiki/03-WORKLOG/2026-09-01-104-ttp-tileset.md`

**이전 변경 (2026-09-01 103차) — 해안 지형 디테일** (사용자 캡처 7장 리포트 — 실렌더 6지점·pageerror 0, 빌드 4/4·typecheck 0):

- **원인 실측**: "모래 미적용"의 정체 = terrain엔 's'가 이미 있었고 **Kenney sand(크림) ↔ tan
  포장(베이지)이 육안 동일**했던 것. 방파제는 man_made 태그가 아니라 **coastline 조각**(way 3개뿐).
- **파이프라인**(`build_osm_tilemap.py` 후처리): 방파제 추론(양끝-바다 런 ≤ 10타일 + 인프라
  컴포넌트 → `b` 73→2,510타일) · 해변-바다 연결 성장. 속초 재빌드(patch 보존).
- **렌더**(`SeamlessChunks`): 모래 웜 틴트(골든) · 서프 포말 밴드 · 수심 챔퍼 거리+버킷 디더 ·
  테트라포드(외해측 수심 판정 — `ts_gem_tetra` 축소 베이크) · 섬/암초(조도 — 45° 컷·여 산포).
- **후속(같은 날)**: 경계 파편 정리 — 지형 디노이즈(소금-후추 72타일 동화) · 삼각 빗변
  경계선 베이크(사각 테두리와 연속) · 잔디 방향 삼각 비활성(블롭 전담). 실렌더 3지점 확인.
- 상세: `docs/wiki/03-WORKLOG/2026-09-01-103-coast-terrain-detail.md`

**이전 변경 (2026-09-01 102차) — 맵 후속 잔여 일괄 + 섬 지역 분류** (사용자 로드맵 확정 — 실렌더 하네스 5종·pageerror 0, 빌드 4/4·typecheck 0):

- **섬 지역 분류**: `tools/regions_config.py`에 `access` 필드 신설 — 울릉·독도 = `'boat'`
  (도보권 불가 — 배 출조 콘텐츠(흘림낚시·다운샷·갯바위) 시 개방). **OSM 1차 확장 = land 14개** 확정.
- **교통·도로**: 사고 차량 **추월**(180s halt 정체 해소 — 왼쪽 차로 횡 이동·대향차 틈 대기·복귀) ·
  회전교차로 **분리섬**(OSM oneway 쌍 사이 쐐기 — 접근로 10/10 oneway 실측) · **신호 교차로**
  (w≥4×2 정점 클러스터 90곳 — 신호등 프롭 + 대형(half≥3.2)만 스크램블 X자 횡단보도).
- **편집기 새 도로 그리기**(F7 도구 탭): 클릭 정점 · 기존 정점/선 자동 접속(투영점 삽입) ·
  우클릭 취소 · Enter 확정 · 폭 프리셋 4종(roads.json 실측 분포 정합).
- **렌더**: 지면 오토타일(잔디 블롭 16조합 흙 림 · tan/pier 8방위 테두리) · 물 타일셋(수심 버킷
  절차 베이크 — Kenney엔 바다 셀 없음) · 지붕 2×2 패널 타일링 + 대각 스텝 45° 컷.
- 상세: `docs/wiki/03-WORKLOG/2026-09-01-102-map-followups-batch.md`

**이전 변경 (2026-08-28 101차) — dev 맵 편집기(F7)·순간이동 + 차도 벡터 마킹(노란 중앙선·대각선) + 프롭 10종** (사용자 피드백 3건 — 실렌더 PASS·pageerror 0, 빌드 4/4·typecheck 0):
