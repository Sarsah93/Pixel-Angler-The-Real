...
**최근 주요 변경 (2026-08-05 72차) — 장비 시스템 개편: 착용품 인벤토리 이탈 + 인체 배치형 장비창(캐릭터 렌더·드래그 장착·우클릭 해제) + 캐스팅 힌트 장비 게이트** (사용자 지시 3건 — 실마우스 실렌더 검증, 빌드 4/4·typecheck 0):
...
- **★ 작업 이어받기 (2026-08-05 기준) — 진행 상황 & 다음 착수 지점**:
  - **[완료 상태]** 원물 자유 손질(52~63차) · 회썰기 미니게임 2뷰(65·69차) · **사시미 접시 플레이팅 + 가격 체계**(66·68차 — 필렛 = 원물가 분할, 모듬 고정표/단품 = 괴리율 보정식(kg시세×실수율×인분마진)) · 재장착 체인(rib→pin→peel, 67차 필렛 소실 수정) · 도마 조각 스테이징 → 접시 드래그(69차) · 도감/도움말/메인메뉴 안전망(71차) · **장비 시스템 개편(72차 — 착용품 그리드 이탈·인체 배치형 장비창·드래그 장착)**.
  - **[다음 착수 — 사용자 지정 순서, 2026-08-05 갱신]**
    **0. (사용자 진행 중) 수동 검증** — 회뜨기·플레이팅 + **72차 장비 개편**(장비창 디자인·드래그 장착 감각·해제 흐름).
    **1. 넙치류(광어·도다리) 손질 — 다섯장뜨기 트리 분리** (flat 분기 + 전용 뷰/좌표. `filletCount` 4~5 스키마는 이미 있음).
    **2. 두족류(오징어·문어·갑오징어) 전용 손질 트리 신규** — 눈 위 신경·먹물·다리(현재 스텁). 위 둘로 `getButcheryFamily` unsupported/스텁 해제.
