...
> **최종 업데이트**: 2026-08-12 — **91차: 무늬오징어 3구간 재구성(껍질 2회 뜯기·내장 연출 2프레임·날개 2단) — 14스테이지**.
> 상세는 아래 "✅ 직전 완료 (91차)"와 [워크로그 091](../docs/wiki/03-WORKLOG/2026-08-12-091-squid-skin-viscera-fin-rework.md).
>
...
### 🚧 다음 착수 — 껍질 분리(4단계) 재구성 → 수동 검증 계속 → 두족류 3종 확장 → 광어 F9 잔여 → 해루질 (2026-08-11 갱신)

**0. 🚧 무늬오징어 수동 검증 — 진행 중** (90차 1~3단계 실측 → **91차 3구간 재구성 완료**)

- ~~0a. 껍질 분리(4단계) 재구성~~ — **91차 완료**: 껍질 온전(영상 참고 **추론 합성**) → 잡기 →
  가로 뜯기(4.1) → 아래로 마무리(합성 + 실측 스윕 배선) → 완료(4.2). + 내장 뽑기 연출 2프레임 ·
  날개 2단 분리(껍질째 → 날개살). 12→**14스테이지**. 실사 입수 시 합성 키만 교체.
- **0b. 신설·잔여 좌표 F9 실측** — `CEPH_FINSKIN_PATHS`(날개 껍질째 — 91차 근사 신설) ·
  flip_skin 들추기(스프라이트가 합성A로 바뀜) · 아가미·3분할·부리. 절차는 90차와 동일
  (스킬 `f9-guide-coords` + 실판정 스크립트).
- **0c. 합성 3종 품질 확인**(사용자) — `squid_skin_on`·`squid_skin_down`·`squid_finskin_on`은
  추론 이미지다. 어색하면 파이프라인 `deriveSkinOver` 파라미터 조정 또는 실사 교체.
- 함께 확인할 것: 아가미 붙은 안쪽면·껍질 완전 분리본 실사(계속 대기) ·
  사이드바 어류 문구("삼면뜨기 (양살 2필렛 + 중골)") 잔존(3종 확장 시 일괄).

**1. 두족류 나머지 3종** (위 검증 통과 후) — 무늬오징어는 87차에 **완주 가능**해졌다(잠금 해제 완료).

- 스펙 `CEPHALOPOD_BUTCHERY_SPEC.md` §11.3 ④~⑧ 순서:
  ④ 한치 15(`peelStopBand` — 껍질 중단 후 절단) → ⑤ 갑오징어 13(`bone_lift`·속껍질 `peelTool:'towel'`) →
  ⑥ 문어 11(`invert`·`salt_apply`·`flip`·radial 뷰) → ⑦ 수율·등급 → ⑧ 슬라이싱(`whole`).
- 신규 프리미티브 4종(`bone_lift`·`invert`·`salt_apply`·`flip`)은 **타입만 있고 판정 미구현** —
  ⑤⑥에서 `primitiveInput` 분류 등록 + 판정 함수를 함께 만든다.
- **소모품 2종 등록 선행**(`coarse_salt`·`kitchen_towel`) — ⑤⑥이 요구한다.
- ⚠ 착수 전 §0.5(코드 정합 v3.1) 필독. 87차 구현이 §0.5.4(스테이지 1개=작업 1개·순서강제)를 따랐으니
  새 종도 같은 규약으로 붙인다.
...
**3. 해루질 관련 작업** — 손질 어종 확장이 끝나면 착수. **퀘스트/스토리는 모든 컴포넌트 구현 후**(사용자 방침).

### ✅ 직전 완료 (91차, 2026-08-12) — 무늬오징어 3구간 재구성: 껍질 2회 뜯기 + 내장 연출 2프레임 + 날개 2단
