...
### 🚧 다음 착수 — 손질 어종 확장 → 해루질 (사용자 지정 순서, 2026-08-05 갱신)
> **0. (사용자 진행)** 수동 검증 — 회뜨기·플레이팅(조각 → 접시 드래그 감각·가격 체감) + **72차 장비 개편**
>    (인체 배치형 장비창 디자인 · 인벤 → 슬롯 드래그 장착 · 우클릭 해제 · 칸 부족 안내).
> **1. 넙치류(광어·도다리) 손질 — 다섯장뜨기 트리 분리** — `buildButcheryStages` flat 분기
>    (중앙선 반신 경계 → 중골 위 분리 → 상·하 양측 반복, `filletCount` 4~5) + 광어 전용 뷰/좌표.
>    현재 flat 프로필·문구는 있으나 섹션 트리는 원형어와 공용.
> **2. 두족류(오징어·문어·갑오징어) 전용 손질 트리 신규** — 눈 위 신경·먹물·다리
>    (현재 cephalopod = 도마 게이트만 있는 스텁). 위 둘로 `getButcheryFamily` unsupported/스텁 해제.
...
### ✅ 직전 완료 (72차, 2026-08-05) — 장비 시스템 개편 (착용품 인벤 이탈 + 인체 배치형 장비창 + 캐스팅 게이트)
