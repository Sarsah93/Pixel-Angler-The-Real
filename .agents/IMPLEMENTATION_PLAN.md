...
### 📋 다음 착수 작업 (코드 — 우선순위 순)
1. **UI 전수조사 고순위 4건 수정** — ShopPanel 그리드 스크롤 부재 · EquipmentPanel/LicensePanel 이름 오버플로 ·
   ButcheryPanel 결과창 desc↔버튼 겹침. (AGENTS §4 "UI 레이아웃 검수 정책" 기준 — 전 팝업 최장/최다 콘텐츠로 재검수)
2. **필렛 재장착 배선 + 껍질 있는 필렛 스프라이트** — 인벤 '껍질·갈빗대 붙은 필렛'(`trim_fillet_ribs`)을
