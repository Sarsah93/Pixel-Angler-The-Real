...
> **최종 업데이트**: 2026-07-21 (19차 — 자전거 정·후면 z 정합 + `inv_bike` 아이템 연동(보유 시 R) + 탑승 중 캐스팅 무반응 + **파이트 드래그인**(제압 후 3m까지 릴링으로 끌어와야 랜딩) + **신선도 상세 v1**(단계 체인·남은 시간·환경 계수·활용 보정, lazy 감쇄) + **백운포 방파제 자동 연결**(`connect_components` — 부산 재생성) + 날씨 연출 강화(비 2레이어·물파문·우박). 18차: 접지/크기·액션 잠금)
