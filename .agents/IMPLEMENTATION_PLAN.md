...
### 🚧 다음 착수 — 광어 가이드 조정 → 두족류 → 해루질 (사용자 지정 순서, 2026-08-05 야간 갱신)
> **1. 광어류 가이드 검증 및 조정** (사용자 실플레이 피드백 기반):
>    - **F9 실측 좌표 반영** — `FLAT_GUIDE`(core ButcheryProcess) 전 구간이 현재 근사 기본값:
>      시메 탭 · 방혈 호 · **머리 S커브(7점)** · 비늘 스윕 · 꼬리 앞/뒤 · 중앙선 · 상/하 지느러미 경계
...
>      (대문어 giant_octopus는 참문어 트리 공유 여부를 구현 시 결정.)
>    - ⚠ **한치는 어종 DB 미등록** — 에셋(`public/fish/swordtip_squid.png`)만 선로드 상태(20차 노트).
>      **오라클(ORACLE_FISH_DB)+FISH_DATABASE+경락 매핑 4계층 등록 선행** 후 손질 편입.
>    - 트리 구상(리서치 필요): 오징어류 = 몸통-다리 분리 → 내장·먹물주머니 제거(터뜨리면 등급 하락?)
>      → 연골 제거 → 껍질(막) 벗기기 → 몸통 펼치기 / 문어 = 눈 위 신경 절단 → 내장 → 빨판·다리 손질
>      → 소금 문지르기(점액). 갑오징어 = 갑(뼈) 제거 추가. **전용 뷰 스프라이트/파라메트릭 필요**
>      (PixelButcherFish에 cephalopod 렌더 분기 — flat 분기와 같은 패턴).
>    - 산출물: 몸통 회(오징어 사시미 — 회썰기 연동), 다리, 먹물(요리 재료), 내장(폐기/밑밥).
>      trimmings 에셋은 사용자 제공 대기 (필요 목록을 구현 시 정리해 요청).
>    - 완료 시 `getButcheryFamily` cephalopod 스텁 해제 + UtilizationPanel [손질 시작] 활성.
> **3. 해루질 관련 작업** — 손질 어종 확장이 깔끔하게 끝나면 바로 착수 (사용자 지정 2026-08-05.
...
### ✅ 직전 완료 (73차, 2026-08-05) — 외부 QA 버그 수정 2건 (접시 회전 렌더 + 씬 전환 검은 화면 전수조사)
