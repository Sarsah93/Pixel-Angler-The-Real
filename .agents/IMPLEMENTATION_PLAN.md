...
### 🚧 다음 착수 — **광어 가이드 완료 → 두족류 손질 트리** → 해루질 (2026-08-07 갱신)
> ⚠ **순서 확정(사용자 2026-08-07): "광어 가이드가 다 끝나야 두족류를 시작할 수 있어"** — 아래 2번(광어 잔여)이
> 선행이고, 그중 F9 실측은 사용자 측정 대기다. 측정본이 오면 반영(스킬 `f9-guide-coords`) 후 두족류로 넘어간다.
> **1. 두족류 손질 트리 구현** (광어 완료 후). 준비물은 80차에 전부 갖춰졌고 **트리부터가 미착수**다.
...
>      (85차 개방 클리핑은 렌더 층뿐이라 84차 기준 그대로 유효).
