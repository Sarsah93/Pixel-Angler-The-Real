...
> **최종 업데이트**: 2026-08-06 (**80차 — 도마 90° 회전 축 + 광어 포 뜨기 벌어짐 연출 + 배쪽 단면 실사 + 두족류 정합/에셋**: 좌우·상하 뒤집기와 **독립된 회전 축**(`BoardRotation` · R/Shift+R · 세로 도마 프레임 · 좌표 변환 `rotNorm`/`unrotNorm`) 신설 → 넙치류 포 뜨기 11스테이지를 **꼬리 아래 세로 배치**로 지정하고 칼길을 꼬리→머리로 반전 / **칼질 후** 살이 젖혀 열리는 보간 연출(`flatOpenMs`) / 사용자 사진 3번(배쪽 단면) 픽셀화 → `gutsExposed` 상태에 실사 렌더 / **`pixelize_butchery.cjs` 누끼 사문 수정 3건**(bgTol 미전달로 TH가 undefined → 배경 제거 무력화 · 배경색 평균→중앙값 · 알파 경로) / 두족류 **스펙 정합 v3.1 §0.5**(speciesId 4건·프리미티브 실재 6종·심볼 대조표) + core 1단계(뷰 9종·프로필 4종·부산물 19종·tuning 29키) + **에셋 15키** + **부리 공정 `ceph_beak_out` 신설**. 상세는 AGENTS 80차) // **79차 — 한치(창꼴뚜기) 어종 4계층 등록**: 두족류 손질의 선행 조건 해소 — 오라클(`swordtip_squid` · egiOnly · nightBonus 1.9 · 외투장 12~40cm · 야간 표층~중층) + 도감(Uroteuthis edulis · 제철 5~9월 · 회 35,000원/kg) + 텍스처 배선(에셋은 20차부터 선로드였음) + 경락 3곳(**MAFRA `'한치'`가 무늬오징어로 흘리던 오매칭 정정** · KOSIS 전용 항목을 `'오징어'`보다 앞에 배치) + 에기 spawnBinding(egiOnly는 여기 없으면 영영 미스폰). 검증: 야간 표층 83.6% / 바닥 12.3% 수심층 분리 · 도감 카드 실렌더 · pageerror 0. 상세는 AGENTS 79차) // **78차 — 광어 F9 실측 1차 + 배쪽 포 뜨기 3D 렌더 + 작업 패널 2열**(FLAT_GUIDE 실측 7종 · lift 3회화 · 중앙선 힌지 플랩 렌더. 상세는 AGENTS 78차) // 72차 — 장비 시스템 개편: 착용품 인벤토리 그리드 이탈(`SLOT_EQUIPPED` + `EquipResult` 사유 반환 — 칸 부족 "아이템 창 공간이 부족합니다.") + 인체 배치형 장비창(캐릭터 렌더·우클릭 해제·인벤 드래그 장착 `inventory-drop`) + 캐스팅 힌트 장비 게이트(낚싯대 착용 기준 — 미착용 무반응) + `__GS` dev 전역. 상세는 AGENTS 72차) // 71차 — 도마 도움말 팝업(드래그·중앙·X) + 도감 전면 정비(페이징·실사 카드·헤더 겹침) + 메인메뉴 씬 전환 안전망: 어종 도감이 페이징 없이 전 종을 렌더해 화면 밖으로 잘리던 실버그 수정(12종/페이지·5페이지 + 실사 이미지 카드), 조과 기록 7행·헤더 라벨↔버튼 겹침 수정, `MainMenuScene.fadeOutThen`(fadeIn 중 재진입 검정화면 멈춤 방지). 상세는 AGENTS 71차) // 70차 — 방어류 가이드 좌표 어종군 분리(`GUIDE_COORDS{bream|amberjack}` 9세트)+제거 영역(FIN_ERASE.amberjack·HEAD_EXTRA_ERASE)+타이틀 로고 배치 // 이전: 2026-07-31 (**55차 — 밑손질 유도선 틀어짐 수정**: 스프라이트 교체마다 생선이 확대·축소·이동(384↔192↔472px)해 도마 rect 고정 유도선이 어긋나던 것을 **온마리 원형 틀 고정(`computeFishFrame`) + 부위 영역 부분 삭제(`erase` 다각형)** 로 교체 — 머리는 실제 절단선을 따라 삭제(F9 편집 자동 반영), 지느러미는 돌출부만(가슴 제외), **적용은 개복 전 측면 뷰만**. + **dev 항법**(좌측 하단 섹션 건너뛰기 / 개별 작업 11섹션·19작업 점프 — 앞 단계 전부 완료 처리). 상세는 AGENTS 55차) // 54차 — UI 전수조사 고순위 4건**: ShopPanel 윈도우드 그리드 스크롤(마스크 대신 "보이는 행만 생성" — 팬텀 히트 방지, 휠 게이트·스크롤바 썸 드래그·`n–m / N행`) + EquipmentPanel/LicensePanel 이름 상한(신규 `ui/TextFit.ts` — `clampTextWidth` 말줄임 / `fitTextHeight` 축소) + LicensePanel 상세 흐름 배치 + ButcheryPanel 결과창 desc↔버튼 겹침(실측 11px 침범 → 12px 여유). **Playwright(설치 Chrome channel) 실렌더 실측 검증**. 상세는 AGENTS 54차) // 53차 — 회칼 손 장착 버그(**구세이브 시드 복원 마이그레이션** — `deserialize`가 저장 아이템을 verbatim 복원해 52차 이전 세이브의 회칼에 `tool`/`equippable` 누락 → `isKnifeItem` 강제 복원+`placeKey` 백필) + **연출 완료 전 스프라이트/작업/팝업 전환 방지**(액션 핸들러 pre-refresh 생략 + `pendingAfterAction` 큐 — 연출 중 완료 처리(팝업·섹션전환) 큐잉 → onComplete doRefresh 뒤 실행) + **2면(fillet_1)만 새 구조**(구 sever+ribsever 폐기 → **score+ribcut+sever**, ribcut=갈비뼈 끊기 strong·sever=꼬리쪽→배쪽 분리 yields. **1면(fillet_0)은 불변**. 뷰: 등쪽/갈비뼈끊기=측면 `{fam}_spine`/배쪽 sever=정면 `{fam}_belly`) + 코딩 규칙 §4 신설(세이브 시드 복원·UI 레이아웃 검수). 상세는 AGENTS 53차. // 52차 후속 7·8·9 — **장뜨기 뷰 전면 교체**(등쪽 = 등을 카메라 쪽으로·머리 우 / 배쪽 = 배를 카메라 쪽으로·꼬리 우, 칼집 회차별 **벌어짐 4단계**: 닫힘→붉은 살→뼈 노출→반대쪽까지) + **칼이 지나간 뒤 벌어지는 연출**(openOverride로 스테이지 전환 후에도 유지) + 배쪽 3구간 분리 경로·꼬리칼집 실측 좌표 + BACK_DOWN 라벨 '등 위로'로 정정. // 52차 후속 7·8 — **지느러미 3선 실측 좌표 + 선마다 칼 연출**(submitCut `matchedPath` 반환 → 방금 그은 선을 따라 칼 스윕) + **비늘치기 스윕 데이터화·커버리지 게이지**(`ButcheryStage.sweepPath` 신설 — scale_base 20점 지그재그/scale_flip 13점 실측, 44샘플 커버리지 = 진행률·fillTarget 0.92: 제자리 흔들기 0.045로 편법 차단) + **비늘 튐 연출**(커서 호버 위치마다 부채꼴 파티클 + 마무리 애니가 실제 스윕 경로·진행 방향 앞으로 분사) + **복면(뱃살 정면) 뷰·체강 탑뷰 신설**(`tools/gen_butchery_views.cjs` → `PixelFishViews.ts` — 배따기/내장=복면, 핏줄/세척=내장 꺼낸 공간 탑뷰) + **상하 미러 폐지 = 뱃살 항상 아래쪽**. // 52차 — **회뜨기 자유 손질 대개편**: 섹션 11개 순서 강제 + 섹션 내 작업 자유 선택(달성도·'완료됨 N%' 도장), 수동 상하/좌우 뒤집기(자동 뒤집기 폐지), 손 장착 회칼 게이트, 부산물 팝업(보관/버리기)+레저 정산·체크포인트 종료(2면 뜨기/갈빗대 후 필렛만 저장 가능·그 전 이탈=원물 복구), FSM 24스테이지(핏줄/꼬리 뒷면/갈빗대/지아이 신설) + **손질 단계 스프라이트 시스템**(tools/pixelize_butchery.cjs — 실사 누끼→도트 파이프라인·돔류는 가이드 SVG 자동 추출 bream_vessel/fillet1~3·어종군 제네릭 선택+폴백 오버레이) + **trimmings 어종군 매핑**(머리/척추/갈빗대/순수필렛=돔류·방어류 분리, 껍질/내장/지아이=공통 스택) + ESC 메뉴 '집으로 가기'(확인 팝업) + WorldMap 귀가 검정화면 수정 + dev 테스트 어획 6종. 상세는 AGENTS.md 47~52차 참조. // 이전: 2026-07-23 30차 — **FP 찌 채비 심화** (FP_FLOAT_RIG_DEPTH_SPEC): **지형 관통 클램프**(매프레임 바닥−clearance 위로 followRise 3m/s — zMaxM이 하강 한계만 제한해 얕은 여밭 융기 시 바닥을 파고들던 실버그 수정, 조경 sinkMult 상한 1.6) + **구멍찌/수중찌 분리**(흰 몸통 제거·수중찌 별도 드로어 — 회수 [0.90→0, 0.95→0.10, 1→1] 등장·구멍찌 곁 상승) + **입질 단계별 잠김 연속 α**(biteDip 0.06/0.14/0.40m 데이터 주입 → 1단계 α0.79/2단계 0.52/3단계 0 + `floatSubmerged` 래치: 3단계·파이트 완전 숨김→회수 0.90부터 재등장) + **invLerp01/piecewiseLerp 공통 헬퍼**(분기점 급전환 금지) + tuning seabed/zone/float/subfloat 신설·META 5종. 29차 — **접근 연출 피드백 5건**: ① 수중 채비 **후반 램프 α**(선형 이동비 7/8→0.10·7.5/8→0.55·도달→1, 그 전 완전 숨김 — easeOutCubic 조기 노출 정정) + 루어 원줄 수면 종단(수면 아래 비표시) ② 파이트 물고기 = **방향 타원 그림자**(9/10→0.10·9.5/10→0.30·도달→0.50 + 크기 0.6→1.3배, 머리 끝 목줄 연결 — 아이콘·구 fishShadow 폐기) ③ 물고기 횡 견인 → 찌/원줄 앵커 대각 이동 ④ 밑밥 예측 고스트 기본 off(감으로 리드) ⑤ 수평뷰 조류 강도 셰브론(</<</<<<). 28차 — **FP 착수→침강→회수 접근 연출** (FP_CAST_RETRIEVE_SPEC — 구 "중앙·3배" 정정 → **70% 지점·2배**): vp=easeOutCubic(1−distM/castDist) 단일 매핑으로 위치·크기·투명도 구동(착수 castScaleMin 0.72·수중 채비 α0 → 70% 지점 2배·불투명), **원줄 α = 채비α×0.5**(비파이트 연한 흰색 0xeef6ff / 파이팅 텐션색), 물고기 그림자 far 0.15→near 0.90, **SINK CAMEO**(착수 직후 무입력·무조류 800ms α0.5→0+14px 하강, 릴링 시 취소), 로드팁 접근 비례 추가 휨(+10°), 랜딩 420ms 후 결과 팝업(겹침 방지), tuning.ts retrieve 확장(growFactor/castScaleMin/mainLineAlphaFactor/sinkCameo* + META 슬라이더). 매핑 수치 검증: y268→504·scale0.72→1.44·vp0.5에서 물고기 0.53/원줄 0.25. 27차 — **통합 가이드 시스템**: `data/GuideContent.ts`(GUIDES 4카테고리 19페이지 — 파이트5/회수4/밑밥5/회뜨기5) + `ui/GuidePanel.ts` 공용 허브(탭+삽화 카드+◀▶/점 네비 — 새 시스템은 데이터 추가로 끝) + 삽화 `guide_<cat>_<n>.png` 19장(목업 SVG→헤드리스 크롬, fight 로드 곡선 대비 보정) + FP 구 텍스트 가이드 410줄 대체(F1/?/가이드북=허브, 밑밥 버튼=밑밥 탭) + **카테고리별 최초 1회 자동표시** flags `guideSeen.<cat>`(회수=FP 최초 진입/파이트=최초 챔질 성공/밑밥=최초 C/회뜨기=최초 손질 시작→닫으면 ButcheryPanel 진행) + 26차 ChumGuidePanel 허브 흡수·폐기. 잔여: 삽화 실게임 스크린샷 교체·탑다운 허브 진입·i18n. 26차 — **튜닝 중앙화 + 밑밥 가이드**: core `config/tuning.ts`(TUNING/TUNING_META/get·setTuning — feel/balance 분류, 밑밥 종류별 침강 ChumTypeSpec 파우더0.5/압맥0.9/경단1.5), 밑밥 파슬 물리 TUNING 소비 재정렬(시뮬 하네스와 동일 수식), `scripts/chumSyncSim.ts`(조류×밑밥 격자 스윕 — 리드 정합·0.7↑ 8~23% 스킬 신호), client `dev/DevTuningPanel.ts`(F8 슬라이더+스냅샷 복사), FP 씬 TUNING 배선(앵커/배율/투척점/구름/syncToBiteMul), `CoolerStore.chumTypeKey()`(배합→종류), **밑밥 운용 가이드 팝업**(ChumGuidePanel 5페이지 — SVG→PNG 텍스처 5장, C 최초 사용 자동표시 GameState.flags.chumGuideSeen 세이브 + '밑밥' 버튼 재열람). 잔여: fight/rod/yield 테이블 소비 전환, ChumType별 밑밥 spread 시각 차등 dev 관찰. 25차 — **부력찌/수중찌 소켓 분리**: RigStepKey `subFloat` 신설(소켓 8→9 — 부력찌 필수/수중찌·좁쌀봉돌 선택), `InvItem.floatBuoyG` 찌 제원(양수 부력/음수 침력 — U창·1인칭 침강·총중량 공통 소비), 부력찌 6종(제로찌/구멍찌 0.5·0.8·1.0/기울찌/잠길찌)+수중찌 3종 시드, 잠길찌 판정 잔존부력 마진(max(35%, 2.5g) — 표준 매칭 조합 부유), 구세이브 float→subFloat 마이그레이션, 원투 판정 양 소켓 비움 조건. 24차 — **FP 채비 회수·랜딩 세트(최종) + 밑밥 3D 파슬 투척·조류 동조** (FP_RETRIEVE_LANDING_CHUM_FINAL PART A+B): core `ChumParcel/stepChum/computeChumSync(depthGate×horizNear×freshness)/maxChumSync/predictChumPath/optimalThrowX` + FP **투척점 13개 커서 스냅**·예측 드리프트 고스트·동조 피크 마커·세 뷰(정면/수평/수직) 단일 parcel 동시 렌더 + **retrieveGroup 회수 세트**(모드별 구성 — 찌 채비=찌+목줄+봉돌+미끼/루어=단독, retrieveT→easeOutCubic: 중앙~하단 중간 앵커 0.75H·×2·alpha 1 수렴, 원줄은 컨테이너 밖 rodTip→세트top 매 프레임 재드로우) + **dragIn 랜딩 물고기 세트 편입**(머리 카메라쪽+지친 롤+부상). 시뮬: 리드 +7.1m↔그리드 +6.0m 피크 80% / 실시간 피크 t=26.7s≈tSink / 매핑 y272→540·×2. 23차 — **인벤토리 세이브 연동**(아이템/퀵슬롯/채비/편대/루어 모드 — 신선도 실경과 자동 반영·참조 정리) + **어종 이미지 5종**(놀래미/쥐노래미/망상어 + 용치놀래기 암/수 성별 분기) + **숭어(보리숭어)/가숭어(참숭어) 표준명 정정** + **쿨러 세이브 연동**(로드 시 실경과 시간 신선도/매질 만료 반영·밑밥 보존) + **쿨러 판매 가드·밑밥 비우기** + **FP 인벤토리 I 토글**(드래그 이송 대상) + **가이드 개선**(열람 중 낚시 일시정지·가이드북 ? 아이콘·입질 타임라인/텐션 게이지 시각 도해). 22차 — **쿨러 휴대 아이템화**(`inv_cooler` 보유 게이트: 어창/밑밥 전체 연동, 미보유 시 어획 인벤 직행) + **어획 3선택지 팝업**(쿨러/인벤토리/방생 — 쿨러 없으면 비활성) + **쿨러 드래그 앤 드랍 이송**(패널 밖 드랍 = 인벤 이송, 비모달화로 쿨러+인벤 동시 오픈) + **상세보기 신선도 실시간**(단일 상태 색상 표기 + '다음 상태로 변경되기까지 남은 시간 00일 00시 00분 00초' 초단위 카운트) + 활어/필렛 섭취 차단. 21차 — **쿨러 매질 시스템**(해수 1h 활어 무제한→만료 강제 '보통'·경고 / 얼음 2h 활어 1h→신선 정지→만료 강제 '해동' / 하단 3버튼: 해수(두레박+바다근처)·얼음(각얼음 소모)·비우기 / 타이틀 '쿨러 (매질, 00시 00분 00초)') + **신선도 상태 그래프 재설계**(8단계: 활어→신선→보통→나쁨→부패 / 냉동→해동 / 냉장→보통 — 구 냉장→냉동 선형 체인 폐기) + **쿨러 자동 이송 폐지**(우클릭 '인벤토리로 넣기' 수동 이송·이송 시점부터 시계 재시작) + **인벤 우클릭 사용하기(녹색·섭취 SFX)/버리기 빨간색+확인창**. 시뮬 28항목 PASS. 20차: 어종 실사 이미지 19종 + 회뜨기 수율/회칼)
...
### 🚧 다음 착수 — 두족류 손질 트리 → 광어 잔여 → 해루질 (2026-08-06 갱신)
> **1. 두족류 손질 트리 구현** ← **현재 지점**. 준비물은 80차에 전부 갖춰졌고 **트리부터가 미착수**다.
>    - **이미 완료(80차)**: 뷰 유니온 9종 · 프리미티브 11종 추가 · `CephalopodProfile` 4종(+대문어 공유) ·
>      부산물 19종 + 아이콘 배선 · 가이드 좌표 상수 · `tuning.ceph` 29키 + F8 슬라이더 10종 ·
>      **trimmings 에셋 15키**(squid 12 + octopus 3) · 한치 어종 등록(79차).
>    - **미착수**: 스테이지 트리 4종 · 섹션 매핑 · `ButcheryProcess` 신규 프리미티브 판정 ·
>      `CephalopodTemplateRenderer` · `getButcheryFamily` cephalopod 스텁 해제.
>    - **작업 순서** = `CEPHALOPOD_BUTCHERY_SPEC.md` **§11.3의 2단계부터**:
>      ② 무늬오징어 **14**스테이지(부리 공정 포함) + `ButcheryProcess` 분기 + 게이트 해제 →
>      ③ `SquidLayers` 렌더 = **1종 완주 가능** → ④ 한치 15 → ⑤ 갑오징어 13(`bone_lift`·속껍질) →
>      ⑥ 문어 11(`invert`·`salt_apply`·`flip`·방사 뷰) → ⑦ 수율·등급 → ⑧ 슬라이싱 연동.
>    - ⚠ **착수 전 §0.5(코드 정합 v3.1)를 반드시 먼저 읽을 것** — v3 본문은 레포 접근이 끊긴 채
>      작성돼 speciesId·심볼·프리미티브 실재 여부가 실제 코드와 다르다. **충돌 시 §0.5가 우선**.
>    - ⚠ 소모품 2종(`coarse_salt`·`kitchen_towel`) 미등록 — 문어/갑오징어 단계에서 필요.
>    - **사용자 대기 에셋**: 오징어 껍질(`ceph_skin`) · 아가미(`ceph_gill`) · 갑오징어 속껍질(`ceph_inner_skin`).
>      없어도 트리는 진행 가능(아이콘만 비고 텍스트 표기).
> **2. 광어 가이드 잔여**:
>    - **F9 실측 — 84차 재구성으로 좌표 목록 갱신**: `upSep1`·`upSep2`·`dnSep1`·`dnSep2`(경계·중앙선 보간 파생 근사)
>      + `dnScore`(중앙선 미러 근사) + `engawa`. 구 `upLift`/`gutSweep`은 **폐지**(측정 불요).
>      **측정 화면 = 84차(포 뜨기 3단계·칼 팔로우·도마 가로 고정)가 최종 기준** — 그 이전 화면에서 잰 값은 무효.
>    - 칼 연출 체감 튜닝(delay/speed/tilt — F8) 실플레이 조율.
...
### ✅ 직전 완료 (84차, 2026-08-07) — 광어 포 뜨기 공정 재정의: 3단계 × 4작업 + 칼 팔로우 연출
...
> 순수 필렛을 **아이템창 → 도마 드래그 자체가 막혀 있음**(현재 도마 수용은 어획물·두족류·껍질필렛만 — `UtilizationPanel` draggable L1321·drop L1165).
...
>
> **✔ 확정 결정 (사용자, 2026-08-03)**: ① 썰기 뷰 = **파라메트릭 먼저**(`gen_butchery_views.cjs` 방식 — 필렛 측면 썰기 뷰, 나중에 실사 pixelize 교체) ② 컷 판정 = **유도선 드래그**(기존 `guided_cut`·`submitCut` 재사용).
>
> **구현 순서 (A~E)**:
> - **A. 순수 필렛 도마 수용** — `UtilizationPanel`의 draggable(L1321)·drop(`dropFishOnBoard` L1181) 술어에 `inv_fillet_`(≠`inv_filletskin_`/`inv_filletribs_`) 추가 → 도마에 올라오면 [손질 시작] 대신 **[회썰기]** 버튼(가이드는 §손질 시작 L1051 패턴). `this.sashimiPanel` 필드(병렬 `butcheryPanel`)·destroy L1806·`onEscIntercept` L1798 정리 추가.
> - **B. SashimiPanel 신설** — `ButcheryPanel` 모델(DraggablePanel·씬 포인터 판정·`guided_cut` 스트로크). z-plane 14 세로 유도선 / y-plane 16 사선 유도선(근사 기본 좌표 → 사용자 F9 실측). 파라메트릭 측면 썰기 뷰(도마+필렛 블록+컷 진행).
> - **C. 야나기바 게이트** — `getBestKnife(손 장착 회칼).tier === 'yanagiba'`일 때만 y-plane(고급) 활성. `막칼`/`사시미`는 z-plane만.
> - **D. 사시미 산출** — 신규 `inv_sashimi_{speciesId}_{seq}`(category food·subCategory '회(사시미)'·iconTexture `food_assorted_sashimi`), **소/중/대/특대 = 필렛 `weightG` 임계**(기본값 제안 후 튜닝), 등급 = 컷 정확도. `sashimiValuePerKg` 가격식 재사용. 순수 필렛 소모.
> - **E. '요리하기' 메뉴** — `InventoryPanel.openContextMenu`(L349)에 순수 필렛/고급 사시미 대상 '요리하기' 추가(추후 스시 연계 — 지금은 스텁/안내).
> - ⚠ 스키마 부분 스캐폴딩 존재: `Butchery.ts:287`(`loin_clean → 회썰기` `nextStageId:'slice'`)·`ButcheryGuideCuts.ts:138,275`(`→ 회썰기 단계`) — 재사용 검토.
> - 검증: 빌드 4/4·typecheck 0 + Playwright 실렌더(순수 필렛 드롭 → 회썰기 → 사시미 산출·야나기바 게이트 on/off).
...
> 남은 항목: **Q2 🔶 부분 완료**(23차 — 시각 도해 2종·가이드 일시정지·가이드북 아이콘. 잔여: 조류 존/침강/정렬도 상관 도해 확장) → **Q5**(신규 API 5종). Q1 ✅ 완료(9차). Q4는 Q3 프로필 구조로 대비 완료.
>
> **차기 소과제 (2026-07-22 등록)**: ① 사용하기 아이템 효과 적용(HP/피로/버프) ② 쿨러 어획 '손질하기' 메뉴 → ButcheryPanel 연결 ③ 개볼락/창꼴뚜기(한치) DB 등록 + 이미지 매핑(에셋 선로드됨) ④ 회썰기(두께/각도)·두족류 손질 트리 ⑤ 상점 '기타' 매입 신설 시 쿨러 판매 가드 재확인 ⑥ 정식 사운드 에셋 도입(현 WebAudio 플레이스홀더 대체)
>
> **회뜨기 자유 손질 잔여 (2026-07-30 등록 — 사용자 백로그, AGENTS 52차 잔여와 동일)**:
> ⓐ **방어류 실사 스프라이트 투입** — 사용자 제공 4장(핏줄 뷰/등쪽 길내기 1~3)을 `food assets/butchery/amberjack_vessel.png`·`amberjack_fillet1~3.png`로 저장 → `node tools/pixelize_butchery.cjs` (배쪽 사진도 제공 예정: `amberjack_belly_score1~3.png` 권장)
> ⓑ **필렛 재장착** — 인벤의 '껍질과 갈빗대가 붙어있는 필렛'을 도마에 올려 갈빗대 섹션부터 재개 (ButcheryProgress 스키마·아이템 발급은 완비, 도마 수용/재개 배선만)
> ⓒ **회 썰기(12단계)** — 순수 필렛 도마 재장착 → 세로 9등분(일반 회칼)=사시미 / 대각 11등분(야나기바 이상)=고급사시미, 소/중/대/특대 = 원물 크기 연동, 측면 뷰 에셋 별도
> ~~ⓓ dev 가이드선 곡선 편집~~ ✅ **완료(52차 후속 3)** — F9 편집기에 [곡선 그리기](자유 드래그 → Chaikin ×2 + 호길이 리샘플 7점) · [+ 점](중점 삽입 세분화) · [스무딩] · 핸들 우클릭 삭제. core 실판정 검증(곡선 따라감 1.00 통과 / 직선 0.44 실패)
> ~~ⓔ 회뜨기 유도 연출 개정 — VENTRAL 뷰~~ 🔶 **부분 완료(후속 8)** — 복면(뱃살 정면) 뷰 + 체강 탑뷰 신설·배따기/내장/핏줄/세척 배선. 잔여: 영역 유도·홀드 게이지
> ⓕ **UI 전수조사**(텍스트 오버플로/스크롤 미비/HUD 겹침 — 전 팝업 별도 패스) ⓖ **신규 스테이지 가이드선 곡선 실측 튜닝**(vessel/tail_b/rib/pin — 직선 기본값을 곡선 편집기로 생선 실루엣에 맞춤) · 광어(flat) 섹션 트리 분리
> ⓗ **뷰 스프라이트 실사 교체** — 복면/체강 뷰는 현재 파라메트릭 생성분(`tools/gen_butchery_views.cjs`). 실사 사진을 `food assets/butchery/{bream,amberjack}_ventral.png`·`_cavity.png`로 저장 후 `node tools/pixelize_butchery.cjs` 실행하면 자동 우선 적용
> ⓘ **새 뷰 기준 유도 좌표 실측** — 배따기(gut_open 정중선)·내장(gut_scoop)·핏줄(vessel_scrub) 스윕을 F9 편집기로 복면/체강 뷰에 맞춰 재측정([복사] 스니펫이 `sweepPath: [...]` 형식으로 출력됨)
> ⓙ **장뜨기 뷰 실사 교체 + 좌표 미세조정** — `{bream,amberjack}_dorsal0~3`·`_belly0~3`은 파라메트릭 생성분(실사 3장 기준 근사). 사진을 같은 이름으로 넣어 파이프라인 실행하면 교체되고, 등쪽 3점·배쪽 5점(3구간 분리) 경로는 F9로 미세조정 가능
>
> **24차 잔여 튜닝 (FP 회수·밑밥 — dev 실관찰 후)**: ⓐ RETRIEVE_ANCHOR_Y(0.70~0.75H)·세트 배율(1.8~2.2)·원줄 굵기·alpha 시작값 ⓑ 투척점 수/간격(현 13개·1.5m)·sinkRate·spread·동조 시그마·동조→입질 배율 ⓒ 밑밥 종류별 sinkRateMps 차등(현재 전 종 0.22 고정 — CoolerStore 배합 재료 연동) ⓓ 예측 고스트를 실제 stepChum 궤적과 대조하는 dev 오버레이.
