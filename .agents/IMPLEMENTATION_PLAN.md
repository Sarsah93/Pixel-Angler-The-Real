...
> **최종 업데이트**: 2026-08-09 — **87차: 두족류 손질 잠금 해제 (무늬오징어 14스테이지 완주)**.
> 상세는 아래 "✅ 직전 완료 (87차)"와 [워크로그 087](../docs/wiki/03-WORKLOG/2026-08-09-087-cephalopod-squid-tree.md).
>
...
### 🚧 다음 착수 — 무늬오징어 수동 검증 → 두족류 3종 확장 → 광어 F9 잔여 → 해루질 (2026-08-09 갱신)

**0. ⏸ 무늬오징어 손질 수동 실플레이 검증 + 가이드 좌표 F9 실측** ← **여기서부터 재개**

- 89차에 도마 그림이 **실사 도트**로 교체됐다 — 사용자가 dev 모드에서 **가이드 위치를 실측 반영할 차례**다
  (사용자 예고 2026-08-10). ⚠ 좌표 기준이 도마 rect → **피사체 rect**로 바뀌어 87차 근사값은 전량 재측정 대상.
- 함께 확인할 것: `ceph_skin_done`·`ceph_gill_wash` 스프라이트 배정(전용 사진이 없어 근사) ·
  사이드바 어류 문구("삼면뜨기 (양살 2필렛 + 중골)") 잔존.

- 87차 구현은 **자동 검증만** 통과했다(core 실판정 12/12 + Playwright 실렌더).
  사람이 조작해 본 적이 없어 **유도선 위치·드래그 체감·연출 타이밍·안내 문구**는 미확인이다.
  나머지 3종 확장은 이 검증 뒤에 한다.
- 재현 절차·중점 확인 항목 → [워크로그 087 §5](../docs/wiki/03-WORKLOG/2026-08-09-087-cephalopod-squid-tree.md#재현-절차-다음-세션-수동-검증용)
- 요약: dev 서버 → 세이브 **불러오기**(두족류 dev 어획은 로드 시 주입) → 회칼 오른손 착용 →
  U 요리 탭 → 무늬오징어 도마 드래그 → [손질 시작] → 14스테이지.
...
### ✅ 직전 완료 (89차, 2026-08-10) — 무늬오징어 도마 실사 도트 배선 + 좌표계 피사체 기준화
