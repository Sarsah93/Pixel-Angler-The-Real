...
> **최종 업데이트**: 2026-08-11 — **90차: 무늬오징어 가이드 F9 실측 반영(1~3단계) + 트리 12스테이지 개편**.
> 상세는 아래 "✅ 직전 완료 (90차)"와 [워크로그 090](../docs/wiki/03-WORKLOG/2026-08-11-090-squid-guide-coords-stage-trim.md).
>
...
**0. 🚧 무늬오징어 수동 검증 — 진행 중** (90차에 1~3단계 실측·구조 피드백 반영 완료)

- **0a. 껍질 분리(4단계) 재구성** ← **에셋 입수 시 즉시 착수**. 목표 흐름:
  껍질 온전(신규 에셋) → 잡기(전환 연출) → 왼쪽까지 뜯김(현 4.1→4.2) →
  아래로 마무리(`CEPH_PEEL_FINISH_SWEEP` — 실측 보관됨) → 분리 완료.
  대기 에셋: ① 껍질 온전 몸통(필수) ② 분리 완료(권장) ③ 껍질 단독=아이콘(선택) — 요구사항 전달 2026-08-11.
- **0b. 5~6단계(아가미·날개·3분할·부리) F9 실측** — 사용자 측정 계속. 반영 절차는 90차와 동일
  (스킬 `f9-guide-coords` + 실판정 스크립트).
- 함께 확인할 것: `ceph_gill_wash` 스프라이트 배정(제거 완료본 근사 — 가장 어긋남) ·
...
**1b. 두족류 시트 도트 추출** — `gen_ceph_stages.cjs` 신설 → `PixelCephStages.ts`.
현재 `ui/CephalopodFish.ts`는 파라메트릭 플레이스홀더이고 **SVG 4장이 정본**이다.

**1c. 두족류 가이드 F9 실측** — 87차 신설 9경로(펼치기·내장·펜·껍질 들춤·아가미·날개 2선·3분할 2컷)가 근사값.
...
### ✅ 직전 완료 (90차, 2026-08-11) — 무늬오징어 가이드 F9 실측(1~3단계) + 트리 12스테이지 + 당김 연출
