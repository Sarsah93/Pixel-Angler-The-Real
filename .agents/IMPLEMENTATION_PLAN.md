...
> **최종 업데이트**: 2026-07-21 (15차 — **파이트 2D 무대 1인칭 통합**: 원형 상단 앵커 수중 단면뷰(텐션 그라데이션 줄·깊이 알파·제압 부상 롤) + FightingPhase 로드 스티어 밀당(steerDir — 횡 러닝 같은쪽=버티기/반대쪽=제압, 랜딩 45.7→18.4초 검증) + ←/→ 드리프트 다트. 12차 레거시 FishingScene 파이트 2D와 별개로 활성 1인칭 경로에 가시화)
